...
final walletStatsProvider = FutureProvider.autoDispose<WalletStats>((
  ref,
) async {
  final userId = ref.watch(currentUserIdProvider);
  if (userId == null) {
    return WalletStats(totalEarned: 0, totalWithdrawn: 0, currentBalance: 0);
  }
...
final selectedTransactionTypeProvider = StateProvider.autoDispose<String?>(
  (ref) => null,
);

// Transaction search query
final transactionSearchQueryProvider = StateProvider.autoDispose<String>(
  (ref) => '',
);

// Date range filter
final transactionDateRangeProvider = StateProvider.autoDispose<DateRange?>(
  (ref) => null,
);
...
final createWithdrawalRequestProvider =
...
final refreshWalletProvider = Provider.autoDispose<Future<void> Function()>((
  ref,
) {
  return () async {
