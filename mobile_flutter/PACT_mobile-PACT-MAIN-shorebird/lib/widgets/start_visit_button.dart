...
class _StartVisitButtonState extends ConsumerState<StartVisitButton> {
  ...
  String _getButtonText(bool isCurrentVisit, bool isCapturingGPS) {
    if (isCapturingGPS) return _isArabic ? 'جاري التقاط GPS...' : 'Capturing GPS...';
    ...
    try {
      ...
      if (mounted) {
        ...
        AppSnackBar.show(
          ...
          message: hasConnection
              ? (_isArabic ? 'تم بدء الزيارة بنجاح!$accuracyText' : 'Visit started successfully!$accuracyText')
              : (_isArabic ? 'تم بدء الزيارة بدون اتصال - ستتم المزامنة عند الاتصال$accuracyText' : 'Visit started offline - will sync when online$accuracyText'),
      ...
      if (mounted) {
        final message = isNetworkError
            ? (_isArabic ? 'لا يوجد اتصال بالإنترنت. يرجى التحقق من الشبكة والمحاولة مرة أخرى.' : 'No internet connection. Please check your network and try again.')
            : (_isArabic ? 'فشل بدء الزيارة: ${e.toString()}' : 'Failed to start visit: ${e.toString()}');
        AppSnackBar.show(
          context,
          message: message,
