...
class BulkLocalityPermitUploadDialog extends StatefulWidget {
  final List<LocalityPermitData> localities;
...
class _BulkLocalityPermitUploadDialogState
  ...
  int get _totalSitesCount {
    return _permitItems
        .where((item) => item.isReady)
        .fold(0, (sum, item) => sum + item.siteIds.length);
  }
  ...
  Future<void> _uploadAllPermits() async {
    ...
    if (_successCount > 0 && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            '$_successCount locality permit(s) uploaded successfully${_errorCount > 0 ? ', $_errorCount failed' : ''}',
          ),
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  l10n?.translate('bulkLocalityPermitUpload') ??
                ...
                Text(
                  '${widget.localities.length} ${l10n?.translate('localitiesSelected') ?? 'localities selected'}',
                  style: GoogleFonts.poppins(
                    fontSize: 13,
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              l10n?.translate('bulkLocalityUploadInstructions') ??
  ...
  Widget _buildPermitItemCard(
      int index, BulkLocalityPermitItem item, AppLocalizations? l10n) {
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        '${item.state} - ${item.siteIds.length} ${item.siteIds.length == 1 ? 'site' : 'sites'}',
                        style: GoogleFonts.poppins(
                          fontSize: 12,
                          color: Colors.grey.shade600,
                        ),
