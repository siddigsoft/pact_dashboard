...
class _DownPaymentRequestDialogState
  ...
  void _setQuickAmount(double pct) {
    ...
    _amountController.text =
        val == val.floorToDouble() ? val.toStringAsFixed(0) : val.toStringAsFixed(2);
  ...
  bool get _isValidAmount =>
      _parsedAmount != null && _parsedAmount! > 0 && _parsedAmount! <= _budgetUnits;
    ...
    try {
      await ref
          ...
          .createRequest(
            ...
            paymentType:
                _mode == _RequestMode.full ? 'full_advance' : 'installments',
  ...
  Widget _buildBudgetStrip(AsyncValue<List<SiteVisit>> siteVisitsAsync) {
    return Container(
      ...
      child: siteVisitsAsync.when(
        ...
        data: (all) {
          final visits =
              all.where((v) => v.status.toLowerCase().startsWith('accept')).toList();
  ...
  Widget _buildModeSection() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _sectionLabel(_isAr ? 'طريقة الاستلام' : 'How would you like to receive it?'),
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        child: Column(
          children: [
            Icon(
              isFull ? Icons.check_circle_outline_rounded : Icons.splitscreen_rounded,
              size: 22,
            ...
            Text(
              isFull
                  ? (_isAr ? 'استلام كامل المبلغ دفعة واحدة' : 'Receive the full budget at once')
                  : (_isAr ? 'اختر مبلغاً أقل من الإجمالي' : 'Request less than the total'),
              textAlign: TextAlign.center,
              style: GoogleFonts.poppins(
                fontSize: 9,
                color: selected ? AppColors.primaryBlue : Colors.grey.shade400,
  ...
  Widget _buildAmountSection() {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _sectionLabel(_isAr ? 'المبلغ المطلوب (SDG)' : 'Requested Amount (SDG)'),
        const SizedBox(height: 10),
        ...
        TextField(
          ...
          decoration: InputDecoration(
            ...
            disabledBorder: OutlineInputBorder(
              borderRadius: BorderRadius.circular(14),
              borderSide:
                  BorderSide(color: AppColors.primaryBlue.withValues(alpha: 0.4)),
            ),
