...
class AdvanceReceiptConfirmationDialog extends StatefulWidget {
  ...
  const AdvanceReceiptConfirmationDialog({
    super.key,
  ...
  @override
  State<AdvanceReceiptConfirmationDialog> createState() =>
      _AdvanceReceiptConfirmationDialogState();
}

class _AdvanceReceiptConfirmationDialogState
    extends State<AdvanceReceiptConfirmationDialog> {
  String _method = 'uuid';
  ...
  Future<void> _confirmReceipt() async {
    if (_method == 'handwriting' && _points.where((p) => p != null).length < 10) {
      setState(() {
    ...
    try {
      ...
      final userName = profile?['full_name'] as String? ?? user.email ?? 'Unknown';

      final request = await Supabase.instance.client
          .from('down_payment_requests')
          .select('requested_by, status, metadata')
          .eq('id', widget.requestId)
          .single();

      if (request['requested_by'] != user.id) {
        throw Exception(widget.isArabic
            ? 'يمكن فقط لمقدم الطلب تأكيد الاستلام'
      ...
      if (status != 'partially_paid' && status != 'fully_paid') {
        throw Exception(widget.isArabic
            ? 'لا يمكن تأكيد الاستلام إلا للسلف المدفوعة'
            : 'Can only confirm receipt for paid advances');
      }

      final existingMetadata = request['metadata'] as Map<String, dynamic>? ?? {};
      final existingRc = existingMetadata['receipt_confirmation'] as Map<String, dynamic>?;
      if (existingRc != null && existingRc['confirmed'] == true) {
        throw Exception(widget.isArabic
      ...
      final signatureHash = _generateHash('$signatureId:${user.id}:${widget.requestId}:$now');

      final updatedMetadata = Map<String, dynamic>.from(existingMetadata);
      updatedMetadata['receipt_confirmation'] = {
        'confirmed': true,
        'confirmedBy': user.id,
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              widget.isArabic
                  ? 'تم تأكيد استلام السلفة بنجاح'
                  : 'Advance receipt confirmed successfully',
            ),
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            isArabic ? 'تأكيد استلام أموال السلفة' : 'Confirm advance funds received',
                            style: GoogleFonts.poppins(
                              fontSize: 12,
                  ...
                  child: Column(
                    children: [
                      ...
                      _buildDetailRow(
                        ...
                        'SDG ${widget.amount.toStringAsFixed(0)}',
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          isArabic
                              ? 'بتأكيد الاستلام، أنت تقر بأنك استلمت أموال السلفة.'
                              : 'By confirming receipt, you acknowledge that you have received the advance funds.',
                          style: GoogleFonts.poppins(
                            fontSize: 11,
  ...
  Widget _buildMethodOption(String method, String label, IconData icon) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Column(
          children: [
            Icon(icon, color: isSelected ? AppColors.primaryBlue : Colors.grey[500], size: 28),
