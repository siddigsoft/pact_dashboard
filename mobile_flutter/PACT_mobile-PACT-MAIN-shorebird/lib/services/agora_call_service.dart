...
class AgoraCallService {
  ...
  final _incomingCallController = StreamController<AgoraIncomingCall>.broadcast();
  Stream<AgoraIncomingCall> get incomingCallStream {
    _agoraLog('incomingCallStream getter called, hasListener=${_incomingCallController.hasListener}');
      ...
      _engine!.registerEventHandler(
        RtcEngineEventHandler(
          onJoinChannelSuccess: (RtcConnection connection, int elapsed) {
            _agoraLog('onJoinChannelSuccess', 'channel=${connection.channelId} localUid=${connection.localUid}');
          ...
          onUserOffline: (RtcConnection connection, int remoteUid, UserOfflineReasonType reason) {
            _agoraLog('onUserOffline', 'remoteUid=$remoteUid reason=${reason.name}');
            if (_remoteUid == remoteUid) {
              _remoteUid = null;
              _remoteUserController.add(null);
              // Remote left the channel (e.g. caller hung up) — end our side so call ends on both
              if (_isInCall) {
                _agoraLog('onUserOffline', 'remote left channel, ending call');
                endCall();
              }
            }
          },
          ...
          onConnectionLost: (RtcConnection connection) {
            _agoraLog('onConnectionLost', 'attempting reconnect, _isInCall=$_isInCall');
  ...
  Future<void> _setupSignalingChannel() async {
    ...
    _signalingChannel!
        .onBroadcast(
          event: 'agora-signal',
          callback: (payload) {
            _agoraLog('_setupSignalingChannel() RAW broadcast received', payload);
  ...
  void _handleSignal(Map<String, dynamic> payload) {
    ...
    try {
      ...
      final Map<String, dynamic> signalMap = payload['payload'] is Map<String, dynamic>
          ? payload['payload'] as Map<String, dynamic>
          : payload;
      final signal = CallSignal.fromJson(signalMap);
      _agoraLog('_handleSignal() parsed', 'type=${signal.type.name} from=${signal.from} to=${signal.to} callId=${signal.callId}');
      ...
      if (signal.to != _userId) {
        _agoraLog('_handleSignal() IGNORE not for us', 'to=${signal.to} _userId=$_userId');
        return;
      }

      _agoraLog('_handleSignal() received', '${signal.type.name} from ${signal.fromName}');
  ...
  void _handleIncomingCall(CallSignal signal) {
    _agoraLog('_handleIncomingCall() ENTER', 'callId=${signal.callId} _isInCall=$_isInCall');
    if (signal.payload == null ||
        signal.payload!['callId'] == null ||
        signal.payload!['channelName'] == null) {
      _agoraLog('_handleIncomingCall() INVALID', 'missing payload.callId or payload.channelName');
      return;
    }
    ...
    final isAudioOnly = signal.payload?['isAudioOnly'] as bool? ?? signal.isAudioOnly ?? false;

    // Deduplicate: ignore repeat callRequest for same callId
    if (_processedIncomingCallIds.contains(callId)) {
      _agoraLog('_handleIncomingCall() DUPLICATE', 'callId=$callId already shown');
      return;
    }
    ...
    _agoraLog('_handleIncomingCall() pushing to incomingCallStream', 'callId=$callId channelName=$channelName');
    _agoraLog('_handleIncomingCall() controller state', 'isClosed=${_incomingCallController.isClosed} hasListener=${_incomingCallController.hasListener}');
    try {
      _incomingCallController.add(incomingCall);
      _agoraLog('_handleIncomingCall() SUCCESS pushed to stream', 'callId=$callId');
    } catch (e, st) {
  ...
  void _handleCallAccepted(CallSignal signal) {
    _agoraLog('_handleCallAccepted()', 'signal.callId=${signal.callId} _currentCallId=$_currentCallId');
    if (signal.callId == _currentCallId) {
      _agoraLog('_handleCallAccepted() emitting accepted', 'channel=$_currentChannelName');
  ...
  void _handleCallRejected(CallSignal signal) {
    _agoraLog('_handleCallRejected()', 'signal.callId=${signal.callId} _currentCallId=$_currentCallId');
    if (signal.callId == _currentCallId) {
      _isInCall = false;
      _currentCallId = null;
  ...
  void _handleCallEnded(CallSignal signal) {
    ...
    _agoraLog('_handleCallEnded()', 'endedCallId=$endedCallId _currentCallId=$_currentCallId from=${signal.from}');
    if (endedCallId != null &&
      ...
      _agoraLog('_handleCallEnded() IGNORED', 'callId mismatch or not from remote');
    }
  }
  ...
  Future<AgoraCallResult> startCall({
    ...
    _agoraLog('startCall() ENTER', 'remoteUserId=$remoteUserId remoteUserName=$remoteUserName audioOnly=$audioOnly _isInCall=$_isInCall');
    if (_isInCall) {
    ...
    if (!kIsWeb && _engine == null) {
      ...
      return AgoraCallResult(success: false, error: 'Call service not initialized');
    }
    ...
    try {
      ...
      _agoraLog('startCall() state set', 'callId=$_currentCallId channel=$_currentChannelName');
      ...
      _agoraLog('startCall() sending callRequest', 'to=$remoteUserId channel=$_currentChannelName');
      await _sendSignal(
        remoteUserId,
  ...
  Future<AgoraCallResult> acceptCall(AgoraIncomingCall incomingCall) async {
    _agoraLog('acceptCall() ENTER', 'callId=${incomingCall.callId} channel=${incomingCall.channelName} callerId=${incomingCall.callerId}');
    ...
    try {
      ...
      _agoraLog('acceptCall() sending callAccept', 'to=${incomingCall.callerId} channel=${incomingCall.channelName}');
      await _sendSignal(
      ...
      _agoraLog('acceptCall() DONE success', 'channelName=${incomingCall.channelName}');
  ...
  Future<String?> _fetchRtcToken(String channelName) async {
    try {
      ...
      if (response.status != 200) {
        _agoraLog('_fetchRtcToken() ERROR', 'status=${response.status} ${response.data}');
  ...
  Future<void> joinChannel(String channelName, {String? token}) async {
    ...
    try {
      ...
      if (rtcToken == null || rtcToken.isEmpty) {
        _agoraLog('joinChannel() ERROR', 'No token available. Ensure Edge Function agora-token is deployed and secrets are set.');
  ...
  Future<void> rejectCall(AgoraIncomingCall incomingCall) async {
    _agoraLog('rejectCall()', 'callerId=${incomingCall.callerId} callId=${incomingCall.callId}');
  ...
  Future<void> endCall() async {
    _agoraLog('endCall() ENTER', '_remoteUserId=$_remoteUserId _currentCallId=$_currentCallId');
    if (!_isInCall) {
    ...
    if (_processedIncomingCallIds.length > 50) _processedIncomingCallIds.clear();
  ...
  Future<void> _sendSignal(
    ...
    _agoraLog('_sendSignal() ENTER', 'to=$toUserId type=${type.name} payload=$payload isAudioOnly=$isAudioOnly _userId=$_userId');
    if (_userId == null) {
