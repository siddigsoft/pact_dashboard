...
import 'package:pact_mobile/helpers/audio_helper.dart';
...
class CallSignal {
  ...
  static CallSignalType _stringToSignalType(String? type) {
    switch (type) {
      case 'offer':
        return CallSignalType.offer;
      case 'answer':
        return CallSignalType.answer;
      case 'ice-candidate':
      case 'iceCandidate':
        return CallSignalType.iceCandidate;
      case 'call-request':
      case 'callRequest':
        return CallSignalType.callRequest;
      case 'call-accepted':
      case 'callAccept':
      case 'callAccepted':
        return CallSignalType.callAccepted;
      case 'call-rejected':
      case 'callReject':
      case 'callRejected':
        return CallSignalType.callRejected;
      case 'call-ended':
      case 'callEnd':
      case 'callEnded':
        return CallSignalType.callEnded;
      case 'call-busy':
      case 'callBusy':
        return CallSignalType.callBusy;
      default:
        return CallSignalType.callEnded;
    }
  }
...
class WebRTCCallService {
  ...
  Stream<CallParticipant?> get remoteParticipantStream =>
  ...
  Future<bool> initiateCall({
    ...
    _currentCallId =
        'call_${DateTime.now().millisecondsSinceEpoch}_${_uuid.v4().substring(0, 16)}';
    ...
    debugPrint(
      '[WebRTC] Initiating call to: $targetUserName, callId: $_currentCallId',
    );
  ...
  Future<void> toggleSpeaker() async {
    if (_localStream != null) {
      ...
      if (audioTracks.isNotEmpty) {
        try {
          final currentState = await AudioHelper.getSpeakerphoneOn();
          await AudioHelper.setSpeakerphoneOn(!currentState);
        } catch (e) {
  ...
  Future<void> _handleOffer(dynamic offerData, String from) async {
    ...
    await _sendSignal(CallSignalType.answer, from, payload: answer.toMap());
  ...
  Future<void> _sendBusySignal(
    ...
    await _sendSignalToUser(
      to,
      CallSignalType.callBusy,
      overrideCallId: callId,
      overrideCallToken: callToken,
    );
  }
