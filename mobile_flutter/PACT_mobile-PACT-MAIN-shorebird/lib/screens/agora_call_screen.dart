...
class _AgoraCallScreenState extends State<AgoraCallScreen>
  ...
  void initState() {
    super.initState();
    debugPrint('[AgoraCall] AgoraCallScreen initState() channel=${widget.channelName} remoteUserName=${widget.remoteUserName}');
    _initAnimations();
  ...
  void _subscribeToCallState() {
    debugPrint('[AgoraCall] AgoraCallScreen _subscribeToCallState() subscribing to callStateStream');
    _callStateSubscription = _agoraService.callStateStream.listen((state) {
      debugPrint('[AgoraCall] AgoraCallScreen callStateStream event: status=${state.status} error=${state.status}');
      if (!mounted) return;

      switch (state.status) {
        case CallStatus.connected:
          debugPrint('[AgoraCall] AgoraCallScreen state=connected, setting _isConnected=true');
          setState(() {
            _isConnecting = false;
            _isConnected = true;
          });
        ...
        case CallStatus.busy:
          debugPrint('[AgoraCall] AgoraCallScreen state=${state.status} calling _endCall()');
          _endCall();
          break;
        case CallStatus.reconnecting:
          ...
          break;
        default:
          debugPrint('[AgoraCall] AgoraCallScreen state=${state.status} (no UI change)');
          break;
      }
    });
  }
  ...
  void _subscribeToRemoteUser() {
    _remoteUserSubscription = _agoraService.remoteUserStream.listen((uid) {
      debugPrint('[AgoraCall] AgoraCallScreen remoteUserStream event: uid=$uid');
      if (!mounted) return;
      setState(() {
        _remoteUid = uid;
  ...
  Future<void> _startCall() async {
    debugPrint('[AgoraCall] AgoraCallScreen _startCall() ENTER channel=${widget.channelName} isOutgoing=${widget.isOutgoing}');
  ...
  void _endCall() {
    ...
    debugPrint('[AgoraCall] AgoraCallScreen _endCall() calling _agoraService.endCall()');
    _agoraService.endCall();
    WakelockPlus.disable();

    if (mounted) {
      debugPrint('[AgoraCall] AgoraCallScreen _endCall() scheduling Navigator.pop()');
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          if (!widget.isAudioOnly && (AgoraCallService.isWeb ? _agoraService.isReady : true))
            IconButton(
              icon: Icon(
          ...
          if (widget.remoteUserAvatar != null)
            ClipOval(
              child: Image.network(
                ...
                height: 32,
                fit: BoxFit.cover,
                errorBuilder: (_, __, ___) => _buildSmallAvatarFallback(),
              ),
            )
          else
