...
void main() {
  group('DistanceHelper', () {
    test('haversine should calculate known distances correctly', () {
      ...
      expect(DistanceHelper.haversine(0, 0, 0, 0), 0.0);
      ...
      expect(antipodes, closeTo(2 * pi * DistanceHelper.earthRadius / 2, 1000));
    });

    test('isWithinRadius should correctly determine point inclusion', () {
      // Test point within radius
      expect(DistanceHelper.isWithinRadius(0, 0, 0, 0.001, 1000), true);

      // Test point outside radius
      expect(DistanceHelper.isWithinRadius(0, 0, 1, 1, 1000), false);

      // Test point exactly on radius
      const lat2 = 0.008983; // Approximately 1000m north
      expect(DistanceHelper.isWithinRadius(0, 0, lat2, 0, 1000), true);
    });
  });
}
