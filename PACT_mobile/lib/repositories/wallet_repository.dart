...
class WalletRepository {
  ...
  /// Stream down payment requests visible to a supervisor for dashboard (same logic as web).
    ...
    if (hubId != null && hubId.isNotEmpty) {
      ...
      return base
          ...
          .map(
            (data) => data
                .map((json) => DownPaymentRequest.fromJson(json))
                .where((r) => r.requestedBy == userId || r.hubId == hubId)
                .toList(),
          );
    }
    return base
  ...
  /// Stream all down payment requests (for admin dashboard stats; RLS applies)
  Stream<List<DownPaymentRequest>> watchAdminDownPaymentRequestsForDashboard() {
    return _supabase
        .from('down_payment_requests')
        .stream(primaryKey: ['id'])
        .order('created_at', ascending: false)
