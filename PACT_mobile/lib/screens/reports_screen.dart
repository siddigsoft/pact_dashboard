...
class _ReportsScreenState extends State<ReportsScreen> {
  ...
  Future<void> _loadFromCache() async {
    ...
    final cachedVisitsData = cachedItem?.data;
    final cachedVisits =
        (cachedVisitsData?['visits'] as List?)
    ...
    final cachedReportsData = cachedReportsItem?.data;
    final cachedReports =
        cachedReportsData?['reports'] as Map<String, Map<String, dynamic>>?;
    ...
    final cachedLocationsData = cachedLocationsItem?.data;
    final cachedLocations = cachedLocationsData ?? {};
    if (cachedLocations.isNotEmpty) {
      _siteLocations = cachedLocations.map(
        (k, v) => MapEntry(
          k,
                    ...
                    decoration: BoxDecoration(
                      gradient: LinearGradient(
                        colors: [
                          ...
                          const Color(
                            0xFF1976D2,
                          ).withValues(alpha: 0.1), // Light blue
                        ],
                        begin: Alignment.topLeft,
                        end: Alignment.bottomRight,
                      ),
