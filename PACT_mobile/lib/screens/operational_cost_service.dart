...
class OperationalCostService {
  ...
  Future<bool> tier2Review({
    ...
    try {
      ...
      final submitterRole = (submission['submitter_role'] ?? '')
          .toString()
          .toLowerCase();
  ...
  String exportToCsv(
    ...
    if (isArabic) {
      buffer.writeln(
        'المرجع,الفئة,نوع التمويل,المبلغ,العملة,الحالة,الوصف,مقدم الطلب,المشروع,المحور,التاريخ,المستوى 1,المستوى 2,المستوى 3,تمت التسوية',
      );
    } else {
      buffer.writeln(
        'Reference,Category,Funding Type,Amount,Currency,Status,Description,Submitter,Project,Hub,Date,Tier 1,Tier 2,Tier 3,Reconciled',
      );
    }
