...
class _ProfileScreenState extends ConsumerState<ProfileScreen>
  ...
  Map<String, String>? _currentClassification;
  ...
  Future<void> _loadLookupNames(PACTUserProfile profile) async {
    ...
    try {
      ...
      try {
        ...
        final allClassifications = (classificationsResponse as List).map((c) {
        ...
        debugPrint('Classifications found: ${allClassifications.length}');

        // Current = the active record (or most recent if none marked active)
        final active = allClassifications
            .where((c) => c['is_active'] == 'true')
            .toList();

        _currentClassification = active.isNotEmpty
            ? active.first
            : allClassifications.isNotEmpty
            ? allClassifications.first
            : null;

        // History = ALL records (including current, marked accordingly)
        // This ensures previous records are always visible
        _classificationHistory = allClassifications;
      } catch (e) {
                            ...
                            _buildSection(
                              ...
                              children: [
                                if (_currentClassification != null) ...[
                                  ...
                                  if ((_currentClassification!['effective_from'] ??
                                          '')
                                      .isNotEmpty) ...[
                                    const SizedBox(height: 8),
                                    _buildInfoRow(
                                      ...
                                      _formatDate(
                                        DateTime.parse(
                                          _currentClassification!['effective_from']!,
                                        ),
                                      ),
                                  ...
                                  if (profile.classification != null &&
                                      profile.classification!.hasRetainer) ...[
                                    const SizedBox(height: 8),
                                    _buildInfoRow(
                                      'Retainer',
                                ...
                                if (_classificationHistory.isNotEmpty ||
                                  ...
                                  Align(
                                    ...
                                    child: TextButton.icon(
                                      onPressed: () =>
                                          _showClassificationHistory(context),
              ...
              children: [
                ...
                if (_classificationHistory.isNotEmpty)
                  Container(
                    ...
                    child: Text(
                      '${_classificationHistory.length} records',
                      style: GoogleFonts.poppins(
                        fontSize: 10,
                        color: AppColors.primaryOrange,
                        fontWeight: FontWeight.w600,
                                ...
                                Row(
                                  children: [
                                    ...
                                    if (assignedAt != null &&
                                      ...
                                      Text(
                                        _formatDate(DateTime.parse(assignedAt)),
                                        style: GoogleFonts.poppins(
                                          fontSize: 10,
  ...
  Widget _buildBankAccountSection(PACTUserProfile profile) {
    ...
    final hasAccount =
        bankAccount != null && (bankAccount.accountNumber?.isNotEmpty ?? false);
      ...
      child: Column(
        ...
        children: [
          ...
          if (hasAccount) ...[
            _buildInfoRow(
              'Account Name',
              bankAccount.accountName?.isNotEmpty == true
  ...
  void _showBankAccountSheet(PACTUserProfile profile) {
    ...
    final branchCodeCtrl = TextEditingController(
      text: existing?.branchCode ?? '',
    );
    final sheetFormKey = GlobalKey<FormState>();
    bool isSaving = false;
    // Resolve existing bank name to a bank in the list (or null if not found)
    String? selectedBankName =
        kSudaneseBanks.map((b) => b.nameEn).contains(existing?.bankName)
        ? existing?.bankName
        : null;
                    ...
                    children: [
                      ...
                      DropdownButtonFormField<String>(
                        ...
                        items: kSudaneseBanks
                            ...
                            .toList(),
                        onChanged: (v) =>
                            setSheetState(() => selectedBankName = v),
                        validator: (v) => (v == null || v.isEmpty)
                            ? 'Bank name is required / مطلوب'
                            : null,
                      ),
                                  ...
                                  try {
                                    ...
                                    if (mounted) {
                                      ScaffoldMessenger.of(
                                        ...
                                        const SnackBar(
                                          content: Text(
                                            'Bank account saved / تم حفظ الحساب البنكي',
                                          ),
                                          backgroundColor: Colors.green,
                                        ),
                                      );
