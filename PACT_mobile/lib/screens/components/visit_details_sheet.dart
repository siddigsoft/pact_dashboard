  ...
  Future<void> _onEndVisitCaptureLocation() async {
    ...
    showDialog(
      ...
      builder: (_) => _ProgressDialog(
        ...
        message: _isArabic
            ? 'جاري التقاط موقع الموقع وإيقاف التتبع...'
            : 'Capturing site location and stopping tracking...',
      ),
    );
    ...
    try {
      ...
      if (!hasService) {
        throw Exception(
          _isArabic ? 'خدمات الموقع معطلة' : 'Location services are disabled',
        );
      }
      ...
      if (!ok) {
        throw Exception(
          _isArabic ? 'فشل حفظ موقع الموقع' : 'Failed to save site location',
        );
      }
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              _isArabic
                  ? 'تم حفظ موقع الموقع وإيقاف التتبع.'
                  : 'Site location saved and tracking stopped.',
            ),
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              '${_isArabic ? 'فشل إنهاء الزيارة' : 'Failed to end visit'}: $e',
            ),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              _isArabic
                  ? 'استغرق التحديث وقتاً طويلاً. يرجى التحقق من الشبكة والمحاولة مرة أخرى.'
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              '${_isArabic ? 'خطأ في إكمال الزيارة' : 'Error completing visit'}: $e',
            ),
          ),
        );
      }
  ...
  void _showReportDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          _isArabic
              ? 'هل ترغب في إرسال تقرير لهذه الزيارة الآن؟'
  ...
  Widget _buildStatusBadge(String status) {
    ...
    switch (status.toLowerCase()) {
      ...
      case 'claimed':
        ...
        statusText = _isArabic
            ? 'محجوز - في انتظار القبول'
  ...
  Widget _buildActionButtons() {
    ...
    if (currentUserId != null) {
      switch (status) {
        ...
        case 'available':
        case 'pending':
          final isClaimedByCurrentUser = _visit.claimedBy == currentUserId;
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text(
              _isArabic
                  ? 'يرجى تقديم سبب لرفض هذه الزيارة:'
      ...
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                '${_isArabic ? 'خطأ في رفض الزيارة' : 'Error rejecting visit'}: $e',
              ),
            ),
          );
        }
