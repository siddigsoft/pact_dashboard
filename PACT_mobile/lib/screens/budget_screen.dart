...
class _BudgetScreenState extends State<BudgetScreen> {
          ...
          setState(() {
            _projects = cachedProjects != null
                ? List<Map<String, dynamic>>.from(
                    (cachedProjects as List).map(
                      (e) => Map<String, dynamic>.from(e),
                    ),
                  )
                : [];
            _budgets = List<Map<String, dynamic>>.from(
              (cachedBudgets as List).map((e) => Map<String, dynamic>.from(e)),
            );
            _isLoading = false;
            _isOffline = true;
          });
  ...
  double _getTotal(String field) {
    return _filtered.fold(
      0.0,
      (sum, b) => sum + ((b[field] ?? 0) as num).toDouble(),
    );
  }
                          ...
                          child: Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(
                                  '${(pct * 100).toStringAsFixed(1)}% utilised',
                                  style: const TextStyle(
                                    color: Colors.white60,
                                    fontSize: 12,
                                  ),
