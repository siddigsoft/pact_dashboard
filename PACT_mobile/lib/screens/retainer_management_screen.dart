...
class RetainerManagementScreen extends StatefulWidget {
  ...
  @override
  State<RetainerManagementScreen> createState() =>
      _RetainerManagementScreenState();
}
...
class _RetainerManagementScreenState extends State<RetainerManagementScreen> {
  ...
  List<Map<String, dynamic>> get _filtered => _filterStatus == 'all'
      ? _retainers
      : _retainers.where((r) => (r['status'] ?? '') == _filterStatus).toList();
  ...
  Widget build(BuildContext context) {
    final total = _retainers.fold(
      0.0,
      (sum, r) => sum + ((r['amount'] ?? 0) as num).toDouble(),
    );
    final paid = _retainers
        .where((r) => r['status'] == 'paid')
        .fold(0.0, (sum, r) => sum + ((r['amount'] ?? 0) as num).toDouble());
        ...
        child: Column(
            ...
            ReusableAppBar(
              ...
              actions: [
                IconButton(
                  ...
                  onPressed: () => _loadRetainers(refresh: true),
                ),
              ],
            ),
          Container(
          ...
          Container(
            ...
            child: SingleChildScrollView(
              ...
              child: Row(
                children: ['all', 'pending', 'processing', 'paid', 'rejected']
                    .map(
                      (s) => Padding(
  ...
  Widget _summaryTile(String label, String value, Color color) => Column(
    children: [
      Text(
        value,
        style: TextStyle(
