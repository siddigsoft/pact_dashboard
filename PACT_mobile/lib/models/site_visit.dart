class SiteVisit {
  ...
  /// Get the MMP file ID (alias for mmpId)
  String? get mmpFileId => mmpId;

  /// Get the MMP file name from additional data
  String? get mmpFileName {
    // First try to get from mmp_files relationship
    if (mmpFiles != null) {
      if (mmpFiles is Map<String, dynamic>) {
        return mmpFiles['name'] as String?;
      } else if (mmpFiles is List && mmpFiles.isNotEmpty) {
        final first = mmpFiles.first;
    ...
    // Fallback to additionalData
    if (additionalData != null && additionalData!['mmp_file_name'] != null) {
      return additionalData!['mmp_file_name'] as String?;
    }
    return null;
  }
  ...
  final int? rating;
  final String? mmpId;
  final dynamic mmpFiles; // MMP relationship data (Map or List)
  ...
  SiteVisit copyWith({
    ...
    return SiteVisit(
      ...
      mmpFiles: mmpFiles ?? this.mmpFiles,
