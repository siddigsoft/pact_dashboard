...
enum OperationalCostStatus {
  ...
  bool get canApprove =>
      this == OperationalCostStatus.pending ||
      this == OperationalCostStatus.underReview;
  bool get canPay => this == OperationalCostStatus.approved;
  bool get isSettled =>
      this == OperationalCostStatus.paid ||
...
class OperationalCostSubmission {
  ...
  double? get reconciledAmount =>
      reconciledAmountCents != null ? reconciledAmountCents! / 100.0 : null;
  bool get isEditable => status == OperationalCostStatus.pending;
  bool get isCancellable =>
      status == OperationalCostStatus.pending ||
      status == OperationalCostStatus.underReview;
  bool get needsReconciliation =>
      fundingType == FundingType.advance &&
      !isReconciled &&
      status == OperationalCostStatus.paid;
  ...
  String get derivedStatus {
    ...
    if (tier1Status == 'rejected' ||
        tier2Status == 'rejected' ||
        tier3Status == 'rejected' ||
        status == OperationalCostStatus.rejected) {
      return 'rejected';
    }
    if (hasThreeTiers) {
  ...
  static Map<String, dynamic> getStatusDisplay(
  ...
  factory OperationalCostSubmission.fromJson(Map<String, dynamic> json) {
    ...
    return OperationalCostSubmission(
      id: json['id']?.toString() ?? '',
      userId:
          json['user_id']?.toString() ?? json['submitted_by']?.toString() ?? '',
      projectId: json['project_id']?.toString(),
      hubId: json['hub_id']?.toString(),
      expenseCategory:
          ExpenseCategory.fromValue(json['expense_category']?.toString()) ??
      ...
      tier1ReviewedBy:
          json['tier1_approved_by']?.toString() ??
          json['tier1_reviewed_by']?.toString(),
      tier1ReviewedAt:
          _parseDateTime(json['tier1_approved_at']) ??
          _parseDateTime(json['tier1_reviewed_at']),
      tier1Notes: json['tier1_notes']?.toString(),
      tier1Status: json['tier1_status']?.toString(),
      tier2ReviewedBy:
          json['tier2_approved_by']?.toString() ??
          json['tier2_reviewed_by']?.toString(),
      tier2ReviewedAt:
          _parseDateTime(json['tier2_approved_at']) ??
          _parseDateTime(json['tier2_reviewed_at']),
      tier2Notes: json['tier2_notes']?.toString(),
      tier2Status: json['tier2_status']?.toString(),
      tier3ReviewedBy:
          json['tier3_approved_by']?.toString() ??
          json['tier3_reviewed_by']?.toString(),
      tier3ReviewedAt: _parseDateTime(
        json['tier3_approved_at'] ?? json['tier3_reviewed_at'],
      ),
      ...
      reconciledAmountCents: json['reconciled_amount_cents'] is int
          ? json['reconciled_amount_cents']
          : null,
      ...
      submitterName:
          json['submitter_name']?.toString() ??
          (json['profiles'] is Map
              ? json['profiles']['name']?.toString()
              : null),
      submitterRole:
          json['submitter_role']?.toString() ??
          (json['profiles'] is Map
              ? json['profiles']['role']?.toString()
              : null),
      projectName:
          json['project_name']?.toString() ??
          (json['projects'] is Map
              ? json['projects']['name']?.toString()
              : null),
      hubName:
          json['hub_name']?.toString() ??
  ...
  static int _parseAmount(Map<String, dynamic> json) {
    if (json['amount_cents'] != null) {
      return json['amount_cents'] is int
          ? json['amount_cents']
          : int.tryParse(json['amount_cents'].toString()) ?? 0;
    }
    if (json['amount'] != null) {
      final amount = json['amount'] is double
          ? json['amount']
  ...
  OperationalCostSubmission copyWith({
    ...
    return OperationalCostSubmission(
      ...
      reconciledAmountCents:
          reconciledAmountCents ?? this.reconciledAmountCents,
      ...
      fundReceiptConfirmedAt:
          fundReceiptConfirmedAt ?? this.fundReceiptConfirmedAt,
      fundReceiptSignatureUrl:
          fundReceiptSignatureUrl ?? this.fundReceiptSignatureUrl,
  ...
  @override
  String toString() =>
      'OperationalCostSubmission(id: $id, amount: $amount $currency, status: ${status.value})';
}
...
class OperationalCostStats {
  ...
  factory OperationalCostStats.fromSubmissions(
    List<OperationalCostSubmission> submissions,
  ) {
    final pending = submissions
        .where((s) => s.status == OperationalCostStatus.pending)
        .toList();
    final underReview = submissions
        .where((s) => s.status == OperationalCostStatus.underReview)
        .toList();
    final approved = submissions
        .where((s) => s.status == OperationalCostStatus.approved)
        .toList();
    final rejected = submissions
        .where((s) => s.status == OperationalCostStatus.rejected)
        .toList();
    final paid = submissions
        .where((s) => s.status == OperationalCostStatus.paid)
        .toList();
    final outstanding = submissions
        .where((s) => s.needsReconciliation)
        .toList();
...
class CostSubmissionPermissions {
  ...
  factory CostSubmissionPermissions.fromRole(String? role) {
    ...
    final isSupervisor =
        r == 'supervisor' || r == 'hubsupervisor' || r == 'hub_supervisor';
    final isFOM =
        r.contains('fom') ||
        r.contains('field operation') ||
        r.contains('fieldoperation');
    final isCoordinator =
        r == 'coordinator' ||
        (r.contains('coordinator') && !r.contains('country'));
    final isCountryDirector = r == 'countrydirector' || r == 'country_director';
    final isDataCollector =
        r == 'data_collector' || r == 'datacollector' || r == 'enumerator';
  ...
  bool canApproveTier2(OperationalCostSubmission submission) {
    ...
    if (submission.tier2Status != null && submission.tier2Status != 'pending') {
      return false;
    }
  ...
  bool canApproveTier3(OperationalCostSubmission submission) {
    ...
    if (submission.tier3Status != null && submission.tier3Status != 'pending') {
      return false;
    }
