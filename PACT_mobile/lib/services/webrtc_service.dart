...
class WebRTCService {
  ...
  // User and Data getters
  String? get userId => _userId;
  String? get userName => _userName;

  // Screen sharing and peer connection getters
  ...
  Future<void> rejectCall(String callerId, String callId) async {
    ...
    // Track the missed call before sending signal
    await _trackMissedCall(
      callerId: callerId,
      callerName: _callState.remoteUserName ?? 'Unknown',
      callerAvatar: _callState.remoteUserAvatar,
    );
  ...
  /// Track a missed call in the database
  Future<void> _trackMissedCall({
    ...
    try {
      ...
      debugPrint('[WebRTC] Missed call recorded for $callerName');
    } catch (e) {
      debugPrint('[WebRTC] Error recording missed call: $e');
    }
  }

  /// Get list of missed calls for the current user
  Future<List<Map<String, dynamic>>> getMissedCalls() async {
    ...
    try {
      ...
      // Filter for non-cleared calls
      final allCalls = List<Map<String, dynamic>>.from(response as List);
      final uncleared = allCalls
          .where((call) => call['cleared_at'] == null)
          .toList();

      debugPrint('[WebRTC] Retrieved ${uncleared.length} missed calls');
      return uncleared;
    } catch (e) {
  ...
  /// Clear a specific missed call
  Future<bool> clearMissedCall(String missedCallId) async {
    ...
    try {
      ...
      debugPrint('[WebRTC] Cleared missed call: $missedCallId');
      return true;
    } catch (e) {
      debugPrint('[WebRTC] Error clearing missed call: $e');
      return false;
    }
  }
      ...
      if (_isScreenSharing) {
        ...
        if (_peerConnection != null && _localStream != null) {
          ...
          for (final track in videoTracks) {
            if (track.label?.contains('camera') == true || !track.enabled) {
              if ((track.id?.isNotEmpty ?? false)) {
                track.enabled = true;
              }
            }
          }
        }
        debugPrint('[WebRTC] Screen sharing disabled');
      } else {
        // Start screen sharing
        _isScreenSharing = true;
        await startScreenCapture();
        debugPrint('[WebRTC] Screen sharing enabled');
      }
    } catch (e) {
      debugPrint('[WebRTC] Error toggling screen share: $e');
      _isScreenSharing = false;
      _errorController.sink.add('Screen sharing error: $e');
    }
  }
  ...
  Future<void> startScreenCapture() async {
    try {
      ...
      if (videoTracks.isEmpty) {
        throw Exception('No video track in screen stream');
      }

      final screenVideoTrack = videoTracks.first;
      final localVideoTracks = _localStream!.getVideoTracks();

      if (localVideoTracks.isNotEmpty) {
        final localVideoTrack = localVideoTracks.first;
        if (_peerConnection != null) {
          final sender = await _peerConnection!.getSenders();
          for (var s in sender) {
            if (s.track?.kind == 'video') {
              await s.replaceTrack(screenVideoTrack);
              break;
            }
          }
        }
        await _localStream!.removeTrack(localVideoTrack);
        await _localStream!.addTrack(screenVideoTrack);
      }

      debugPrint('[WebRTC] Screen capture started');
    } catch (e) {
      debugPrint('[WebRTC] Error starting screen capture: $e');
      rethrow;
    }
  }
  ...
  Future<void> stopScreenCapture() async {
    try {
      ...
      final screenVideoTracks = _localStream!.getVideoTracks().where((track) {
        return track.label?.contains('screen') == true ||
            track.label?.contains('Display') == true;
      }).toList();

      if (screenVideoTracks.isNotEmpty) {
        for (var track in screenVideoTracks) {
          await _localStream!.removeTrack(track);
          await track.stop();
        }
      }

      debugPrint('[WebRTC] Screen capture stopped');
    } catch (e) {
      debugPrint('[WebRTC] Error stopping screen capture: $e');
      rethrow;
    }
  }
