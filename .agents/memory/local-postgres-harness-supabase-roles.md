...
**How to apply:** Create `authenticated` in the schema stub and call `psql -U "$(id -un)"`; run server setup, migration, and assertions in the same shell invocation so the temporary server remains available.