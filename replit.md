...
*   **Financial Management:** Includes advanced systems for transportation costs, down-payments, operational cost submission with two-tier approvals, digital signatures, PDF certificate generation, aging reports, budget vs. actual comparisons, duplicate payment detection, cash flow forecasting, period close management, reconciliation, and a Reclaim Financial Gap System. The Cost Submission page (`/cost-submission` desktop and mobile) opens with a bilingual EN/AR ExpenseTypeSelector that lets users pick between operational/project costs (existing two-tier flow) and personal reimbursement (out-of-pocket receipts going through Manager → Finance → Paid). Personal mode renders the shared `PersonalExpenseFlow` component (also used standalone at `/my-expenses`); both modes write to existing tables (`operational_cost_submissions` and `expense_claims` / `expense_claim_lines` respectively) with no schema changes, and switching away from a dirty personal claim triggers a discard-confirm dialog.
...
*   **Audit & Security Sidebar Group:** A dedicated sidebar section surfaces previously-hidden audit pages — Hierarchy Changes (`/hierarchy-audit`), System Audit Logs (`/audit-logs`, super-admin only), Audit & Compliance (`/audit-compliance`), and Login Analytics (`/login-analytics`). Each page enforces its own role guard via `useAuthorization().hasAnyRole`, so menu visibility and route guards stay in sync. The Hierarchy Audit Log page provides bilingual EN/AR labels, six KPI tiles (total / week / reporting line / department / people affected / active editors), a 14-day activity bar chart, click-to-expand row details (full UUIDs, exact ISO timestamp, age in days, untruncated reason), keyboard-accessible disclosure (`role="button"`, `aria-expanded`, Enter/Space handlers), cross-link banner to sibling audit pages, and a 12-column CSV export with all UUIDs.
...
*   **Project Field Tasks:** A lightweight tracker for tasks within projects, managing priority, status, assignments, due dates, and location details. Supports MS Project-style typed dependencies (FS / SS / FF / SF) with positive/negative `lag_days` via `project_field_task_dependencies`, with cycle-detection trigger, same-project guard, and RLS that requires SELECT visibility on both endpoint tasks. Writes route through the SECURITY DEFINER RPCs `upsert_project_field_task_dep` / `delete_project_field_task_dep` (authorised via a no-op self-update probe `can_write_project_field_task` on the successor task — only users who can already UPDATE the successor may add/remove its deps); direct table writes are blocked by the `pftd_no_direct_writes` RLS policy. The legacy `project_field_tasks.dependencies` uuid[] column is preserved for backward-compat reads; a one-shot migration block seeds the typed table with FS/lag-0 rows. UI reads (prereq lists, blocking lists, "Depends on N"/"Blocking N" badges, cycle-prevention) prefer the typed table when any rows exist for the project, falling back to the legacy uuid[] only when empty. The TaskForm exposes a per-edge type+lag editor; TaskDetailDialog renders a type/lag badge on each prerequisite row; the **Gantt view** draws fixed-pixel bars (24 px/day) with an SVG connector overlay — one orthogonal-elbow polyline per typed dep, color-coded per type (FS=blue, SS=green, FF=amber, SF=red), with a `+Nd`/`-Nd` lag label and arrow marker; source anchor follows dep type (FS/FF=predecessor end, SS/SF=predecessor start) and is shifted by `lag_days * DAY_PX`.
*   **Project Flow Stage Admin (`/admin/project-flow-stages`):** Admin/super-admin page that overrides the hard-coded `PROJECT_FLOWS` lifecycle stages per project type via `project_flow_stage_overrides` (bilingual EN/AR label + description + key_outputs, typical_duration_days, is_disabled). `useProjectFlow` resolves the canonical type via `getProjectFlow().type` so legacy aliases (survey/monitoring/training) pick up overrides keyed against their canonical id; the override fetch only swallows table-missing errors (PGRST205 / 42P01) and surfaces real RLS/network errors.
