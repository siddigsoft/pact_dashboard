...
*   **Mobile Specific Features:** Includes a bilingual welcome screen, Admin-Managed Support Contacts System, Mobile Admin Dashboard (Flutter-based with Supabase Realtime), Permission Handler Service, Help & Support System, Unified Communications Screen (WebRTC audio/video, enhanced call features), Shorebird OTA Updates, Mobile Digital Signatures, Mobile Field Team Map (admin-only real-time location monitoring), Bilingual Notification Service, Mobile Support Ticket System, Mobile Operational Cost Submission System, Mobile Advance Requests Report, and Mobile Site Claiming with GPS Proximity.
*   **Mobile User Manual:** Dedicated bilingual (English/Arabic) mobile user manual at `/mobile-documentation` with 15 comprehensive sections covering installation, authentication, site visits, offline mode, maps, wallet, cost submissions, communication, settings, help, updates, troubleshooting, and gestures. Includes PDF and DOCX export in both languages, quick reference cards, and language toggle. Accessible to all roles via sidebar Help & Support section and mobile More menu.
