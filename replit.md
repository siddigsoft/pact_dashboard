...
*   **MyTasksV2 Inbox detail panel (`/my-tasks`):** Inline reading pane mirrors the full `/tasks/:id` page — rich-HTML description, Hours & Timesheet summary card, Output Elements with progress bars, the shared `TaskInsightsTabs` component (Approvals / Dependencies / Activity), recent activity list, and an "Open full task" deep link in the toolbar. Hydrates `co_assignees`, hours, and project links via a one-shot `personal_tasks` fetch when a row is selected.
