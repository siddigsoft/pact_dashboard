...
*   **Sidebar Favorites System:** User-customizable sidebar with favorite pages feature. Users can click the star icon next to any menu item to add it to their favorites. Features include: dedicated "Favorites" section at the top of the sidebar with amber/gold styling, drag & drop reordering of favorite pages using @dnd-kit library, star toggle buttons that appear on hover for each menu item, preferences automatically saved to database via SettingsContext, and favorites persist across sessions. The system uses the existing MenuPreferences type with favoritePages array stored in user settings.
