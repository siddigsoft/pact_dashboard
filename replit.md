...
*   **CRM Module (Full):** Five-page CRM module visible to SuperAdmin, Admin, FOM, ProjectManager, CountryDirector. Tables: `crm_partners` (name, type, sector, country, contact info, focal point, status, notes), `crm_engagements` (meeting/call/email/visit/report/proposal log per partner), `crm_contacts` (individual contacts per partner with primary-flag), `crm_opportunities` (pipeline with stages: prospect/proposal/negotiation/won/lost, value_usd, expected_close_date). Pages: `/crm` (CRM Overview dashboard with KPI strip, recent engagements, partner type distribution, open pipeline, quick actions), `/crm/partners` (Partners & Donors grid with slide-over detail panel showing Info/Engagements/Projects tabs), `/crm/contacts` (searchable contacts directory with card layout, partner filter), `/crm/engagements` (all engagements chronologically grouped by month, type and partner filters), `/crm/opportunities` (pipeline with board/list toggle, stage advance button, summary strip). All 5 pages have sidebar entries under "CRM" group.
