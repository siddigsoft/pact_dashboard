...
*   **First-Claim Dispatch System:** An Uber/Lyft-style site claiming system where dispatched sites can be claimed by any enumerator within matching state and locality on a first-come, first-served basis. Features atomic claiming via PostgreSQL `claim_site_visit` RPC function with `SELECT FOR UPDATE SKIP LOCKED` to prevent race conditions, one-click ClaimSiteButton component with optimistic UI, and Supabase Realtime subscription (`useSiteClaimRealtime` hook) for live site availability updates across all users. Status flow: "Dispatched" (available) → "Assigned" (claimed). Error handling includes specific messages for ALREADY_CLAIMED and CLAIM_IN_PROGRESS scenarios. Four dispatch modes available:
    - **Open Dispatch:** "Dispatch for Claim" button makes sites available to any data collector in matching areas without pre-selecting enumerators
    - **State Dispatch:** Dispatch to all collectors in a specific state
    - **Locality Dispatch:** Dispatch to all collectors in a specific locality
    - **Individual Dispatch:** Direct assignment to a specific collector (status becomes "Assigned" immediately)
*   **Classification-Based Fee Calculation:** Enumerator/data collector fees are calculated at claim/accept time, not dispatch time. The workflow:
    ...
    - **At Claim/Accept Time:** The `claim_site_visit` RPC function atomically looks up the collector's classification level (A=Experienced, B=Intermediate, C=Entry-level) and role scope from `user_classifications` table, then retrieves the appropriate fee from `classification_fee_structures` table. Fee structure: Level A=600 SDG, Level B=350 SDG, Level C=200 SDG. If no classification exists, defaults to 50 SDG.
    - **Simplified Fee Formula:** Fees are stored directly in SDG (not cents). Formula: `base_fee × multiplier = final_fee`. With multiplier=1.0 (default), what you set is exactly what enumerators receive. Example: Set 600 SDG → enumerator gets 600 SDG.
    - **Atomic Fee Persistence:** The RPC function saves enumerator_fee, transport_fee, total cost, and claim_fee_calculation metadata in a single atomic transaction, preventing race conditions and RLS policy issues.
    - **Unified Acceptance UX:** AcceptSiteButton provides consistent UX for both dispatched sites (first-claim) and smart-assigned sites. Shows confirmation dialog with site details and fee breakdown (Transport Budget + Collector Fee = Total Payout) before acceptance. Guards prevent duplicate accept/claim attempts on already-processed sites.
    - **After Claim/Accept:** The enumerator_fee, transport_fee, and total cost are saved to `mmp_site_entries` atomically during the claim/accept process, with full audit trail in `additional_data.claim_fee_calculation`.
    ...
    - **Admin Fee Management:** Classification Fee Management page at `/classification-fees` allows administrators to view and edit fees for all classification levels. Features inline editing, simple/advanced mode toggle, and real-time updates.
    - Key files: `use-claim-fee-calculation.ts` (hook), `ClaimSiteButton.tsx` (dispatched claim UI), `AcceptSiteButton.tsx` (unified accept UI), `DispatchSitesDialog.tsx` (dispatch), `SiteDetailDialog.tsx` (display), `UserContext.tsx` (classification loading), `ClassificationFeeManagement.tsx` (admin page), `20251128_fix_claim_enumerator_fee.sql` (RPC migration).
