...
*   **Accounting Phase 4 — Advanced Controls (Complete):** All 8 Phase 4 items delivered. Period Close (`/accounting/period-close`) — Open→Soft→Hard→Locked workflow with live health checks. Tax Management (`/accounting/tax`) — VAT/WHT/customs registry with GL mapping, bilingual names, rate calculator. Multi-Currency (`/accounting/multi-currency`) — FX rate history, latest-rate cards, live converter. Budget Encumbrance (`/accounting/budget-encumbrance`) — commitment accounting from PRs/POs with Budget vs Actual vs Encumbered comparison table; liquidate/cancel actions. Donor Fund Reports (`/accounting/donor-reports`) — fund activity split by restriction type (unrestricted/donor-restricted/board-designated/quasi-endowment), bar chart, and inter-fund elimination analysis for consolidated reporting. Segregation of Duties (`/accounting/sod`) — detects self-approved journal entries (created_by = posted_by), top-offender summary, date-range filter, SOD enforcement flag toggle (log-only vs blocking), CSV export. SQL migration at `supabase/migrations/20260520_acct_phase4_advanced.sql`. All pages gracefully handle missing tables with migration-required banners. Phase Roadmap in Accounting Settings now shows Phase 4 as complete.
