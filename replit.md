...
*   **Project Flow Engine UI (Task #4 DONE):** Full lifecycle UI for all 10 project types. Components: `FlowStrip` (horizontal stepper — completed nodes are clickable Popovers showing who advanced, when, and notes), `FlowTab` (stage cards with key outputs checklist + "Go to [Module]" buttons + advance dialog + stage editing/reordering dialog), `FlowStageBanner` (compact context strip — component exists but is NOT rendered in Activities/Team/Costs/Budget tabs per spec). ProjectDetail: FlowStrip above tabs, 6th "Flow" tab, existing tabs untouched. ProjectList: stage indicator + thin progress bar on cards, type filter updated to all 10 types. ProjectForm: collapsible type-preview panel listing all flow stages. Stage advance sends in-app notifications to all team members via Supabase. Stage editing allows per-project reordering (up/down arrows) and skip-toggling, stored as `custom_flow_stages` JSONB on projects table. Cross-page integration: `useLinkedProjects(entityId, type)` hook + `LinkedProjectsBadge` component added to `MMPDetailView.tsx` and `SiteVisitDetail.tsx`, showing dismissible badge strips for projects referencing the current MMP or site visit. DB: `related_mmps text[]` and `related_site_visits text[]` columns added to `projects` table via migration `20260401_project_related_links.sql`. All flow components in `src/components/project/flow/`, hook at `src/hooks/useProjectFlow.ts`, config at `src/config/projectFlows.ts`.
...
*   **Project Flow Enhancements (Gap Fixes):** Archive/Unarchive projects (DB: `archived` boolean on projects table, migration `20260401_project_archive_stalled.sql`; web: Archive/Unarchive button in ProjectDetail for admin/PM, "Show Archived" toggle in ProjectList with dashed badge on archived cards). PDF Export from ProjectDetail using jsPDF — exports header, project details, flow stages, and activities. Enhanced ProjectDetail header with Health Score widget (activity + stage progress composite, amber if stalled), Stalled banner (amber alert if no stage advance in 14+ days for active projects), Archived banner (muted notice with read-only message). Email notification on stage advance: `sendStageNotifications` now also invokes the `dispatch-notification` edge function with `send_email: true` alongside fast-path in-app inserts. Stalled project daily in-app alerts: `useProjectStalledAlert` hook runs once per session for admin/FOM, creates `project_stalled` event_type notifications for all active non-archived projects stalled 14+ days, de-duplicated by day. Mobile (Flutter): `ProjectDetailScreen` restructured with `DefaultTabController` (3 tabs — Flow, Comments, Documents). Comments tab queries `project_comments` with add/refresh UX. Documents tab queries `project_documents` with file-type icons and size display.

*   **Project Field Tasks System:** Lightweight field task tracker within each project — for non-MMP field operations (site visits, assessments, interviews, trainings, etc.). DB: `project_field_tasks` table with `project_id`, `title`, `description`, `priority` (low/medium/high/critical), `status` (todo/inprogress/done/cancelled), `assigned_to`, `due_date`, `state_name`, `locality_name`, `stage_id` (optional link to a flow stage), `notes`, `created_by`. Frontend: `useProjectTasks` hook + `ProjectFieldTasksPanel` component as a new "Tasks" 7th tab on ProjectDetail. Features: 4-status summary strip (clickable filters), priority sorting, overdue highlighting, assignee search + in-app notification on assignment, quick status dropdown on each card, optional flow stage link, field notes, location (state/locality), full create/edit/delete dialogs. Permission: same as archive (admin/FOM/project manager). Read-only on archived projects.
