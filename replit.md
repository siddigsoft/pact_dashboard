...
*   **Grouped Multi-Item Cost Submissions (Feb 2026):** Multi-item cost submission requests are now grouped under a shared `request_group_id` (UUID) and `request_title` in the `operational_cost_submissions` table. The CostSubmission page displays grouped items as expandable cards showing total amount, item count, and categories. Approval, rejection, recall, and delete actions apply to all items in a group atomically, with tier-level status guards to prevent inconsistent state updates. Single-item submissions remain displayed individually.
*   **Notification System Consolidation (Feb 2026):** Removed duplicate `notificationService.ts` (case-insensitive duplicate). Normalized priority values from 'low' to 'normal' across all notification services (NotificationTriggerService, recallUtils, push notifications, actionable notifications). Expanded NotificationCategory to include 'recall', 'wallet', 'retainer', 'account'. Expanded relatedEntityType to include 'wallet', 'downPayment', 'costSubmission', 'retainer', 'account', 'recovery'. Added comprehensive bilingual notification triggers for: down payment requests/approvals/rejections, cost submission lifecycle (created/approved/rejected/revision), wallet transactions (credit/debit/withdrawal lifecycle/low balance), retainer management (scheduled/processed/pending/eligibility), account/role changes (role updates, account approval/deactivation/reactivation, hub assignments), transportation advance lifecycle, budget utilization alerts, and payment request/completion notifications. Cleaned up excessive debug logging in NotificationContext. Admin notification queries expanded to include financial and account entity types.
