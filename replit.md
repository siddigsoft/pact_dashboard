...
*   **CRM Module — Partners & Donors:** New `crm_partners` table (name, type: donor/partner/contractor/un_agency/ngo/government, sector, country, contact info, focal point, status: active/inactive/prospect, notes) and `crm_engagements` table (meeting/call/email/visit/report/proposal log per partner). Page at `/crm/partners` with stats cards, searchable/filterable partner grid, slide-over detail panel with Info/Engagements/Projects tabs. Add/Edit partner dialog and Log Engagement dialog. `partner_id` FK added to `projects` table. Sidebar entry under "CRM" group visible to SuperAdmin, Admin, FOM, ProjectManager, CountryDirector.
