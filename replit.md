...
*   **Financial Management:** Provides advanced transportation cost and down-payment systems with two-tier approval workflows and audit trails. **Down-Payment Approval page** has multiple analytical views (Approval default, By State, By Project, By MMP, All Requests Detailed) matching the Transportation Advance Report pattern. An operational cost submission system supports nine expense categories with two-tier approval, digital signatures, PDF certificate generation, aging reports, budget vs. actual comparison, duplicate payment detection, cash flow forecasting, financial period close, and a reconciliation dashboard. Automated deduction of transportation advances from site visit fees is also supported. Fund Receipt Confirmation system allows enumerators to confirm receiving funds from approved withdrawals with bilingual (Arabic/English) UI, optional notes, and audit trail visible to supervisors/admins. Mobile-compatible service at `src/services/fund-receipt-confirmation.service.ts`. **Reclaim Financial Gap System:** Comprehensive tracking of advances affected by site reclaims — auto-cancel pending advances, flag disbursed advances with `manual_reconciliation_required`, Reclaim Impact Report tab with bar chart analytics, bulk resolve, write-off workflow (7 reasons, notes, two-step confirm), and role-based notifications to financial admins/supervisors. Advance write-offs cancel the record and log reason/by/at in metadata. Write-off certificate PDF generated via `src/utils/writeOffCertificatePdf.ts` (full-page PDF, PACT header, watermark, advance/reclaim/write-off sections, amount summary, legal notice) — downloadable per-row from Reclaim Impact table for written-off advances. `NotificationDigestService` extended with `sendReclaimReconciliationDigest()` for targeted HTML email to financial admins listing all pending reconciliations. Scheduled digest edge function at `supabase/functions/reclaim-digest/index.ts` runs weekly (Monday 08:00 UTC) via cron schedule in config.toml — sends bilingual HTML email to all financial admins/admins with pending count, total exposure, site/reason/amount table, and CTA deep-link. Re-dispatch is hard-blocked (window.confirm) when unresolved disbursed advances exist. Site reclaim events appear in `SiteVisitAuditTrail` timeline with RotateCcw icon. Mission Control Dashboard shows a live "Pending Reconciliation" alert card linking to the Reclaim Impact tab. Deep-link support via `?tab=reclaimImpact` URL param. Sidebar shows orange reconciliation badge on the Transportation Advance Report link when pending count > 0 (financial/admin roles only).
