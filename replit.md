...
*   **Operational Cost Submission System:** Dedicated expense tracking for FOM/Coordinators covering 9 expense categories (permits, incentives, communications, training, supplies, equipment, transportation, food/refreshments, other). Features two-tier approval workflow (Supervisor/FOM → Admin) with database-enforced tier separation via RLS policies.
*   **Country Director Role:** Senior leadership oversight role with read-only access across all resources. Provides comprehensive visibility into operations, finances, and team activities without approval authority. Can submit operational costs (approved by Admin → Super Admin).
*   **Data Team Role:** Analytics and reporting focused role with read access to projects, MMPs, site visits, finances, users, and audit logs. Can create and export reports for analysis purposes.
