...
*   **WhatsApp Channel (WasenderAPI):** `supabase/functions/send-whatsapp/index.ts` — accepts `user_ids` or `phone_numbers`, fetches phone from `profiles.phone`, normalizes Sudan numbers (+249), builds bilingual EN+AR messages from 40+ event templates, calls `POST https://api.wasenderapi.com/api/send-message` with `Authorization: Bearer WASENDER_API_KEY`. Set `WASENDER_API_KEY` in Supabase dashboard → Edge Functions → Secrets.
