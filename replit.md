...
*   **Mobile Offline Infrastructure:** Features IndexedDB for local storage, a Sync Manager for robust offline-to-online data synchronization with configurable conflict resolution, and a Service Worker for caching. Offline capabilities include site visit workflows (start/complete), GPS/photo capture, cost submissions, and cached MMP lists. The Android APK is Flutter-based with native plugins for enhanced push notifications and continuous GPS tracking. Complete offline support for `_startVisit` and `_completeVisit` operations with `OfflineDataService` queuing (`queueStartVisit`, `queueCompleteVisit`), local state updates, and automatic sync when connectivity is restored. Photos are converted to base64 and stored in the sync queue, then uploaded to Supabase Storage during `syncPendingCompleteVisits`. **Known limitation:** Offline completions update site status and upload photos but do not create associated `reports` table records or process signature/cost metadata until full online completion pipeline parity is implemented.
...
*   **Comprehensive Coverage Analytics Dashboard:** Enhanced Analytics Reports page (`src/components/reports/AnalyticsReports.tsx`) with professional gradient header, 4 main tabs (Overview, Productivity, Efficiency, Coverage Analytics), and MMP-style sub-tab navigation. Coverage Analytics features 4 sub-tabs (By Hub, By State, By Locality, By Activity) with date range filtering, activity view toggle (Per Hub/Per State), and export functionality (Excel) for all coverage views. All interactive elements include data-testid attributes for E2E testing, and all array operations use safe fallbacks to prevent runtime errors.
*   **Admin-Managed Support Contacts System:** Centralized support contact management shared between web and mobile platforms (`src/pages/SupportContacts.tsx`). Supports bilingual contact information (English/Arabic) with name, role, email, phone, and WhatsApp fields. Role-based access restricts management to admin and super_admin users. Mobile integration via `support_contacts` Supabase table with real-time sync.
*   **Mobile Admin Dashboard:** Flutter-based admin screens (`lib/screens/admin_dashboard_screen.dart`, `lib/screens/super_admin_screen.dart`) with role-based navigation access. Admin Dashboard provides support contacts CRUD, user approval management, and system statistics. Super Admin screen offers complete user management with role changes and bulk operations. Custom drawer menu (`lib/widgets/custom_drawer_menu.dart`) conditionally displays admin options based on user role.
*   **Comprehensive Help & Support System:** Mobile bilingual Help & Support screen (`lib/screens/help_support_screen.dart`) with tabbed interface covering Getting Started, Troubleshooting, Field Operations, and Contact Support. Features expandable FAQ sections with English/Arabic language toggle and direct contact integration with support team.
*   **Unified Communications Screen:** Mobile Communications screen (`lib/screens/communications_screen.dart`) consolidating calls and messages into a single interface for streamlined team communication. Features real-time online presence indicators via Supabase Realtime (`lib/services/presence_service.dart`), WebRTC audio calls (`lib/services/webrtc_service.dart`), and direct messaging via ChatService. The separate Chats screen has been disabled in favor of this unified interface.
*   **Shorebird OTA Updates:** Integration with Shorebird code push for over-the-air updates (`lib/services/update_service.dart`). Settings screen and drawer menu display current version, build number, and patch number (e.g., "Version 1.0.5 (Build 8, Patch 9)") using `shorebird_code_push` package.
*   **Mobile Digital Signatures:** Flutter Digital Signatures screen (`lib/screens/digital_signatures_screen.dart`) with canvas-based signature drawing, PNG export via RepaintBoundary, signature management (set default, delete), and signing history with document tracking. First signature automatically set as default.
*   **Mobile Field Team Map:** Admin-only Field Team Map screen (`lib/screens/field_team_map_screen.dart`) for monitoring field team locations in real-time. Features flutter_map integration, color-coded online status (green=online <15min, orange=recent <60min, red=offline), 30-second auto-refresh, filtering by status, and direct call/message actions via WebRTC. Includes in-screen role-based access guards with Access Denied UI for unauthorized users.
*   **Bilingual Notification Service:** Mobile notification service (`lib/services/bilingual_notification_service.dart`) with comprehensive Arabic/English translations for all notification types including messages, payments, sync, calls, signatures, updates, and site assignments. Uses Hive for locale persistence and flutter_local_notifications for delivery.
