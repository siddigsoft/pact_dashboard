...
*   **Task Lifecycle Notifications (`useTaskNotifications` hook):** Fires in-app + email notifications for all task events: created, started, acknowledged, completed, delayed, rejected, cancelled, overdue, and reminder (1-day/3-day before deadline). Used in MyTasksV2 (toggle done, save) and Team Task Monitor (create/update). WhatsApp notification channel stubbed — activated once API key configured. Phone numbers stored in `profiles.phone_number`.
