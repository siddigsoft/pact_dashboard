...
class _CostHistoryTabState extends State<CostHistoryTab> {
  ...
  // ponytail: was building a CSV string and discarding it (told the user
  // "export prepared" while sharing nothing). Reuses the service's
  // exportToCsv (same logic already used for full-list exports) and
  // actually writes + shares the file via the share_plus/path_provider
  // dependencies already used elsewhere in this app for exports.
  Future<void> _handleExportCsv(OperationalCostSubmission submission) async {
    final csvContent = _service.exportToCsv([
      submission,
    ], isArabic: widget.isArabic);
    final ref = 'PACT-OC-${submission.id.substring(0, 8).toUpperCase()}';
    final dir = await getTemporaryDirectory();
    final file = File('${dir.path}/$ref.csv');
    await file.writeAsString(csvContent);
    await Share.shareXFiles([XFile(file.path)], subject: ref);
  }
