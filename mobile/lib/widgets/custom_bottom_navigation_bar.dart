...
class CustomBottomNavigationBar extends StatelessWidget {
  ...
  int get _itemCount {
    if (isSupervisor) return 5;
    if (isEmployee) return 5;
    return isCoordinator ? 7 : 6;
  }
  ...
  List<Widget> _buildNavItems(BuildContext context, double itemWidth) {
    ...
    if (isSupervisor) {
      // Team Sites, Cost Submission, and Down-Payment dropped: all still
      // reachable from More (Down-Payment is also a tab inside Approvals).
      // Team Sites has no web equivalent for supervisors — not worth a
      // permanent bottom-nav slot. Communications opens Calls & Communications
      // (not chat-only).
      return [
        Expanded(
          child: _buildNavItem(
