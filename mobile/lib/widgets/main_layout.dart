...
import '../screens/mmp_management_screen.dart';
...
class _MainLayoutState extends State<MainLayout> {
  ...
  StreamSubscription<int>? _tasksBadgeSub;
  ...
  Future<void> _checkUserRole() async {
    try {
      ...
      if (response != null && mounted) {
        ...
        setState(() {
          _userRole = role;
          _isEmployee = UserRoleHelper.isEmployee(role);
          _isCoordinator = UserRoleHelper.isCoordinator(role);
          _isSupervisor = UserRoleHelper.isSupervisor(role);
  ...
  void _onItemTapped(int index) {
    ...
    Widget? screen;
    if (_isEmployee) {
      // Employee: Home (0), Tasks (1), Communications (2), Wallet (3), More (4)
      ...
      // Supervisor nav: Home (0), Approvals (1), MMP (2), Communications (3), More (4)
      // Team Sites, Cost Submission, and Down-Payment dropped from the bar —
      // all still reachable from More (Down-Payment is also a tab inside
      // Approvals; Team Sites has no web equivalent for this role).
      if (index < 0 || index > 4) return;
      switch (index) {
        case 0:
          screen = const DashboardScreen();
        ...
        case 1:
          ...
          break;
        case 2:
          screen = const MmpManagementScreen();
          break;
        case 3:
          screen = const CommunicationsScreen();
          break;
      ...
      // Home (0), Verify (1), Sites (2), Tasks (3), Communications (4), Wallet (5), More (6, coordinator)
      final isCoordinator = _isCoordinator;
      final maxIndex = isCoordinator ? 6 : 5;
  ...
  Future<void> _initTasksBadge() async {
    if (_isSupervisor) return;
    try {
      await PersonalTaskNotificationService.instance.initialize();
      final count =
          await PersonalTaskNotificationService.instance.countNeedsAcknowledgement();
      if (mounted) setState(() => _tasksBadgeCount = count);
      _tasksBadgeSub =
          PersonalTaskNotificationService.instance.pendingAckBadgeStream.listen(
        (count) {
          if (mounted) setState(() => _tasksBadgeCount = count);
        },
      );
    } catch (e) {
      debugPrint('[MainLayout] tasks badge init: $e');
    }
  }

  @override
  void dispose() {
    _tasksBadgeSub?.cancel();
    ...
    return Scaffold(
      ...
      bottomNavigationBar: _isLoadingRole
          ...
          : CustomBottomNavigationBar(
              ...
              tasksBadgeCount: _tasksBadgeCount,
            ),
    );
  }
}
