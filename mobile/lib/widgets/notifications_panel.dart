...
import '../screens/personal_task_detail_screen.dart';
  ...
  void _navigateFromNotification(UserNotification n) {
    ...
    switch (decision.kind) {
      ...
      case NotificationRouteKind.main:
        final taskId = decision.mainArgs?['taskId']?.toString();
        if (decision.mainArgs?['tab'] == 'tasks' &&
            taskId != null &&
            taskId.isNotEmpty) {
          navigator.push(
            MaterialPageRoute<void>(
              builder: (_) => PersonalTaskDetailScreen(taskId: taskId),
            ),
          );
        } else {
