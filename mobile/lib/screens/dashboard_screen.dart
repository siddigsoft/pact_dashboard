...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  // MMP Filter state
  String? _selectedMmpId;
  List<Map<String, dynamic>> _availableMmps = [];
  ...
  void _applyProfileData(Map<String, dynamic> profileResponse) {
    ...
    _isEmployee = UserRoleHelper.isEmployee(_userRole);
  ...
  Future<void> _cacheDashboardData(String userId) async {
    try {
      ...
      final data = {
        ...
        'dataCollectorVisits': _dataCollectorVisits
            .map((v) => v.toJson())
            .toList(),
      };
      await offlineDb.cacheItem(
  ...
  Future<void> _loadCoordinatorDashboard() async {
    try {
      ...
      if (filteredResponse.isNotEmpty) {
        debugPrint('DEBUG: Raw API response sample:');
        debugPrint('${filteredResponse.first}');
        debugPrint(
          'DEBUG: Has mmp_file_id? ${filteredResponse.first.containsKey('mmp_file_id')}',
        );
        debugPrint(
          'DEBUG: Has mmp_files? ${filteredResponse.first.containsKey('mmp_files')}',
        );
        debugPrint(
          'DEBUG: Available keys: ${filteredResponse.first.keys.join(', ')}',
        );
      }

      _coordinatorVisits = siteVisits;

      // Build MMP filter options
      _buildMmpFilterOptions();
  ...
  /// Build MMP filter options from coordinator visits
  void _buildMmpFilterOptions() {
    if (!_isCoordinator || _coordinatorVisits.isEmpty) {
      _availableMmps = [];
      return;
    }

    _availableMmps = buildMmpFilterOptions(_coordinatorVisits);

    if (_availableMmps.isNotEmpty) {
      debugPrint(
        'DEBUG: MMP filter built with ${_availableMmps.length} unique MMPs',
      );
      for (final mmp in _availableMmps) {
        debugPrint('  - ${mmp['name']}: ${mmp['count']} sites');
      }
    }
  }
                  ...
                  : RefreshIndicator(
                      ...
                      child: SingleChildScrollView(
                        ...
                        child: Column(
                          ...
                          children: [
                            if (_isEmployee) _buildEmployeeWelcome(context),
  ...
  Widget _buildCoordinatorCards() {
    // Apply MMP filter to coordinator visits
    final filteredVisits = _selectedMmpId == null
        ? _coordinatorVisits
        : _coordinatorVisits
              .where((v) => v.mmpFileId == _selectedMmpId)
              .toList();

    // Calculate metrics for filtered data
    int total = filteredVisits.length;

    int completed = filteredVisits.where((v) {
      final status = v.status.toLowerCase();
      return status == 'completed' || status == 'complete';
    }).length;

    int active = filteredVisits.where((v) {
      final status = v.status.toLowerCase();
      return status == 'assigned' ||
          status == 'ongoing' ||
          status == 'in progress' ||
    ...
    int pending = filteredVisits.where((v) {
      final status = v.status.toLowerCase();
      return status == 'pending' ||
          status == 'dispatched' ||
          status == 'permitverified' ||
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          // Tab Content - maximized sites area
          Padding(
  ...
  Widget _buildOverviewTab() {
    ...
    return Column(
      ...
      children: [
        ...
        ...filteredVisits.take(10).map((visit) => _buildVisitListItem(visit)),
      ],
    );
  }
  ...
  Widget _buildEmployeeWelcome(BuildContext context) {
    return Container(
      width: double.infinity,
      ...
      decoration: BoxDecoration(
        gradient: const LinearGradient(
          colors: [Color(0xFF0F2041), Color(0xFF1D3461)],
        ),
        borderRadius: BorderRadius.circular(16),
      ),
      child: Column(
        ...
        children: [
          ...
          Text(
            'Your tasks, calendar, and payslips — same as the web app.',
            style: GoogleFonts.poppins(
