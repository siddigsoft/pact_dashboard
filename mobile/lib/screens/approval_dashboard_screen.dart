...
/// Mirrors web `getActivePendingTier` in operationalCostApproval.ts.
int? _activePendingCostTier(ops.OperationalCostSubmission submission) {
  if (submission.tier1Status == ops.TierApprovalStatus.pending) return 1;
  if (submission.tier2Status == ops.TierApprovalStatus.pending) return 2;
  return null;
}
            ...
            Expanded(
              child: TabBarView(
                ...
                children: [
                  ...
                  const DownPaymentApprovalBody(),
                  const PendingTaskApprovalsBody(),
                ],
              ),
            ),
          ],
...
class _CostSubmissionCardState extends ConsumerState<_CostSubmissionCard> {
  ...
  Widget build(BuildContext context) {
    ...
    final tier = _activePendingCostTier(submission);
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              'Submitted by: ${submission.displaySubmitterName}',
              style: Theme.of(context).textTheme.bodyMedium,
            ),
            Text(
            ...
            Row(
              ...
              children: [
                ...
                ElevatedButton(
                  onPressed: tier != null
                      ? () => _showApprovalDialog(context, true, tier)
                      : null,
                  child: Text(tier == 2 ? 'T2 Approve' : 'Approve'),
                ),
                const SizedBox(width: 8),
                OutlinedButton(
                  onPressed: tier != null
                      ? () => _showApprovalDialog(context, false, tier)
                      : null,
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              Text('Submitted by: ${submission.displaySubmitterName}'),
  ...
  void _showApprovalDialog(BuildContext context, bool approve, int tier) {
    ...
    showDialog(
      ...
      builder: (dialogContext) => AlertDialog(
        title: Text(
          approve
              ? 'Approve Cost Submission${tier > 1 ? ' (Tier $tier)' : ''}'
              : 'Reject Cost Submission${tier > 1 ? ' (Tier $tier)' : ''}',
        ),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              await _processApproval(approve, controller.text, tier);
            },
  ...
  Future<void> _processApproval(bool approve, String notes, int tier) async {
    try {
