...
class _MonitoringPlanScreenState extends State<MonitoringPlanScreen> {
  ...
  Future<void> _loadPlans({bool refresh = false}) async {
    ...
    try {
      final data = await _supabase
          ...
          .select(
            'id, mmp_code, status, month, year, project_name, total_sites, covered_sites, created_at',
          )
  ...
  List<Map<String, dynamic>> get _filtered => _plans.where((p) {
    final matchSearch =
        _searchQuery.isEmpty ||
        (p['mmp_code'] ?? '').toLowerCase().contains(
          _searchQuery.toLowerCase(),
        ) ||
        (p['project_name'] ?? '').toLowerCase().contains(
          _searchQuery.toLowerCase(),
        );
    final matchStatus =
                              ...
                              children: [
                                ...
                                Row(
                                  children: [
                                    ...
                                    Text(
                                      '${p['covered_sites'] ?? 0}/${p['total_sites'] ?? 0} sites',
                                      style: const TextStyle(
                                        fontSize: 12,
                                        color: Colors.grey,
                                      ),
