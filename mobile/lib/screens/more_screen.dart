...
import 'daily_task_templates_screen.dart';
import 'my_tasks_screen.dart';
import 'pending_task_approvals_screen.dart';
...
class _MoreScreenState extends State<MoreScreen> {
  ...
  /// Supervisor-only site pipeline (web: /supervisor/sites); hidden if also coordinator.
  bool get _isPureSupervisor => _isSupervisor && !_isCoordinator;

  bool get _canSeeCalendar => !UserRoleHelper.isDataCollector(_role);

  bool get _canSeeProgrammeOps =>
      _showAllForAdmin || _isSupervisor || _isCoordinator;

  bool get _canSeeCycleClose => _showAllForAdmin || _isSupervisor;

  bool get _canSeeMonitoringForm =>
      _showAllForAdmin || _isSupervisor || _isCoordinator;

  bool get _canSeeSiteVerification =>
      !_isEmployee &&
      (_isCoordinator || _isSupervisor || _showAllForAdmin);

  bool get _canSeeSignatures =>
      _showAllForAdmin || _isSupervisor || _isCoordinator;

  bool get _canSeeWallet =>
      !_isEmployee &&
      (_isSupervisor || _isCoordinator || _showAllForAdmin || _isFinance);
  ...
  Widget build(BuildContext context) {
    final fieldOps = <_MoreItem>[
      _MoreItem(
        Icons.task_alt,
        'My Tasks',
        () => _navigateToScreen(const MyTasksScreen()),
      ),
      if (UserRoleHelper.canSeeTeamMonitor(_role))
        _MoreItem(
          Icons.groups_outlined,
          'Team Monitor',
          () => _navigateToScreen(const TeamTasksScreen()),
        ),
      if (UserRoleHelper.canSeeTeamMonitor(_role))
        _MoreItem(
          Icons.approval_outlined,
          'Task Approvals',
          () => _navigateToScreen(const PendingTaskApprovalsScreen()),
        ),
      if (_canSeeAdminFeatures())
        _MoreItem(
          Icons.repeat,
          'Daily task templates',
          () => _navigateToScreen(const DailyTaskTemplatesScreen()),
        ),
      if (_canSeeCalendar)
        _MoreItem(
      ...
      if (_isPureSupervisor)
        _MoreItem(
          Icons.map_outlined,
          'Team Sites',
          () => _navigateToScreen(const TeamSitesScreen()),
        ),
      if (_canSeeMonitoringForm)
        _MoreItem(
          Icons.fact_check,
          'Monitoring Form',
      ...
      if (_canSeeSiteVerification)
        _MoreItem(
          Icons.verified_user,
          'Site Verification',
          () => _navigateToScreen(const SiteVerificationScreen()),
        ),
      if (_isSupervisor && !_isEmployee)
        _MoreItem(
          Icons.phone_in_talk,
          'Calls & Communications',
          () => _navigateToScreen(const CommunicationsScreen()),
        ),
      _MoreItem(
        Icons.folder_copy,
        'Documents',
