...
final _outstandingAdvancesProvider =
    FutureProvider.autoDispose<List<OperationalCostSubmission>>((ref) async {
      ...
      try {
        final supabase = ref.watch(supabaseClientProvider);
        // ponytail: funding_type/is_reconciled don't exist on this table
        // (verified live) — fund_receipt_confirmed is the real "has this
        // payout been accounted for" flag that replaced them.
        final response = await supabase
            ...
            Expanded(
              child: TabBarView(
                ...
                children: [
                  ...
                  _HistoryTab(isArabic: isArabic, userRole: widget.userRole),
                ],
              ),
            ),
          ],
...
class _CostRequestFormState extends ConsumerState<_CostRequestForm> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      // Fetch user's hub_id and role from profiles table
      ...
      final submitterRole = profileResponse?['role']?.toString() ?? 'enumerator';
...
class _ReconcileCardState extends State<_ReconcileCard> {
  ...
  Future<void> _reconcile() async {
    ...
    setState(() => _isSubmitting = true);
    try {
      // ponytail: is_reconciled/reconciled_amount_cents/reconciliation_notes
      // don't exist on this table (verified live) — fund_receipt_confirmed
      // is the real flag that replaced "reconciled"; there's no dedicated
      // column for the actual/balance amounts, so they're folded into
      // fund_receipt_notes rather than silently dropped.
      final notes = StringBuffer(
        'Actual: ${_actual.toStringAsFixed(2)}, Balance: ${_balance.toStringAsFixed(2)}',
      );
      if (_notesCtrl.text.trim().isNotEmpty) {
        notes.write('. ${_notesCtrl.text.trim()}');
      }
      await Supabase.instance.client
          ...
          .update({
            ...
            'fund_receipt_notes': notes.toString(),
...
class _HistoryTab extends ConsumerWidget {
  final bool isArabic;
  final String? userRole;
  const _HistoryTab({this.isArabic = false, this.userRole});
