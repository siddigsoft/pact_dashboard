...
import '../widgets/enhanced_offline_toggle.dart';
...
class _MMPScreenState extends State<MMPScreen> {
  ...
  // Start tracking user location when online
  Future<void> _startLocationTracking() async {
    if (_isOffline || _isTrackingLocation) return;

    debugPrint('[_startLocationTracking] Starting location tracking');
    _isTrackingLocation = true;

    // Send initial location
    await _updateUserLocation();

    // Schedule periodic location updates (every 30 seconds)
    _locationTrackingTimer = Timer.periodic(const Duration(seconds: 30), (_) {
      _updateUserLocation();
    });
  }

  // Stop location tracking when going offline or leaving screen
  void _stopLocationTracking() {
    debugPrint('[_stopLocationTracking] Stopping location tracking');
    _locationTrackingTimer?.cancel();
    _isTrackingLocation = false;
  }

  // Update user location to Supabase
  Future<void> _updateUserLocation() async {
    if (_userId == null || _isOffline) return;
    ...
    try {
      ...
      _lastReportedLocation = position;

      // Update user profile with live location
      ...
      debugPrint(
        '[_updateUserLocation] Location updated: ${position.latitude}, ${position.longitude}',
      );
    } catch (e) {
  ...
  void _applyProfileData(Map<String, dynamic> profileResponse) {
    ...
    if (UserRoleHelper.isEmployee(_userRole)) {
  ...
  Future<void> _loadDataCollectorData({
    ...
    try {
      ...
      // Print site inventory for debugging
      _printSiteInventory();
  ...
  Future<void> _claimSite(Map<String, dynamic> site) async {
    ...
    // Check if user is online before allowing claim
    if (_isOffline) {
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              isArabic
                  ? 'يجب أن تكون متصلاً بالإنترنت للمطالبة بموقع'
                  : 'You must be online to claim a site',
            ),
                                ...
                                Expanded(
                                  child: Text(
                                    Localizations.localeOf(
                                            ...
                                            'ar'
                                        ? 'بالمطالبة بهذا الموقع، فإنك توافق على إكمال الزيارة وتقديم التقرير المطلوب.'
  ...
  bool _hasSyncedReport(Map<String, dynamic> site) {
    ...
    // Guard: only final statuses can be treated as sent.
    // This prevents Accepted/Assigned/Dispatched sites from appearing in Sent
    // even if stale report markers exist in additional_data.
    final status = (site['status'] as String? ?? '').trim().toLowerCase();
    final isFinalStatus =
        status == 'completed' ||
        status == 'complete' ||
        status == 'submitted' ||
  ...
  /// Compute available MMP options from current site data (all lists combined)
  List<Map<String, dynamic>> get _mmpFilterOptions {
    ...
    debugPrint(
      '📊 MMP Filter Source Breakdown: Available=${_availableSites.length}, SmartAssigned=${_smartAssignedSites.length}, MySites=${_mySites.length}, Coordinator=${_coordinatorSites.length}, Unsynced=${_unsyncedCompletedVisits.length}',
    );

    final mmpMap = <String, Map<String, dynamic>>{};

    for (final site in allSites) {
      final id = _extractMmpId(site);
      if (id == null) continue;

      // Extract name from mmp_files relationship
      String name = '';
      final raw = site['mmp_files'];
      if (raw is Map<String, dynamic>) {
      ...
      if (name.isEmpty) {
        name = 'MMP ${id.substring(0, 6).toUpperCase()}';
      }
    ...
    debugPrint(
      '🔍 MMP Filter Options (from ${allSites.length} total sites): ${options.map((m) => "${m['name']} (${m['count']})").join(", ")}',
    );
    return options;
  }

  /// Diagnostic method to print detailed site inventory for troubleshooting
  void _printSiteInventory() {
    debugPrint('═══════════════════════════════════════════════════════════');
    debugPrint('📊 SITE INVENTORY DIAGNOSTIC');
    debugPrint('═══════════════════════════════════════════════════════════');

    final allLists = {
      '_availableSites': _availableSites,
      '_smartAssignedSites': _smartAssignedSites,
      '_mySites': _mySites,
      '_coordinatorSites': _coordinatorSites,
      '_unsyncedCompletedVisits': _unsyncedCompletedVisits,
    };

    int totalSites = 0;
    final uniqueMmpIds = <String>{};

    for (final entry in allLists.entries) {
      debugPrint('\n${entry.key}: ${entry.value.length} sites');
      totalSites += entry.value.length;

      for (final site in entry.value) {
        final mmpId = _extractMmpId(site);
        if (mmpId != null) {
          uniqueMmpIds.add(mmpId);
          final raw = site['mmp_files'];
          String mmpName = '';
          if (raw is Map<String, dynamic>) {
            mmpName = raw['name']?.toString() ?? 'Unknown';
          } else if (raw is List && raw.isNotEmpty && raw.first is Map) {
            mmpName = (raw.first as Map)['name']?.toString() ?? 'Unknown';
          }
          debugPrint(
            '  - Site: ${site['id']?.toString().substring(0, 8)}... │ MMP: $mmpName ($mmpId)',
          );
        }
      }
    }
    ...
    debugPrint('\n═══════════════════════════════════════════════════════════');
    debugPrint('📈 SUMMARY:');
    debugPrint('   Total Sites Across All Lists: $totalSites');
    debugPrint('   Unique MMPs: ${uniqueMmpIds.length}');
    debugPrint('   Unique MMP IDs: ${uniqueMmpIds.join(", ")}');
    debugPrint('═══════════════════════════════════════════════════════════\n');
  }
      ...
      children: [
        ...
        Builder(
          builder: (context) {
            ...
            final total = mmpOptions.fold(
              0,
              (sum, m) => sum + (m['count'] as int),
            );
            final filtered = _selectedMmpId == null
                ? total
                : (mmpOptions.where((m) => m['id'] == _selectedMmpId).isNotEmpty
                ...
                children: [
                  Expanded(
                    child: _buildTabButton(
                      ...
                      'قابل للمطالبة',
                      Icons.handshake,
                      _getFilteredSites(_availableSites).length,
                    ),
                  ),
                  Expanded(
                    child: _buildTabButton(
                      ...
                      'مخصص',
                      Icons.assignment,
                      _getFilteredSites(_smartAssignedSites).length,
                    ),
                  ),
                  Expanded(
                    child: _buildTabButton(
  ...
  Widget _buildTabButton(
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        // ponytail: labels stacked beside icon/count (not above/below) to halve
        // tab-row height; same icon+EN+AR+count data, just laid out horizontally.
        child: Row(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(
  ...
  Widget _buildSubTabButton(
    ...
    return InkWell(
      ...
      child: Container(
        ...
        // ponytail: icon/labels/count in one row instead of a 4-line column,
        // same data (icon + EN + AR + count), about half the height.
        child: Row(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(
  ...
  Widget _buildClaimableSites() {
    ...
    return ListView(
      padding: const EdgeInsets.all(12),
      children: [
        // Enhanced First-Come First-Served info card
        Container(
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      'Tap "Claim Site" to claim quickly - other enumerators can claim too!',
                      style: GoogleFonts.poppins(
                        fontSize: 10,
                        color: Colors.white.withValues(alpha: 0.9),
                        height: 1.2,
        ...
        Padding(
          ...
          child: TextField(
            ...
            decoration: InputDecoration(
              hintText:
                  l10n?.searchSites ?? 'Search sites or enter site code...',
  ...
  int _getPendingCount() {
    return _getFilteredSites(_mySites).where((site) {
      final status = (site['status'] as String? ?? '').toLowerCase();
      return status == 'accepted' ||
          status == 'assigned' ||
          status == 'dispatched' ||
  ...
  int _getSentCount() {
    return _getFilteredSites(_mySites)
        .where((site) => _hasSyncedReport(site))
        .length;
  }
}
