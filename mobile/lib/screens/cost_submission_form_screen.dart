...
class CostSubmissionFormScreen extends StatefulWidget {
  ...
  @override
  State<CostSubmissionFormScreen> createState() =>
...
class _CostSubmissionFormScreenState extends State<CostSubmissionFormScreen> {
  ...
  Future<void> _loadExisting() async {
    ...
    try {
      ...
      final match = submissions
          .where((s) => s.id == widget.editSubmissionId)
          .toList();
      if (match.isNotEmpty) {
        final s = match.first;
        setState(() {
                ...
                return Expanded(
                  child: Padding(
                    padding: EdgeInsets.only(
                      right: ft != FundingType.values.last ? 8 : 0,
                    ),
                    child: ChoiceChip(
                      label: SizedBox(
  ...
  Future<void> _notifyApproversOfNewSubmission(
    ...
    try {
      ...
      final submitterName =
          userProfile?['full_name'] as String? ?? 'Team Member';
      ...
      final approverIds = (approvers as List)
  ...
  Future<void> _handleSubmit() async {
    ...
    try {
      ...
      // ponytail: operational_cost_submissions has no funding_type/justification
      // columns (verified live) — folded into description text instead of
      // silently dropping what the user typed, matching cost_submit_tab.dart's
      // existing convention.
      final fullDescription =
          '[${_selectedFundingType.value.toUpperCase()}] $description'
          '${justification.isNotEmpty ? '\n\nJustification: $justification' : ''}';
      final vendor = _vendorController.text.trim().isNotEmpty
      ...
      if (widget.editSubmissionId != null) {
        ...
        if (result != null && mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                widget.isArabic ? 'تم تحديث الطلب' : 'Submission updated',
              ),
