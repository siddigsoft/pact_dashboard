...
class UserNotificationService {
  ...
  /// Dedicated Hive box for the notification list.
  /// IMPORTANT: Do NOT rename to `notifications_cache` — that box name is
  /// already used by `OfflineDb.notificationsCacheBox` as `Box<CachedItem>`,
  /// which causes Hive type conflicts on web.
  static const String _cacheBoxName = 'user_notification_list_cache';
  ...
  /// Reset all per-user state without closing the stream controllers.
  /// Must be called on sign-out — this is a singleton, so without this the
  /// next user's `initialize()` call short-circuits on `_isInitialized` and
  /// re-emits the previous user's (or empty) cached list instead of
  /// fetching their own notifications.
  ...
  void _handleInsert(UserNotification notification) {
    ...
    if (!alreadyExists) {
      ...
      final event = (notification.eventType ?? '').toLowerCase();
      final isTask = event.startsWith('task_') ||
          notification.relatedEntityType == 'task';
      final taskId = notification.relatedEntityId;
      final payload = isTask && taskId != null && taskId.isNotEmpty
          ? 'task:$taskId'
          : null;
