...
class AuthService {
  ...
  Future<void> _handleSignIn(User? user) async {
    ...
    try {
      ...
      // Update profiles.last_active (profile row is created by auth trigger on signup)
      try {
        await supabase
            .from('profiles')
      ...
      // Ensure user has a default role via SECURITY DEFINER RPC (bypasses RLS safely)
      try {
        await supabase.rpc('ensure_user_default_role');
      } catch (e) {
        debugPrint('Note: ensure_user_default_role failed (non-critical): $e');
      }
    } catch (e) {
