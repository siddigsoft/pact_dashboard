...
class NotificationTriggerService {
  ...
  Future<bool> send(NotificationTriggerOptions options) async {
    try {
      ...
      final categoryStr = options.category.toString().split('.').last;
      final insertedId = await NotificationInsertService.insertNotification({
        ...
        'event_type': categoryStr,
        'priority': options.priority.toString().split('.').last,
        'link': options.link,
        'action_url': options.link,
        'related_entity_id': options.relatedEntityId,
        'related_entity_type': options.relatedEntityType
            ?.toString()
            .split('.')
            .last,
        'entity_id': options.relatedEntityId,
        'entity_type': options.relatedEntityType?.toString().split('.').last,
        'metadata': {
          'category': categoryStr,
          if (options.targetRoles != null) 'target_roles': options.targetRoles,
