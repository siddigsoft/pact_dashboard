...
class LocationTrackingService {
  ...
  factory LocationTrackingService([FieldOperationsRepository? repo]) {
    if (repo != null) {
      _instance._repository = repo;
    }
    return _instance;
  }
  ...
  late FieldOperationsRepository _repository = FieldOperationsRepository();
  ...
  Future<List<LocationLog>> getCachedLocationLogs(String visitId) async {
    try {
      ...
      final keys = box.keys.where(
        (key) => key.toString().startsWith('${visitId}_'),
      );
      ...
      if (log.visitId != null) {
        try {
          ...
          if (visitExists == null) {
            ...
            print(
              '⏳ Skipping location log sync - visit ${log.visitId} not yet synced',
            );
            return;
          }
        } catch (e) {
