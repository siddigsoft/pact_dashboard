...
import 'services/personal_task_offline_store.dart';
...
Widget _buildMainShell(Object? args) {
  ...
  if (args is Map) {
    ...
    final isEmployee = UserRoleHelper.isEmployee(UserRoleHelper.cachedRole());
    ...
    } else if (tabValue == 'tasks' || tabValue == 'my-tasks' || tabValue == 'my_tasks') {
      index = isEmployee ? 1 : 3;
      final taskId = args['taskId']?.toString();
      child = MyTasksScreen(initialTaskId: taskId);
    } else if (tabValue == 'communications' || tabValue == 'chat') {
      index = isEmployee ? 2 : 4;
      child = const CommunicationsScreen();
    } else if (tabValue == 'wallet') {
      index = isEmployee ? 3 : 5;
...
void main() async {
  ...
  final boxNames = [
    ...
    PersonalTaskOfflineStore.tasksBoxName,
    PersonalTaskOfflineStore.pendingBoxName,
    PersonalTaskOfflineStore.metaBoxName,
  ];
