...
final pendingWithdrawalRequestsProvider =
    FutureProvider.autoDispose<List<WithdrawalRequest>>((ref) async {
      ...
      try {
        ...
        final isFinance = userRole.contains('finance');
        final isFom = userRole.contains('fom');
        final isSupervisor = userRole.contains('supervisor');

        List<dynamic> response;

        if (isSupervisor && !isAdmin && !isFinance && !isFom) {
          // Tier-1 review: a plain supervisor only sees 'pending' requests
          // from teammates in their own hub — matches the web app's
          // useApprovalsData.ts (it never shows them 'supervisor_approved'
          // requests either; those are finance/admin/FOM's tier-2 queue).
          final hubId = profileResponse?['hub_id'] as String?;
          if (hubId == null) return [];
          final teammates = await supabase
              .from('profiles')
              .select('id')
              .eq('hub_id', hubId)
              .neq('id', userId);
          final teamIds = (teammates as List)
        ...
        } else if (isAdmin || isFinance || isFom) {
          // Tier-1 + tier-2 review, unscoped — admins/finance/FOM see every hub.
          response = await supabase
              .from('withdrawal_requests')
              .select('*, profiles!user_id(full_name)')
