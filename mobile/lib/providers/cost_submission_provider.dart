...
final costSubmissionServiceProvider = Provider<CostSubmissionService>((ref) {
  final supabase = ref.watch(supabaseClientProvider);
  return CostSubmissionService(supabase);
});
...
final pendingCostSubmissionsProvider =
    FutureProvider.autoDispose<List<ops.OperationalCostSubmission>>((
      ...
      try {
        ...
        // 'admin' alone already matches super_admin/superadmin (both contain
        // "admin"); a separate .contains('super') check isn't needed for
        // that and was accidentally also matching "supervisor"/"hubsupervisor".
        ...
        if (isAdmin || isCoordinator || isFom || isSupervisor) {
          ...
          // Supervisors only see their own hub's submissions, matching the
          // web app's useApprovalsData.ts hub scoping. Admins/coordinators/FOM
          // see all hubs.
          if (isSupervisor && !isAdmin) {
            final hubId = profileResponse?['hub_id'] as String?;
            query = query.eq('hub_id', hubId ?? '__none__');
          }
...
//
// ponytail: this was calling costSubmissionServiceProvider (which inserts
// into operational_cost_submissions) with a CreateCostSubmissionRequest
// (which is shaped for site_visit_cost_submissions — site_visit_id,
// transportation/accommodation/meal/other cost fields). Wrong table for the
// request shape. costSubmissionRepositoryProvider already targets the right
// table with this exact request type, so it's used directly instead of
// hand-building a request map.
class CreateCostSubmissionNotifier
  ...
  Future<void> create(CreateCostSubmissionRequest request) async {
    ...
    try {
      ...
      final totalCostCents = request.totalCostCents;
      ...
      if (isOnline) {
        ...
        final offlineSubmission = CostSubmission(
          ...
          otherCostsDetails: request.otherCostsDetails,
          submissionNotes: request.submissionNotes,
          supportingDocuments: request.supportingDocuments ?? const [],
