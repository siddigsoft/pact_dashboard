...
| Apply `docs/sql/PROGRESSIVE_OUTPUT_TRACKING.sql` (Item 4 — quantitative output tracking on `task_assignee_elements`) | 🟠 Awaiting user paste in pactdb | User | Adds `target_value`, `current_value`, `unit` columns + new `task_element_progress_log` audit table + `update_task_element_progress` RPC. UI: progress bar + inline numeric input on each element row in `TaskDetail.tsx`. Runbook: `docs/sql/PROGRESSIVE_OUTPUT_TRACKING_APPLY.md` |
| Apply `docs/sql/PROJECT_FLOW_STAGE_OVERRIDES.sql` (Item 5 — admin override of hard-coded project lifecycle stages) | 🟠 Awaiting user paste in pactdb | User | Adds `project_flow_stage_overrides` table (bilingual EN/AR label + description + key_outputs, typical_duration_days, is_disabled). Frontend integration: `useProjectFlow.ts` merges overrides on top of `PROJECT_FLOWS`; admin page at `/admin/project-flow-stages` (admin / super_admin only). Runbook: `docs/sql/PROJECT_FLOW_STAGE_OVERRIDES_APPLY.md` |
| Apply `docs/sql/PROJECT_FIELD_TASK_DEPENDENCIES.sql` (Item 6 — MS Project FS/SS/FF/SF typed task dependencies) | 🟠 Awaiting user paste in pactdb | User | New `project_field_task_dependencies` table with `dep_type` (FS/SS/FF/SF) + `lag_days` (positive=lag, negative=lead). Cycle-detection trigger + same-project guard. Migration block copies legacy `project_field_tasks.dependencies` uuid[] → rows as FS/lag-0 (legacy column kept for backward-compat reads). RLS: write requires SELECT visibility on **both** endpoint tasks (not just predecessor existence — hardened after architect review). Frontend: per-dep type+lag editor in `ProjectFieldTasksPanel.tsx` task form, type/lag badges in detail dialog. Runbook: `docs/sql/PROJECT_FIELD_TASK_DEPENDENCIES_APPLY.md` |
| Item 6 follow-up — route dep writes through SECURITY DEFINER RPC | 🟠 Deferred (Phase 2) | Agent | Current RLS allows any user with SELECT on both endpoint tasks to wire them together (matches the planner UX). For tighter "only the successor's writer may add deps" semantics, ship `upsert_project_field_task_dep` + `delete_project_field_task_dep` RPCs that re-check writer auth on the successor and lock RLS to deny direct writes. |
| Item 6 follow-up — migrate detail/blocking/cycle UI reads to typed table | 🟠 Deferred | Agent | Detail and blocking lists still read `project_field_tasks.dependencies` (uuid[]). After ~1 release of stable typed writes, migrate readers to `project_field_task_dependencies` and drop the legacy column to eliminate the rare "prerequisite row without type/lag badge" state when typed and legacy diverge. |
| Redeploy `send-whatsapp` edge function | 🟠 Awaiting user — `supabase functions deploy send-whatsapp` | User | Item-1 patch removed the duplicate WA send (now flows only through `dispatch-notification → send-whatsapp` fan-out). The deployed function still has the legacy direct-call code path; redeploy to roll out. |
