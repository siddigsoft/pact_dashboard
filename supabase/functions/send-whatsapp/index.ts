...
const EVENT_TO_TEMPLATE: Record<string, TemplateMapping> = {
  ...
  cycle_close_step4_ready:   { template: 'pact_reminder', vars: d => [v(d.recipient_name, 'there'), `Cycle Close Step 4: ${v(d.cycle)} — ${v(d.uncovered_count, '0')} site(s) need uncovered reasons`, v(d.url, APP_URL)] },
  cycle_close_step4_drafts_saved: { template: 'pact_reminder', vars: d => [v(d.recipient_name, 'there'), `Confirm uncovered reasons: ${v(d.cycle)} — ${v(d.uncovered_count, '0')} site(s) waiting for supervisor confirmation`, v(d.url, APP_URL)] },
  cycle_close_step4_all_confirmed: { template: 'pact_reminder', vars: d => [v(d.recipient_name, 'there'), `Uncovered sites confirmed for ${v(d.cycle)} — continue Cycle Close at Exceptions`, v(d.url, APP_URL)] },
...
const EVENT_CATEGORY_MAP: Record<string, WhatsAppCategoryCol> = {
  ...
  cycle_close_step4_ready: 'whatsapp_notify_mmp',
  cycle_close_step4_drafts_saved: 'whatsapp_notify_mmp',
  cycle_close_step4_all_confirmed: 'whatsapp_notify_mmp',
