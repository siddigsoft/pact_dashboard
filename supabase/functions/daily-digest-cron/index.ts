...
async function fetchCoordinators(sb: ReturnType<typeof createClient>): Promise<CoordRow[]> {
  const { data } = await sb.from('mmp_site_entries')
    .select('id, accepted_by, status, hub_office, verified_at, visit_completed_at, completed_at, updated_at, dispatched_at')
  ...
  for (const e of (data ?? [])) {
    ...
    if (verified) { r.totalVerified++; const ts = (e.visit_completed_at ?? (e as any).completed_at ?? e.verified_at ?? (verified ? e.updated_at : null)) as string | null; if (ts && (!r.lastVerifiedAt || ts > r.lastVerifiedAt)) r.lastVerifiedAt = ts }
...
async function fetchWeeklyStats(sb: ReturnType<typeof createClient>): Promise<WeeklyStats | null> {
  ...
  try {
    ...
    const [a, b, c, d] = await Promise.all([
      sb.from('mmp_site_entries').select('id', { count: 'exact', head: true }).in('status', ['verified', 'completed']).or(`verified_at.gte.${weekStart},visit_completed_at.gte.${weekStart},completed_at.gte.${weekStart}`),
...
async function sendDigest(sb: ReturnType<typeof createClient>, recipientId: string, msg: Msg): Promise<void> {
  ...
  const { data: inserted } = await sb.from('notifications').insert({
    ...
    event_type: 'daily_digest', type: 'daily_digest', entity_type: 'mmp',
...
async function sendWeeklySummary(sb: ReturnType<typeof createClient>, recipientId: string, data: DigestData, scopeLabel: string, scopeLabelAr: string): Promise<void> {
  ...
  await sb.from('notifications').insert({
    ...
    event_type: 'weekly_digest', type: 'weekly_digest', entity_type: 'mmp', action_url: '/mmp',
    is_read: false, created_at: new Date().toISOString(),
  })
}
