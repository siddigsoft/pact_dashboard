-- Migration: Down-Payment Request System & Super-Admin Role (TYPE-CORRECTED)
-- Date: 2025-11-25
-- FIXES: hub_id changed from UUID to TEXT to match profiles table schema
...
CREATE TABLE IF NOT EXISTS down_payment_requests (
  ...
  requester_role TEXT,
  hub_id TEXT,  -- CHANGED TO TEXT to match profiles.hub_id
  hub_name TEXT,
...
-- ============================================================================
-- 6. ENFORCE 3-ACCOUNT LIMIT FOR SUPER-ADMINS
-- ============================================================================
...
BEGIN
  IF NEW.hub_id IS NOT NULL THEN
    ...
    AND p.hub_id = NEW.hub_id  -- Both are TEXT now, no type mismatch!
    AND p.status = 'active'
    LIMIT 1;
...
-- ============================================================================
-- 9. RLS POLICIES - Down-Payment Requests
-- ============================================================================
...
-- Hub supervisors can view requests from their hub
...
-- Hub supervisors can update requests from their hub
...
-- ============================================================================
-- 10. RLS POLICIES - Cost Adjustment Audit
-- ============================================================================
...
-- All authenticated users can view audit records
...
-- Only existing super-admins can view super-admin records
...
-- Only existing super-admins can manage super-admin records
...
-- ============================================================================
-- 12. RLS POLICIES - Deletion Audit Log
-- ============================================================================
...
COMMENT ON TABLE down_payment_requests IS 'Down-payment request system - Migration 20251125 (TYPE-CORRECTED)';
COMMENT ON TABLE cost_adjustment_audit IS 'Cost adjustment audit trail - Migration 20251125 (TYPE-CORRECTED)';
COMMENT ON TABLE super_admins IS 'Super-admin role with 3-account limit - Migration 20251125 (TYPE-CORRECTED)';
COMMENT ON TABLE deletion_audit_log IS 'Deletion audit log - Migration 20251125 (TYPE-CORRECTED)';
...
BEGIN
  ...
  RAISE NOTICE '✅ Database-enforced 3-account limit for super-admins';
  RAISE NOTICE '✅ FIX: Changed hub_id to TEXT to match profiles table schema';
END $$;
