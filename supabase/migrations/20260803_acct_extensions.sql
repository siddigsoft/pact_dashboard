...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      INSERT INTO public.acct_journal_entries(period_id, posting_date, description_en, source_type, source_id, status, idempotency_key)
      VALUES(
        v_period.id,
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      -- Insert as DRAFT first, then lines, then UPDATE to posted
      INSERT INTO public.acct_journal_entries(period_id, posting_date, description_en, source_type, source_id, status, idempotency_key)
      VALUES(v_period.id, now()::date, 'EOSB Accrual: ' || v_row.period, 'eosb_accrual', v_row.id::text, 'draft', 'eosb_' || v_row.id::text)
      RETURNING id INTO v_entry_id;
