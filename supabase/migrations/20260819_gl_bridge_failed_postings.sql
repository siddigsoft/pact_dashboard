...
BEGIN
  ...
  IF v_source_table = 'down_payment_requests' THEN
    ...
    IF v_post_amount IS NULL
       AND v_event_type IN (
         'installment_retroactive',
         'down_payment_fully_paid',
         'dp_fully_paid'
       )
       -- A row with a recorded amount belongs to the newer installment path
       -- and must always be retried by that exact amount instead.
       AND NOT EXISTS (
         SELECT 1
         FROM public.acct_gl_bridge_log l
         WHERE l.source_table = v_source_table
      ...
      -- Older source-level backfill errors did not record amount. Their safe,
      -- deterministic recovery amount is the remaining unposted balance for
      -- this one advance—not a bulk retry across any other source records.
      v_post_amount := v_gap;
    END IF;
    IF v_post_amount IS NULL OR v_post_amount <= 0 THEN
      UPDATE public.acct_gl_bridge_log
      SET error_message = 'The selected failure has no retryable amount. Resolve the legacy posting data before retrying.'
      WHERE id = p_log_id;
      RETURN jsonb_build_object(
        'status', 'error',
        'error', 'The selected failure has no retryable amount. Resolve the legacy posting data before retrying.'
      );
    END IF;
