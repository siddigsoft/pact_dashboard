-- =============================================================================
-- DEDUCT APPROVED ADVANCES ON SITE COMPLETION
-- Extends create_wallet_transaction_on_completion to also deduct advances with
-- status 'approved' (when total_paid_amount is not set, use approved_amount or
-- requested_amount). Aligns with web wallet-transactions.ts so that advances
-- disbursed at approval (without a separate "Mark as paid" step) are still
-- deducted when the site is completed.
-- =============================================================================
...
BEGIN
  ...
  IF (NEW.status IS DISTINCT FROM OLD.status)
    ...
    -- Deduct unreconciled transport advances for this site.
    -- Include 'approved' so advances disbursed at approval (without Mark as paid) are deducted.
    -- Amount per row: COALESCE(total_paid_amount, approved_amount, requested_amount, 0).
    SELECT COALESCE(SUM(
      GREATEST(0,
        COALESCE(dpr.total_paid_amount, dpr.approved_amount, dpr.requested_amount, 0)
      )
    ), 0) INTO v_advance_deducted
    FROM public.down_payment_requests dpr
    WHERE dpr.requested_by = v_user_id
      AND dpr.status IN ('partially_paid', 'fully_paid', 'approved')
      AND (dpr.metadata->>'advance_reconciled_at') IS NULL
      AND COALESCE(dpr.total_paid_amount, dpr.approved_amount, dpr.requested_amount, 0) > 0
      AND (
...
COMMENT ON FUNCTION public.create_wallet_transaction_on_completion() IS
  'Creates wallet transactions when site visits are marked completed. Deducts unreconciled transport advances (status partially_paid, fully_paid, or approved) so wallet/site earning reflect net amount (fee minus advance).';
