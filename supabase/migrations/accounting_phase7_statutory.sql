...
do $$ begin
  ...
  if not exists (
    ...
    execute $p$create policy "tax_brackets_modify" on public.acct_tax_brackets
...
do $$ begin
  ...
  if not exists (
    ...
    execute $p$create policy "tax_wh_modify" on public.acct_tax_withholding
      for all to authenticated
      using (
        exists (
          select 1 from public.profiles
...
do $$ begin
  ...
  if not exists (
    ...
    execute $p$create policy "statutory_filings_modify" on public.acct_statutory_filings
      for all to authenticated
      using (
        exists (
          select 1 from public.profiles
