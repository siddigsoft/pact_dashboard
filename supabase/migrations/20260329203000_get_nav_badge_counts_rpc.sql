-- Single round-trip nav badge counts (SECURITY INVOKER — respects RLS). Optional: client falls back to head queries if RPC errors.
...
COMMENT ON FUNCTION public.get_nav_badge_counts(
  uuid, boolean, boolean, boolean, boolean, boolean, boolean, boolean, boolean
) IS 'Nav bar badge counts; mirrors use-nav-badge-counts filters. SECURITY INVOKER.';
