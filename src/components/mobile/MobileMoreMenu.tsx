...
export function MobileMoreMenu({ isOpen, onClose }: MobileMoreMenuProps) {
  ...
  const handleNavigate = (path: string) => {
    ...
    // Close overlay first, then navigate after animation completes
    onClose();
    setTimeout(() => {
      navigate(path);
        ...
        <>
          ...
          >
            <div className="flex flex-col h-full safe-area-top safe-area-bottom">
                  ...
                  {menuSections.map((section) => {
                    ...
                    return (
                      <div key={section.id} data-testid={`section-${section.id}`}>
                        <h3 className="text-[10px] font-bold text-black/40 dark:text-white/40 uppercase tracking-wider mb-1.5 px-1">
                          {section.title}
                        </h3>
              ...
              <div className="p-3 pt-2 pb-3 border-t border-black/10 dark:border-white/10 safe-area-bottom">
                <Button
                  variant="outline"
                  onClick={handleLogout}
                  className="w-full h-11 rounded-full !border-black/20 dark:!border-white/20 !text-black dark:!text-white font-medium gap-2 !bg-transparent"
