...
export function FilterControlsTab({ userId, userRole, isSelectedSuperAdmin }: TabProps) {
  const { loading, loadError, savingKey, effectiveFilter, explainFilter, toggleFilter, effectiveRoleNames, filterConfigs } = useSelectedUserAccess();
  const [query, setQuery] = useState('');
  const [page, setPage] = useState('all');
  const [selectedRole, setSelectedRole] = useState(effectiveRoleNames[0] ?? userRole);
  useEffect(() => {
    if (!effectiveRoleNames.includes(selectedRole)) setSelectedRole(effectiveRoleNames[0] ?? userRole);
  }, [effectiveRoleNames, selectedRole, userRole]);
  ...
  return <div className="flex h-full flex-col overflow-hidden">
    ...
    <div className="flex-1 overflow-y-auto p-4 space-y-2">{rows.map(item => {
      const effect = effectiveFilter(item.key); const trace = explainFilter(item.key);
      const userOverride = filterConfigs.find(row => row.user_id === userId && row.filter_key === item.key);
      const roleOverride = filterConfigs.find(row => row.role === selectedRole && row.filter_key === item.key);
      return <div key={item.key} className={cn('flex items-center gap-3 rounded-lg border p-3', effect === 'blocked' && 'bg-muted/40 opacity-70')}>
        ...
        {userOverride && <Badge variant="outline" className="text-[9px]">{userOverride.is_hidden ? 'User hidden' : 'User visible'}</Badge>}
        {!isSelectedSuperAdmin && <div className="flex gap-1">
          <button disabled={savingKey === `filter:user:${item.key}`} className="inline-flex items-center gap-1 rounded border px-2 py-1 text-[10px]" onClick={() => void toggleFilter(item.key)}>{userOverride ? <><RotateCcw className="h-3 w-3" /> Restore user</> : effect === 'blocked' ? <><Eye className="h-3 w-3" /> Show for user</> : <><EyeOff className="h-3 w-3" /> Hide for user</>}</button>
          <button disabled={savingKey === `filter:role:${item.key}`} className="rounded border px-2 py-1 text-[10px]" onClick={() => void toggleFilter(item.key, 'role', selectedRole)}>{roleOverride ? 'Restore role' : 'Hide for role'}</button>
        </div>}
      </div>;
    })}</div>
