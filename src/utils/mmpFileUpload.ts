...
export async function parseAndCountEntries(file: File, sheetName?: string): Promise<{ entries: MMPSiteEntry[]; count: number; errors: string[]; warnings: string[]; rawErrors: CSVValidationError[]; rawWarnings: CSVValidationError[] }> {
  ...
  try {
    ...
    const rawHeaderMap: Record<string, keyof MMPSiteEntry> = {
      // ── Hub / Office ─────────────────────────────────────────────────────
      'huboffice': 'hubOffice',
      'hub office': 'hubOffice',
      'hub': 'hubOffice',
      'hubofficename': 'hubOffice',
      'hub office name': 'hubOffice',
      'officename': 'hubOffice',
      'office name': 'hubOffice',
      'office': 'hubOffice',
      'fieldoffice': 'hubOffice',
      'field office': 'hubOffice',
      'suboffice': 'hubOffice',
      'sub office': 'hubOffice',
      // ── Site Code ────────────────────────────────────────────────────────
      'sitecode': 'siteCode',
      'site code': 'siteCode',
      'siteid': 'siteCode',
      ...
      'implementing partner': 'cpName',
      'ipname': 'cpName',
      'ip name': 'cpName',
      'cooperating partner': 'cpName',
      'cooperatingpartner': 'cpName',
      'ngo': 'cpName',
      'organization': 'cpName',
      'organisation': 'cpName',
      // ── Main Activity ────────────────────────────────────────────────────
      ...
      'activity type': 'mainActivity',
      'mainactivitytype': 'mainActivity',
      'main activity type': 'mainActivity',
      'programmeactivity': 'mainActivity',
      'programme activity': 'mainActivity',
      'programactivity': 'mainActivity',
      'program activity': 'mainActivity',
      'modality': 'mainActivity',
      'intervention': 'mainActivity',
      'transfermodality': 'mainActivity',
      'transfer modality': 'mainActivity',
      // ── Visit Type ───────────────────────────────────────────────────────
      'visittype': 'visitType',
      ...
      'planned date': 'visitDate',
      'planneddate': 'visitDate',
      'monitoring date': 'visitDate',
      'monitoringdate': 'visitDate',
      // ── Comments ─────────────────────────────────────────────────────────
      'comments': 'comments',
      'comment': 'comments',
      'remarks': 'comments',
      ...
      'activityatdistributionsite': 'siteActivity',
      'activity at distribution site': 'siteActivity',
      'typeofactivityatsite': 'siteActivity',
      'type of activity at site': 'siteActivity',
      // ── Monitoring By ────────────────────────────────────────────────────
      'monitoringby': 'monitoringBy',
      'monitoring by': 'monitoringBy',
      'monitoredby': 'monitoringBy',
      'monitored by': 'monitoringBy',
      'monitorby': 'monitoringBy',
      'monitor by': 'monitoringBy',
      'tpm': 'monitoringBy',
      // ── Survey / PDM Tool ────────────────────────────────────────────────
      ...
      'tooltobeused': 'surveyTool',
      'tool to be used': 'surveyTool',
      'toolused': 'surveyTool',
      'tool used': 'surveyTool',
      'monitoringtool': 'surveyTool',
      'monitoring tool': 'surveyTool',
      'mastersurvey': 'surveyTool',
      'master survey': 'surveyTool',
      // ── Use Market Diversion (MDM trigger) ───────────────────────────────
      'usemarketdiversion': 'useMarketDiversion',
      'use market diversion': 'useMarketDiversion',
      'usemarketdiversionmonitoring': 'useMarketDiversion',
      ...
      'marketdiversionflag': 'useMarketDiversion',
      'market diversion flag': 'useMarketDiversion',
      'marketdiversionrequired': 'useMarketDiversion',
      'market diversion required': 'useMarketDiversion',
      'usemdm': 'useMarketDiversion',
      'use mdm': 'useMarketDiversion',
      'mdmrequired': 'useMarketDiversion',
      'mdm required': 'useMarketDiversion',
      'mdmmonitoring': 'useMarketDiversion',
      'mdm monitoring': 'useMarketDiversion',
      // ── Use Warehouse Monitoring (WHM trigger) ───────────────────────────
      ...
      'warehousemonitoringflag': 'useWarehouseMonitoring',
      'warehouse monitoring flag': 'useWarehouseMonitoring',
      'warehousemonitoringrequired': 'useWarehouseMonitoring',
      'warehouse monitoring required': 'useWarehouseMonitoring',
      'warehouseflag': 'useWarehouseMonitoring',
      'warehouse flag': 'useWarehouseMonitoring',
      'usewhm': 'useWarehouseMonitoring',
      'use whm': 'useWarehouseMonitoring',
      'whmrequired': 'useWarehouseMonitoring',
      'whm required': 'useWarehouseMonitoring',
      'whmmonitoring': 'useWarehouseMonitoring',
      'whm monitoring': 'useWarehouseMonitoring',
    };
...
export async function uploadMMPFile(
  ...
  try {
    ...
    const uploadTimeoutMs = Math.min(
      10 * 60 * 1000,
      Math.max(3 * 60 * 1000, 2 * 60 * 1000 + Math.ceil(file.size / (512 * 1024)) * 10 * 1000)
    );

    const uploadResult = await safeUploadFile(file, {
      bucket: 'mmp-files',
      path: '', // filePath is generated by safeUploadFile
      allowedTypes: ['application/vnd.ms-excel', 'text/csv', 'application/csv', 'text/plain'],
    ...
    if (!uploadResult.success || !uploadResult.url) {
      ...
      return { success: false, error: 'Failed to upload file to storage: ' + (uploadResult.error || 'Unknown error') };
    }
