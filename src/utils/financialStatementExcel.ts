...
export function generateFinancialStatementExcel(
  ...
  if (config.dateRange?.from || config.dateRange?.to) {
    ...
    periodRow.height = 18;
    ws.mergeCells(periodRow.number, 1, periodRow.number, 11);
  }

  ws.addRow([]);

  const summSectionRow = ws.addRow(['SUMMARY']);
  summSectionRow.eachCell(cell => {
    cell.fill = sectionFill();
    cell.font = headerFont(12);
    cell.border = thinBorder();
  });
  summSectionRow.getCell(2).fill = sectionFill();
  summSectionRow.getCell(2).border = thinBorder();
  summSectionRow.height = 24;
  ...
  detailSectionRow.eachCell(cell => {
    cell.fill = sectionFill();
    cell.font = headerFont(12);
    cell.border = thinBorder();
  });
  for (let c = 2; c <= 11; c++) {
    detailSectionRow.getCell(c).fill = sectionFill();
    detailSectionRow.getCell(c).border = thinBorder();
  }
  detailSectionRow.height = 24;
  ws.mergeCells(detailSectionRow.number, 1, detailSectionRow.number, 11);
  ...
  if (rows.some(r => r.t1Approver || r.t2Approver)) {
    ...
    appSectionRow.eachCell(cell => {
      cell.fill = sectionFill();
      cell.font = headerFont(11);
      cell.border = thinBorder();
    });
    appSectionRow.getCell(2).fill = sectionFill();
    appSectionRow.getCell(2).border = thinBorder();
    appSectionRow.height = 22;
