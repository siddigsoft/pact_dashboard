    ...
    try {
      return await withTimeout(
        (async () => {
          ...
          // 4.5. (Removed) The three FK constraints that previously used ON DELETE NO ACTION:
          //   a) site_visit_costs.mmp_site_entry_id  → mmp_site_entries
          //   b) site_visits.mmp_site_entry_id        → mmp_site_entries
          //   c) site_visits.mmp_id                   → mmp_files
          // ...were changed to ON DELETE SET NULL by migration
          // 20260803_mmp_fk_set_null_on_delete.sql, so manual pre-clearing is no longer needed.
