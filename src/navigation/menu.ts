...
export const getWorkflowMenuGroups = (
  ...
  const isSMT =
    ...
    allNormalized.some(c => String(c).toLowerCase() === 'smt');
  ...
  if (!isHidden('/approvals') && (isAdmin || isSuperAdmin || isFinancialAdmin || isFOM || isSupervisor || isSeniorManagement))
    financeItems.push({ id: 'approvals-hub', title: 'Approvals Hub', url: '/approvals', icon: Inbox, priority: 2, isPinned: isPinned('/approvals') });
  if (!isHidden('/finance-hub') && (isAdmin || isSuperAdmin || isFinancialAdmin || isAuditor || isFOM || isCountryDirector || isSeniorManagement))
  ...
  if (!isHidden('/budget-requests') && !isSMT)
  ...
  const canSeeAnalytics = !isSMT && (isAdmin || isSuperAdmin || isICT || isFinancialAdmin || isAuditor || isFOM || isProjectManager || isCountryDirector || isSeniorManagement || perms.reports);
  ...
  if (!isHidden('/hr') && (isAdmin || isSuperAdmin || isFinancialAdmin || isSeniorManagement))
  ...
  if (!isHidden('/calendar') && (isProjectManager || isSMT || isAdmin || isCountryDirector))
  ...
  if (!isHidden('/crm') && !isSMT && (isAdmin || isSuperAdmin || isFOM || isProjectManager || isCountryDirector || isSeniorManagement))
