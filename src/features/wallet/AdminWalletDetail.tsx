...
import { fetchProfileAdminBrief } from '@/features/user/repository/profileRepository';
import { fetchMmpSiteEntriesAcceptedByUser } from '@/features/siteVisit/repository/siteVisitRepository';
import {
  adminManualBalanceAdjustment,
  adminRunWalletRecalculation,
  adminUpdateWalletTransactionAmount,
  fetchWalletRowByUserId,
  fetchWalletTransactionsForUserAdmin,
  subscribeAdminWalletDetailRealtime,
} from '@/features/wallet/repository/walletRepository';
...
const AdminWalletDetail = () => {
  ...
  const loadWalletData = async () => {
    ...
    try {
      ...
      const [profileResult, walletResult, txnResult, sitesResult] = await Promise.all([
        fetchProfileAdminBrief(userId),
        fetchWalletRowByUserId(userId),
        fetchWalletTransactionsForUserAdmin(userId, 200),
        fetchMmpSiteEntriesAcceptedByUser(userId, 100),
      ]);
  ...
  useEffect(() => {
    if (!userId) return;
    return subscribeAdminWalletDetailRealtime(userId, loadWalletData);
  }, [userId]);
  ...
  const saveEditTx = async (txnId: string) => {
    ...
    setSavingTx(true);
    try {
      const { error } = await adminUpdateWalletTransactionAmount(txnId, newAmount);
  ...
  const recalculateWalletTotals = async () => {
    ...
    try {
      const result = await adminRunWalletRecalculation(userId, wallet.id, currency);

      let toastDesc = '';
      if (result.txUpdated > 0) {
        toastDesc = `${result.txUpdated} transactions corrected. `;
      } else if (result.earningTxsCount > 0) {
        toastDesc = 'No corrections needed (amounts match site entries). ';
      }
      toastDesc += `Earned: ${result.newTotalEarned.toLocaleString()} ${result.primaryCurrency}, Balance: ${result.primaryBalance.toLocaleString()} ${result.primaryCurrency}`;
  ...
  const handleAdjustBalance = async () => {
    ...
    try {
      ...
      await adminManualBalanceAdjustment({
        walletId: wallet.id,
        userId,
        currency,
        adjustmentAmount,
