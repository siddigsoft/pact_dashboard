...
import {
  ...
  fetchWalletTransactionsWithSiteEntryAdmin,
  subscribeAdminWalletsListRealtime,
...
import { exportTransactionsToCSV, exportTransactionsToPDF } from '@/features/wallet/lib/export';
...
const AdminWallets: React.FC = () => {
  ...
  const syncAndRecalculateAll = async () => {
    ...
    for (let i = 0; i < walletsToSync.length; i++) {
      ...
      try {
        const r = await adminRunWalletRecalculation(w.user_id, w.id, currency);
        correctedTxCount += r.txUpdated;
        successCount++;
      } catch {
        errorCount++;
      }
  ...
  const loadTransactionDetails = async (walletId: string) => {
    ...
    const { data, error } = await fetchWalletTransactionsWithSiteEntryAdmin(walletId);
  ...
  useEffect(() => {
    return subscribeAdminWalletsListRealtime(() => {
      load();
    });
  }, []);
