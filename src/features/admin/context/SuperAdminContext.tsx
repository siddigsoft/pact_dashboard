...
export function SuperAdminProvider({ children }: { children: React.ReactNode }) {
  ...
  const checkSuperAdminStatus = useCallback(async (userId: string): Promise<boolean> => {
    ...
    try {
      const { data, error } = await saRepo.fetchSuperAdminActiveRecord(userId);
  ...
  const refreshSuperAdmins = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const { data, error } = await saRepo.fetchAllSuperAdmins();
  ...
  const refreshDeletionLogs = useCallback(async () => {
    try {
      const { data, error } = await saRepo.fetchDeletionAuditLogs(100);
      ...
      checkSuperAdminStatus(currentUser.id).then(async (isSuper) => {
        if (isSuper) {
          ...
          if (userRole === 'superadmin' || userRole === 'super_admin') {
            ...
            try {
              const { data: profileByEmail } = await saRepo.fetchProfileIdByEmail(currentUser.email);
              
              if (profileByEmail) {
                const { data: saEntry } = await saRepo.fetchSuperAdminByProfileUserId(profileByEmail.id);
  ...
  const deactivateSuperAdmin = async (data: DeactivateSuperAdmin): Promise<boolean> => {
    ...
    try {
      const { data: updateData, error } = await withTimeout(
        saRepo.updateSuperAdminDeactivate(data.superAdminId, {
          is_active: false,
  ...
  const deleteSuperAdmin = async (data: DeleteSuperAdmin): Promise<boolean> => {
    ...
    try {
      ...
      const { data: superAdminRecord, error: fetchError } = await saRepo.fetchSuperAdminById(data.superAdminId);
      ...
      const { error } = await saRepo.deleteSuperAdminById(data.superAdminId);
      ...
      await saRepo.insertDeletionAuditLogRow({
  ...
  const logDeletion = async (data: CreateDeletionLog): Promise<boolean> => {
    ...
    try {
      const result = await withTimeout(
        (async () => {
          const { error } = await saRepo.insertDeletionAuditLogRow({
          ...
          await saRepo.rpcIncrementSuperAdminDeletionCount(data.deletedBy);
  ...
  const sendNotificationToUser = async (
    ...
    try {
      await saRepo.insertSuperAdminNotification({
        recipient_id: userId,
        title_en: title,
        title_ar: title,
        message_en: message,
