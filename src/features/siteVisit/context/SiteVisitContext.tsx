...
import {
  ...
  updateSiteVisitInDb,
  deleteSiteVisitInDb,
  fetchDataCollectorProfiles as dbFetchDataCollectorProfiles,
    ...
    try {
      ...
      try {
        ...
        if (consideredUsers.length === 0) {
          try {
            const { profiles: profilesData, roles: rolesData } = await dbFetchDataCollectorProfiles();
        ...
        try {
          const activeEntries = await dbFetchMmpSiteEntryWorkloads();
    ...
    try {
      ...
      try {
        ...
        if (hubId && activityName) {
          // Query total and completed sites for this activity in this hub
          const [totalResult, completedResult] = await Promise.all([
            dbFetchMmpSiteEntryIdsForActivity(hubId, activityName),
            dbFetchMmpSiteEntryIdsForActivity(hubId, activityName, 'completed'),
          ]);

          const allIds = new Set([...totalResult.byMainActivity, ...totalResult.byActivityAtSite]);
          const totalSites = allIds.size;

          const completedIds = new Set([...completedResult.byMainActivity, ...completedResult.byActivityAtSite]);
