...
import {
  fetchMmpFilesForArchive,
  fetchMmpSiteEntriesForArchive,
  fetchReportPhotosForArchive,
} from '@/features/archive/repository/archiveRepository';
...
export const ArchiveProvider: React.FC<ArchiveProviderProps> = ({ children, currentUser }) => {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const { data: mmpRows, error: mmpErr } = await fetchMmpFilesForArchive();
      if (mmpErr) throw mmpErr;

      const { data: mmpEntriesData, error: svErr } = await fetchMmpSiteEntriesForArchive();
      if (svErr) {
      ...
      let photoRows: any[] = [];
      try {
        const { data: pr, error: perr } = await fetchReportPhotosForArchive();
