...
import {
  invokeSelfChangePassword,
  signInWithPasswordForBiometricVerification,
  uploadAvatarAndUpdateUserProfile,
} from "@/features/settings/repository/settingsSupabaseRepository";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
import LocationCapture from "@/features/location/components/LocationCapture";
...
import { BankakAccountForm, BankakAccountFormValues } from "@/features/wallet/components/BankakAccountForm";
import { Badge } from "@/components/ui/badge";
import { NotificationSettings as NotificationSettingsComponent } from "@/features/settings/components/NotificationSettings";
...
import { MobileSettingsScreen } from "@/platform/mobile/components/MobileSettingsScreen";
  ...
  const handleConfirmEnableBiometric = async () => {
    ...
    try {
      const { error: signInError } =
        await signInWithPasswordForBiometricVerification({
...
const Settings = () => {
  ...
  const handleAvatarUpload = async (
    ...
    try {
      ...
      const { avatarUrl } = await uploadAvatarAndUpdateUserProfile({
  ...
  const handleChangePassword = async () => {
    ...
    setChanging(true);
    try {
      const { data, error } = await invokeSelfChangePassword({
