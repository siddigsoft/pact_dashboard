...
import { fetchActiveSuperAdminUserIds } from '@/features/approval/repository/approvalRepository';
...
export function ApprovalProvider({ children }: { children: ReactNode }) {
  ...
  const notifySuperAdmins = useCallback(async (request: ApprovalRequest) => {
    try {
      const userIds = await fetchActiveSuperAdminUserIds();
      if (userIds.length > 0) {
        ...
        console.log('[Approval] Notified', userIds.length, 'super admins with email');
      }
    } catch (error) {
