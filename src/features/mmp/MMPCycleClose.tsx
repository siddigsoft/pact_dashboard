...
import {
  fetchAdminFomSuperAdminRecipients,
  getSuperAdminEmails,
  fetchHubNamesByIds,
  fetchUncoveredSiteVisitsForCycleClose,
  fetchClosedMmpFiles,
  fetchClosedCycleSiteVisitsStats,
  fetchMmpSiteEntriesScopeOptions,
  fetchMmpSiteEntryIdsForCloseScope,
  fetchMatchedVisitsForCloseScope,
  markSiteVisitsNotCoveredFlagByIds,
  updateMmpFileById,
  fetchSiteVisitCountsForMmpIds,
  fetchSiteVisitsAdditionalDataForQuality,
  fetchAffectedVisitIdsForStartClosingCycle,
  markSiteVisitsNotCoveredFlagByMmpId,
  updateSiteVisitNotCoveredReason,
  bulkUpdateSiteVisitsNotCoveredReason,
  cancelSiteVisitsForClosedCycle,
  fetchApprovedProfilesForReminders,
  fetchApprovedSuperAdminsForReminders,
  fetchHubIdByNameLike,
  fetchApprovedProfilesByHubIdAndRoles,
} from '@/features/mmp/repository/mmpCycleCloseRepository';
...
const MMPCycleClose = () => {
  ...
  useEffect(() => {
    if (isSupervisor && currentUser?.hubId) {
      ...
      fetchHubNamesByIds(hubIds).then((names) => {
  ...
  const fetchUncoveredSites = useCallback(async () => {
    ...
    try {
      ...
      const data = await fetchUncoveredSiteVisitsForCycleClose({
        mmpIds,
        useNotCoveredFlag: closingMmps.length > 0,
      });
  ...
  const fetchClosedCycles = useCallback(async () => {
    try {
      const data = await fetchClosedMmpFiles();
      ...
      if (records.length > 0) {
        ...
        const siteStats = await fetchClosedCycleSiteVisitsStats(cycleIds);
  ...
  const fetchMmpScopeOptions = useCallback(async (mmpId: string): Promise<MmpScopeOptions> => {
    try {
      const entries = await fetchMmpSiteEntriesScopeOptions(mmpId);
  ...
  const handleScopedClose = async (mmpId: string, scope: CloseScope, scopeValue: string) => {
    ...
    try {
      ...
      siteEntryIds = await fetchMmpSiteEntryIdsForCloseScope({
        mmpFileId: mmpId,
        scope,
        scopeValue,
      });
      ...
      const matchedVisits = await fetchMatchedVisitsForCloseScope({
        mmpFileId: mmpId,
        siteEntryIds,
      });
      ...
      if (visitIds.length > 0) {
        await markSiteVisitsNotCoveredFlagByIds(visitIds);
      }
      ...
      await updateMmpFileById(mmpId, updateData);
  ...
  useEffect(() => {
    const fetchSiteVisitCounts = async () => {
      ...
      try {
        const data = await fetchSiteVisitCountsForMmpIds(mmpIds);
        if (data) {
  ...
  useEffect(() => {
    const fetchQualityData = async () => {
      try {
        const data = await fetchSiteVisitsAdditionalDataForQuality();
  ...
  const handleStartClosingCycle = async (mmpId: string) => {
    ...
    setClosingCycle(true);
    try {
      const affectedVisits = await fetchAffectedVisitIdsForStartClosingCycle({ mmpFileId: mmpId });
      ...
      await markSiteVisitsNotCoveredFlagByMmpId({ mmpFileId: mmpId });

      const mmp = mmpFiles?.find(m => m.id === mmpId);
      const mmpName = mmp?.name || 'MMP';
      const mmpHub = mmp?.hub || mmp?.region || '';

      const supervisors = await fetchApprovedProfilesForReminders({
        hubNameLike: mmpHub || undefined,
      ...
      const foms = await fetchApprovedProfilesForReminders({
        hubNameLike: mmpHub || undefined,
        roles: ['fom', 'Field Operation Manager (FOM)'],
      });
  ...
  const handleAssignReason = async (siteId: string, reason: NotCoveredReason, otherText?: string) => {
    ...
    try {
      ...
      await updateSiteVisitNotCoveredReason({ siteVisitId: siteId, payload: updateData });
  ...
  const handleBulkAssignReason = async () => {
    ...
    try {
      ...
      await bulkUpdateSiteVisitsNotCoveredReason({ siteVisitIds: siteIds, payload: updateData });
  ...
  const handleFinalizeCycleClose = async (mmpId: string) => {
    ...
    try {
      await updateMmpFileById(mmpId, { cycle_status: 'pending_approval' });
  ...
  const handleApproveCycle = async (mmpId: string) => {
    try {
      ...
      await cancelSiteVisitsForClosedCycle({ mmpFileId: mmpId });
  ...
  const handleSendReminders = async (mmpId: string) => {
    try {
      ...
      if (mmpHub) {
        const hubId = await fetchHubIdByNameLike(mmpHub);
        if (hubId) {
          const hubRecipients = await fetchApprovedProfilesByHubIdAndRoles({
            hubId,
            roles: ['Supervisor', 'supervisor', 'fom', 'Field Operation Manager (FOM)'],
          });

          const superAdmins = await fetchApprovedSuperAdminsForReminders();
      ...
      const recipients = await fetchApprovedProfilesForReminders({ roles: recipientRoles });
      const superAdminCc = await getSuperAdminEmails();
