...
import {
  fetchCollectorClassificationFeeForDispatch,
  fetchDispatchCollectorsProfiles,
  fetchLastVisitsBySiteCodesForDispatch,
  fetchPriorAdvancesForDispatch,
  getSupabaseAuthSession,
  fetchDispatchTeamMembers,
  fetchProfileBasicByUserId,
  fetchMmpSiteEntryDispatchSnapshot,
  returnSelectedMmpSiteEntriesToFom,
  updateMmpSiteEntryTransportFeeAndDispatchAdditionalData,
  updateMmpSiteEntryDispatch,
} from '@/features/mmp/repository/dispatchSitesRepository';
...
export const DispatchSitesDialog: React.FC<DispatchSitesDialogProps> = ({
  ...
  useEffect(() => {
    ...
    const loadClassificationFee = async () => {
      ...
      try {
        const fee = await fetchCollectorClassificationFeeForDispatch(selectedCollector);
        if (!cancelled) setCollectorClassificationFee(fee);
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const loadCollectors = async () => {
      try {
        ...
        const data = await fetchDispatchCollectorsProfiles();
    ...
    (async () => {
      try {
        ...
        if (siteCodes.length > 0) {
          const visits = await fetchLastVisitsBySiteCodesForDispatch(siteCodes);
  ...
  const handleReturnSelectedSites = async () => {
    ...
    try {
      ...
      await returnSelectedMmpSiteEntriesToFom(siteIds, returnReason);
  ...
  const handleDispatch = async () => {
    ...
    try {
      ...
      const priorAdvances = await fetchPriorAdvancesForDispatch(selectedIds);
    ...
    try {
      ...
      for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
          const { data: localSession } = await getSupabaseAuthSession();
      ...
      for (const siteEntry of selectedSiteObjects) {
        ...
        if (costs) {
          ...
          try {
            const costUpdatePromise = updateMmpSiteEntryTransportFeeAndDispatchAdditionalData(
              siteEntry.id,
              {
      ...
      const { data: teamMembers } = await fetchDispatchTeamMembers({
        roles: [
          "coordinator",
          "supervisor",
          "admin",
        ...
        stateIds: stateIds.size > 0 ? Array.from(stateIds) : undefined,
      });
      ...
      if (sessionResult.user?.id) {
        try {
          const profilePromise = fetchProfileBasicByUserId(sessionResult.user.id);
      ...
      for (const entryId of Array.from(selectedSites)) {
        ...
        try {
          const fetchPromise = fetchMmpSiteEntryDispatchSnapshot(entryId);
        ...
        try {
          const updatePromise = updateMmpSiteEntryDispatch(entryId, updateData);
