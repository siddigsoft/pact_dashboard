...
export function useBiometric(): UseBiometricReturn {
  ...
  const authenticateAndGetCredentials = useCallback(async (): Promise<{
    ...
    try {
      const { verifyDeviceTrust } = await import('@/platform/mobile/device-trust');
...
export async function saveCredentials(
  ...
  try {
    ...
    try {
      const { registerTrustedDevice } = await import('@/platform/mobile/device-trust');
...
export async function deleteCredentials(): Promise<{ success: boolean; error?: string }> {
  try {
    ...
    if (username) {
      try {
        const { revokeDeviceTrust } = await import('@/platform/mobile/device-trust');
