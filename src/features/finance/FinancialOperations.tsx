...
import { useCostSubmissions } from '@/features/costApproval/context/CostSubmissionContext';
...
import {
  fetchFinancialPeriodCloseRows,
  insertFinancialPeriodCloseRow,
  insertFinancialAuditLog,
  deleteFinancialPeriodCloseRowByMonth,
  fetchWalletTransactionsForPeriodClose,
  fetchOperationalCostSubmissionsForPeriodClose,
  fetchOperationalCostsLatest,
  fetchExchangeRatesLatest,
  fetchOverdueReconciliationItems,
  fetchAgingApprovalsItems,
  fetchAdvanceLiquidationItems,
  fetchCashFlowTransactionsAndPaidCosts,
} from '@/features/finance/repository/financialOperationsRepository';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
const FinancialOperations = () => {
  ...
  const fetchClosedPeriods = async () => {
    try {
      const data = await fetchFinancialPeriodCloseRows();
  ...
  const confirmClosePeriod = async () => {
    ...
    try {
      await insertFinancialPeriodCloseRow({
        periodMonth: month,
        status: 'closed',
        closedBy: actorId,
  ...
  const confirmReopenPeriod = async () => {
    ...
    try {
      await deleteFinancialPeriodCloseRowByMonth(month);
  ...
  useEffect(() => {
    ...
    const fetchPeriodData = async () => {
      ...
      try {
        ...
        const [walletRows, opCostRows] = await Promise.all([
          fetchWalletTransactionsForPeriodClose(fromDate),
          fetchOperationalCostSubmissionsForPeriodClose(fromDate),
        ]);
  ...
  useEffect(() => {
    const fetchOpCosts = async () => {
      try {
        const data = await fetchOperationalCostsLatest();
  ...
  useEffect(() => {
    ...
    const fetchAlerts = async () => {
      ...
      try {
        ...
        const rateData = await fetchExchangeRatesLatest();
        if (rateData) {
          const lastUpdate = parseISO(rateData.updated_at || rateData.created_at);
          ...
          if (hoursStale > 72) {
            alerts.push({
              ...
              timestamp: rateData.updated_at || rateData.created_at,
        ...
        const overdueRecon = await fetchOverdueReconciliationItems(sevenDaysAgo);
        ...
        const agingApprovals = await fetchAgingApprovalsItems(threeDaysAgo);
        ...
        const advanceLiq = await fetchAdvanceLiquidationItems(sevenDaysAgo);
  ...
  useEffect(() => {
    ...
    const fetchCashFlow = async () => {
      ...
      try {
        ...
        const { credits, debits, paidCosts } =
          await fetchCashFlowTransactionsAndPaidCosts(fromDate);
