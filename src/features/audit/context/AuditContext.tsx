...
import { fetchAuditLogs, insertAuditLogRow } from '@/features/audit/repository/auditRepository';
...
export function AuditProvider({ children }: { children: ReactNode }) {
  ...
  const loadLogs = useCallback(async () => {
    ...
    try {
      ...
      const { data: dbLogs, error } = await fetchAuditLogs(MAX_LOGS);
  ...
  const saveLogToDatabase = useCallback(async (log: AuditLogEntry): Promise<boolean> => {
    try {
      const { error } = await insertAuditLogRow(log);
