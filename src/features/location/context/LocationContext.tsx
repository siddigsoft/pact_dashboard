...
import {
  ...
  fetchActiveHubs,
  fetchHubStates,
  fetchSitesRegistryLocalities,
} from '@/features/location/repository/locationRepository';
...
export const useLocationProvider = () => {
  ...
  const refreshLocationData = useCallback(async () => {
    try {
      ...
      const { data: hubsData, error: hubsError } = await fetchActiveHubs();
      if (hubsError) throw hubsError;
      setHubs(hubsData || []);

      const { data: hubStatesData, error: hubStatesError } = await fetchHubStates();
      if (hubStatesError) throw hubStatesError;
      setHubStates(hubStatesData || []);

      const { data: localitiesData, error: localitiesError } = await fetchSitesRegistryLocalities();
