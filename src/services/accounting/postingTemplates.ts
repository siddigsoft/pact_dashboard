...
const PRE_FUNDING_TEMPLATES: PostingTemplate[] = [
  {
    id: 'pre_fund_received',
    sourceTable: 'pre_fund_requests',
    eventType: 'pre_fund_received',
    triggerStatus: 'active',
    triggerCondition: "status transitions to 'active' (receipt file uploaded and confirmed)",
    labelEn: 'Pre-Fund Received — Activate Fund',
    labelAr: 'استلام التمويل المسبق — تفعيل الصندوق',
    featureFlag: 'pre_funding',
    lines: [
      {
        accountCode: '{gl_receipt_account}',
        accountName: 'Cash at Bank (resolved from fund.gl_receipt_account)',
        debitCredit: 'DR',
        amountSource: 'pre_fund_requests.amount',
        currency: 'fund.currency',
        glFunction: 'program',
        description: 'Dr Cash/Bank when pre-fund receipt is confirmed',
      },
      {
        accountCode: '{gl_liability_account}',
        accountName: 'Pre-Fund Liability (resolved from fund.gl_liability_account)',
        debitCredit: 'CR',
        amountSource: 'pre_fund_requests.amount',
        currency: 'fund.currency',
        glFunction: 'program',
        description: 'Cr deferred pre-fund liability — income deferred until expenditure incurred',
      },
    ],
    notes: 'Fires when status → active (receipt confirmed). Account codes resolved from fund.gl_receipt_account and fund.gl_liability_account at posting time. Defers recognition until funds are spent.',
  },
  {
    id: 'pre_fund_committed',
    sourceTable: 'pre_fund_transactions',
    eventType: 'pre_fund_committed',
    triggerStatus: 'commitment',
    triggerCondition: "transaction_type = 'commitment' INSERT — encumbrance raised against budget",
    labelEn: 'Pre-Fund Commitment — Raise Encumbrance',
    labelAr: 'التزام التمويل المسبق — رفع الاحتجاز',
    featureFlag: 'pre_funding',
    lines: [
      {
        accountCode: '{gl_liability_account}',
        accountName: 'Pre-Fund Liability (resolved from fund.gl_liability_account)',
        debitCredit: 'DR',
        amountSource: 'pre_fund_transactions.amount',
        currency: 'fund.currency',
        glFunction: 'program',
        description: 'Dr pre-fund liability to ring-fence committed amount',
      },
      {
        accountCode: '{gl_encumbrance_account}',
        accountName: 'Encumbrance Reserve (resolved from fund.gl_encumbrance_account)',
        debitCredit: 'CR',
        amountSource: 'pre_fund_transactions.amount',
        currency: 'fund.currency',
        glFunction: 'program',
        description: 'Cr encumbrance reserve — creates budget hold for committed spend. Account resolved from fund.gl_encumbrance_account at posting time; posting fails fast if account is not configured.',
      },
    ],
    notes: 'Fires on commitment transactions. Links to acct_budget_encumbrances via pre_fund_transactions.encumbrance_id. Both gl_liability_account and gl_encumbrance_account are resolved from pre_fund_funds.gl_encumbrance_account at execution time — no account code is hardcoded. The GL Bridge Engine must throw an explicit error if either account is missing from the COA.',
  },
  {
    id: 'pre_fund_paid',
    sourceTable: 'pre_fund_transactions',
    eventType: 'pre_fund_paid',
    triggerStatus: 'payment',
    triggerCondition: "transaction_type = 'payment' INSERT — cash disbursed from pre-fund pool",
    labelEn: 'Pre-Fund Disbursement — Reduce Pool Balance',
    labelAr: 'صرف من التمويل المسبق — تخفيض رصيد المجمع',
    featureFlag: 'pre_funding',
    lines: [
      {
        accountCode: '{gl_liability_account}',
        ...
        currency: 'fund.currency',
        glFunction: 'program',
        description: 'Dr pre-fund liability — reduces the deferred cash obligation as cash is disbursed',
      },
      {
        accountCode: '{gl_receipt_account}',
        accountName: 'Cash at Bank (resolved from fund.gl_receipt_account)',
        ...
        currency: 'fund.currency',
        glFunction: 'program',
        description: 'Cr Cash/Bank — cash leaves the pre-fund pool for the disbursement',
      },
    ],
    notes: 'Cash-flow model: disbursement reduces both the deferred liability and the bank balance. Expense recognition is handled by the linked operational module (cost_submissions / down_payment_requests) via pre_fund_transaction_id. If a commitment (pre_fund_committed) was raised earlier, the bridge engine also reverses it: Dr {gl_encumbrance_account} → Cr {gl_liability_account} for the committed amount. All account codes resolved from fund-level settings at execution time.',
  },
  {
    id: 'pre_fund_closed',
    sourceTable: 'pre_fund_reconciliations',
    eventType: 'pre_fund_closed',
    triggerStatus: 'closed',
    triggerCondition: "pre_fund_reconciliations.status transitions to 'closed' — final period reconciliation",
    labelEn: 'Pre-Fund Period Close — Reconcile Variance',
    labelAr: 'إغلاق فترة التمويل المسبق — تسوية الفرق',
    featureFlag: 'pre_funding',
    lines: [
      {
        accountCode: '{gl_liability_account}',
        accountName: 'Pre-Fund Liability — return portion (resolved from fund.gl_liability_account)',
        debitCredit: 'DR',
        amountSource: 'pre_fund_reconciliations.return_amount',
        currency: 'fund.currency',
        glFunction: 'program',
        description: 'Dr liability for the portion being returned to donor (return_amount)',
      },
      {
        accountCode: '{gl_receipt_account}',
        accountName: 'Cash at Bank — donor refund (resolved from fund.gl_receipt_account)',
        debitCredit: 'CR',
        amountSource: 'pre_fund_reconciliations.return_amount',
        currency: 'fund.currency',
        glFunction: 'program',
        description: 'Cr Cash/Bank for the amount physically returned to the donor',
      },
      {
        accountCode: '{gl_liability_account}',
        accountName: 'Pre-Fund Liability — variance/residual (resolved from fund.gl_liability_account)',
        debitCredit: 'DR',
        amountSource: 'pre_fund_reconciliations.variance minus return_amount minus carry_forward_amount',
        currency: 'fund.currency',
        glFunction: 'program',
        description: 'Dr liability for residual variance not returned or carried forward',
      },
      {
        accountCode: '{gl_expense_account}',
        accountName: 'Programme Expense (resolved from fund.gl_expense_account)',
        debitCredit: 'CR',
        amountSource: 'pre_fund_reconciliations.variance minus return_amount minus carry_forward_amount',
        currency: 'fund.currency',
        glFunction: 'program',
        description: 'Cr programme expense — recognises residual balance as cost incurred when fund closes',
      },
    ],
    notes: 'Full period-close reconciliation entry. Lines 1-2 post when return_amount > 0 (donor refund). Lines 3-4 post when residual variance > 0 (expense treatment for undisbursed balance that is neither returned nor carried forward — recognises sunk programme cost). Paired with pre_fund_carry_forward when surplus_action includes carry_forward. Bridge engine fires only the non-zero lines.',
  },
  {
    id: 'pre_fund_carry_forward',
    sourceTable: 'pre_fund_reconciliations',
    eventType: 'pre_fund_carry_forward',
    triggerStatus: 'closed',
    triggerCondition: "pre_fund_reconciliations.status = 'closed' AND surplus_action IN ('carry_forward','split')",
    ...
    lines: [
      {
        accountCode: '{gl_liability_account}',
        accountName: 'Pre-Fund Liability Current Period (resolved from fund.gl_liability_account)',
        debitCredit: 'DR',
        amountSource: 'pre_fund_reconciliations.carry_forward_amount',
        currency: 'fund.currency',
        glFunction: 'program',
        description: 'Dr current-period deferred liability to clear the closed period',
      },
      {
        accountCode: '{gl_cf_account}',
        accountName: 'Pre-Fund Liability Next Period (resolved from fund.gl_cf_account)',
        debitCredit: 'CR',
        amountSource: 'pre_fund_reconciliations.carry_forward_amount',
        currency: 'fund.currency',
        glFunction: 'program',
        description: 'Cr next-period deferred liability — surplus rolls forward to new cycle',
      },
    ],
    notes: 'Fires at period close when carry_forward_amount > 0. gl_cf_account defaults to 2401 (Pre-Fund Liability Next Period) but can be overridden per fund for multi-year grants.',
  },
];
