...
const defaultTemplates: EmailTemplate[] = [
  ...
  {
    id: 'task-assigned',
    name: 'Task Assigned / تم تعيين مهمة',
    subject: 'New Task Assigned: {{taskName}} / تم تعيين مهمة جديدة',
    category: 'tasks',
    isActive: true,
    variables: ['recipientName', 'taskName', 'assignedBy', 'dueDate', 'priority', 'actionUrl'],
    htmlContent: `<div dir="ltr"><h1>Task Assigned to You</h1><p>Hello {{recipientName}},</p><p>A new task has been assigned to you:</p><ul><li>Task: <strong>{{taskName}}</strong></li><li>Assigned by: {{assignedBy}}</li><li>Due: {{dueDate}}</li><li>Priority: {{priority}}</li></ul><p><a href="{{actionUrl}}">View Task</a></p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1>تم تعيين مهمة لك</h1><p>مرحباً {{recipientName}}،</p><p>تم تعيين مهمة جديدة لك:</p><ul><li>المهمة: <strong>{{taskName}}</strong></li><li>عين بواسطة: {{assignedBy}}</li><li>الموعد: {{dueDate}}</li><li>الأولوية: {{priority}}</li></ul><p><a href="{{actionUrl}}">عرض المهمة</a></p></div>`,
    textContent: 'Task Assigned to You\n\nHello {{recipientName}},\n\nTask: {{taskName}}\nAssigned by: {{assignedBy}}\nDue: {{dueDate}}\nPriority: {{priority}}\n\n---\n\nتم تعيين مهمة لك\n\nمرحباً {{recipientName}}،\n\nالمهمة: {{taskName}}\nعين بواسطة: {{assignedBy}}\nالموعد: {{dueDate}}',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'task-completed',
    name: 'Task Completed / اكتملت المهمة',
    subject: 'Task Completed: {{taskName}} / اكتملت المهمة',
    category: 'tasks',
    isActive: true,
    variables: ['recipientName', 'taskName', 'completedBy', 'completedDate', 'notes', 'actionUrl'],
    htmlContent: `<div dir="ltr"><h1>Task Completed</h1><p>Hello {{recipientName}},</p><p>A task has been marked as complete:</p><ul><li>Task: <strong>{{taskName}}</strong></li><li>Completed by: {{completedBy}}</li><li>Date: {{completedDate}}</li></ul><p>Notes: {{notes}}</p><p><a href="{{actionUrl}}">View Task</a></p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1>اكتملت المهمة</h1><p>مرحباً {{recipientName}}،</p><p>تم تمييز مهمة كمكتملة:</p><ul><li>المهمة: <strong>{{taskName}}</strong></li><li>أكملها: {{completedBy}}</li><li>التاريخ: {{completedDate}}</li></ul><p>ملاحظات: {{notes}}</p></div>`,
  ...
  {
    id: 'task-overdue',
    name: 'Task Overdue / المهمة متأخرة',
    subject: 'OVERDUE: {{taskName}} / المهمة متأخرة: {{taskName}}',
    category: 'tasks',
    isActive: true,
    variables: ['recipientName', 'taskName', 'dueDate', 'daysOverdue', 'assignedBy', 'actionUrl'],
    htmlContent: `<div dir="ltr"><h1 style="color:#dc2626;">Task Overdue — Action Required</h1><p>Hello {{recipientName}},</p><p>The following task is overdue:</p><ul><li>Task: <strong>{{taskName}}</strong></li><li>Was Due: {{dueDate}}</li><li>Days Overdue: {{daysOverdue}}</li><li>Assigned by: {{assignedBy}}</li></ul><p><a href="{{actionUrl}}">Update Task Now</a></p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1 style="color:#dc2626;">المهمة متأخرة — مطلوب إجراء</h1><p>مرحباً {{recipientName}}،</p><p>المهمة التالية متأخرة:</p><ul><li>المهمة: <strong>{{taskName}}</strong></li><li>كان موعدها: {{dueDate}}</li><li>أيام التأخير: {{daysOverdue}}</li></ul></div>`,
    textContent: 'Task Overdue — Action Required\n\nHello {{recipientName}},\n\nTask: {{taskName}}\nWas Due: {{dueDate}}\nDays Overdue: {{daysOverdue}}\n\n---\n\nالمهمة متأخرة\n\nمرحباً {{recipientName}}،\n\nالمهمة: {{taskName}}\nكان موعدها: {{dueDate}}',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'task-reminder-1day',
    name: 'Task Due Tomorrow / المهمة موعدها غداً',
    subject: 'Reminder: Task Due Tomorrow — {{taskName}} / تذكير: المهمة موعدها غداً',
    category: 'tasks',
    isActive: true,
    variables: ['recipientName', 'taskName', 'dueDate', 'priority', 'actionUrl'],
    htmlContent: `<div dir="ltr"><h1>Task Due Tomorrow</h1><p>Hello {{recipientName}},</p><p>This is a reminder that the following task is due tomorrow:</p><ul><li>Task: <strong>{{taskName}}</strong></li><li>Due: {{dueDate}}</li><li>Priority: {{priority}}</li></ul><p><a href="{{actionUrl}}">View Task</a></p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1>موعد المهمة غداً</h1><p>مرحباً {{recipientName}}،</p><p>هذا تذكير بأن موعد المهمة التالية غداً:</p><ul><li>المهمة: <strong>{{taskName}}</strong></li><li>الموعد: {{dueDate}}</li></ul></div>`,
    textContent: 'Task Due Tomorrow\n\nHello {{recipientName}},\n\nTask: {{taskName}} is due tomorrow ({{dueDate}}).\n\n---\n\nموعد المهمة غداً\n\nمرحباً {{recipientName}}،\n\nالمهمة: {{taskName}} موعدها غداً ({{dueDate}}).',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'task-comment-added',
    name: 'New Comment on Task / تعليق جديد على المهمة',
    subject: 'New Comment on: {{taskName}} / تعليق جديد على المهمة',
    category: 'tasks',
    isActive: true,
    variables: ['recipientName', 'taskName', 'commentBy', 'commentText', 'actionUrl'],
    htmlContent: `<div dir="ltr"><h1>New Comment on Task</h1><p>Hello {{recipientName}},</p><p><strong>{{commentBy}}</strong> commented on <strong>{{taskName}}</strong>:</p><blockquote style="border-left:3px solid #e5e7eb;padding-left:12px;color:#6b7280;">{{commentText}}</blockquote><p><a href="{{actionUrl}}">View & Reply</a></p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1>تعليق جديد على المهمة</h1><p>مرحباً {{recipientName}}،</p><p>علق <strong>{{commentBy}}</strong> على <strong>{{taskName}}</strong>:</p><blockquote style="border-right:3px solid #e5e7eb;padding-right:12px;color:#6b7280;">{{commentText}}</blockquote></div>`,
    textContent: 'New Comment on Task\n\nHello {{recipientName}},\n\n{{commentBy}} commented on {{taskName}}:\n\n"{{commentText}}"\n\n---\n\nتعليق جديد على المهمة\n\nمرحباً {{recipientName}}،\n\nعلق {{commentBy}} على {{taskName}}:\n\n"{{commentText}}"',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'task-rejected',
    name: 'Task Rejected / تم رفض المهمة',
    subject: 'Task Rejected: {{taskName}} / تم رفض المهمة',
    category: 'tasks',
    isActive: true,
    variables: ['recipientName', 'taskName', 'rejectedBy', 'reason', 'actionUrl'],
    htmlContent: `<div dir="ltr"><h1>Task Rejected</h1><p>Hello {{recipientName}},</p><p>Your task submission has been rejected:</p><ul><li>Task: <strong>{{taskName}}</strong></li><li>Rejected by: {{rejectedBy}}</li></ul><p>Reason: {{reason}}</p><p><a href="{{actionUrl}}">View Task</a></p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1>تم رفض المهمة</h1><p>مرحباً {{recipientName}}،</p><p>تم رفض تقديم المهمة:</p><ul><li>المهمة: <strong>{{taskName}}</strong></li><li>رفض بواسطة: {{rejectedBy}}</li></ul><p>السبب: {{reason}}</p></div>`,
    textContent: 'Task Rejected\n\nHello {{recipientName}},\n\nTask: {{taskName}}\nRejected by: {{rejectedBy}}\nReason: {{reason}}\n\n---\n\nتم رفض المهمة\n\nمرحباً {{recipientName}}،\n\nالمهمة: {{taskName}}\nرفض بواسطة: {{rejectedBy}}\nالسبب: {{reason}}',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  ...
  {
    id: 'dependency-added',
    name: 'Dependency Added / تمت إضافة اعتمادية',
    subject: 'New Dependency on Your Task: {{taskName}} / اعتمادية جديدة على مهمتك',
    category: 'tasks',
    isActive: true,
    variables: ['recipientName', 'taskName', 'dependsOnTask', 'addedBy', 'actionUrl'],
    htmlContent: `<div dir="ltr"><h1>New Dependency Added</h1><p>Hello {{recipientName}},</p><p>A dependency has been added to your task:</p><ul><li>Your Task: <strong>{{taskName}}</strong></li><li>Depends On: {{dependsOnTask}}</li><li>Added by: {{addedBy}}</li></ul><p>Your task is now blocked until <em>{{dependsOnTask}}</em> is completed.</p><p><a href="{{actionUrl}}">View Task</a></p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1>تمت إضافة اعتمادية جديدة</h1><p>مرحباً {{recipientName}}،</p><p>تمت إضافة اعتمادية لمهمتك:</p><ul><li>مهمتك: <strong>{{taskName}}</strong></li><li>تعتمد على: {{dependsOnTask}}</li><li>أضيفت بواسطة: {{addedBy}}</li></ul></div>`,
    textContent: 'New Dependency Added\n\nHello {{recipientName}},\n\nYour task {{taskName}} now depends on {{dependsOnTask}}.\n\nAdded by: {{addedBy}}\n\n---\n\nتمت إضافة اعتمادية\n\nمرحباً {{recipientName}}،\n\nمهمتك {{taskName}} تعتمد الآن على {{dependsOnTask}}.',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'dependency-resolved',
    name: 'Dependency Resolved / تم حل الاعتمادية',
    subject: 'Dependency Resolved — You Can Proceed: {{taskName}} / تم حل الاعتمادية — يمكنك المتابعة',
    category: 'tasks',
    isActive: true,
    variables: ['recipientName', 'taskName', 'resolvedTask', 'actionUrl'],
    htmlContent: `<div dir="ltr"><h1 style="color:#059669;">Dependency Resolved — You Can Proceed!</h1><p>Hello {{recipientName}},</p><p>Great news! A dependency on your task has been resolved:</p><ul><li>Your Task: <strong>{{taskName}}</strong></li><li>Resolved: {{resolvedTask}}</li></ul><p>You can now work on your task.</p><p><a href="{{actionUrl}}">Go to Task</a></p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1 style="color:#059669;">تم حل الاعتمادية — يمكنك المتابعة!</h1><p>مرحباً {{recipientName}}،</p><p>تم حل اعتمادية مهمتك:</p><ul><li>مهمتك: <strong>{{taskName}}</strong></li><li>تم حل: {{resolvedTask}}</li></ul></div>`,
    textContent: 'Dependency Resolved — You Can Proceed!\n\nHello {{recipientName}},\n\nThe dependency on {{taskName}} ({{resolvedTask}}) has been resolved. You can now proceed.\n\n---\n\nتم حل الاعتمادية\n\nمرحباً {{recipientName}}،\n\nتم حل اعتمادية مهمتك {{taskName}}.',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  ...
  // === LEAVE & HR ===
  {
    ...
    name: 'Leave Request Submitted / طلب إجازة مقدم',
    subject: 'New Leave Request from {{requesterName}} / طلب إجازة جديد',
    category: 'hr',
    isActive: true,
    variables: ['recipientName', 'requesterName', 'leaveType', 'startDate', 'endDate', 'days', 'reason', 'actionUrl'],
    htmlContent: `<div dir="ltr"><h1>Leave Request for Approval</h1><p>Hello {{recipientName}},</p><p><strong>{{requesterName}}</strong> has submitted a leave request:</p><ul><li>Type: {{leaveType}}</li><li>From: {{startDate}}</li><li>To: {{endDate}}</li><li>Days: {{days}}</li><li>Reason: {{reason}}</li></ul><p><a href="{{actionUrl}}">Review & Approve</a></p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1>طلب إجازة للموافقة</h1><p>مرحباً {{recipientName}}،</p><p>قدم <strong>{{requesterName}}</strong> طلب إجازة:</p><ul><li>النوع: {{leaveType}}</li><li>من: {{startDate}}</li><li>إلى: {{endDate}}</li><li>الأيام: {{days}}</li><li>السبب: {{reason}}</li></ul><p><a href="{{actionUrl}}">مراجعة والموافقة</a></p></div>`,
    textContent: 'Leave Request for Approval\n\nHello {{recipientName}},\n\n{{requesterName}} has submitted a {{leaveType}} leave request from {{startDate}} to {{endDate}} ({{days}} days).\n\nReason: {{reason}}\n\n---\n\nطلب إجازة\n\nمرحباً {{recipientName}}،\n\nقدم {{requesterName}} طلب إجازة {{leaveType}} من {{startDate}} إلى {{endDate}} ({{days}} أيام).',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'leave-request-approved',
    name: 'Leave Request Approved / تمت الموافقة على الإجازة',
    subject: 'Leave Request Approved — {{leaveType}} / تمت الموافقة على طلب الإجازة',
    category: 'hr',
    isActive: true,
    variables: ['recipientName', 'leaveType', 'startDate', 'endDate', 'days', 'approvedBy', 'comments'],
    htmlContent: `<div dir="ltr"><h1 style="color:#059669;">Leave Request Approved</h1><p>Hello {{recipientName}},</p><p>Your leave request has been approved:</p><ul><li>Type: {{leaveType}}</li><li>From: {{startDate}}</li><li>To: {{endDate}}</li><li>Days: {{days}}</li><li>Approved by: {{approvedBy}}</li></ul><p>Comments: {{comments}}</p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1 style="color:#059669;">تمت الموافقة على طلب الإجازة</h1><p>مرحباً {{recipientName}}،</p><p>تمت الموافقة على طلب إجازتك:</p><ul><li>النوع: {{leaveType}}</li><li>من: {{startDate}}</li><li>إلى: {{endDate}}</li><li>الأيام: {{days}}</li><li>وافق عليها: {{approvedBy}}</li></ul></div>`,
    textContent: 'Leave Request Approved\n\nHello {{recipientName}},\n\nYour {{leaveType}} leave ({{startDate}} – {{endDate}}, {{days}} days) has been approved by {{approvedBy}}.\n\n---\n\nتمت الموافقة على الإجازة\n\nمرحباً {{recipientName}}،\n\nتمت الموافقة على إجازتك {{leaveType}}.',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'leave-request-rejected',
    name: 'Leave Request Rejected / رفض طلب الإجازة',
    subject: 'Leave Request Not Approved — {{leaveType}} / لم تتم الموافقة على طلب الإجازة',
    category: 'hr',
    isActive: true,
    variables: ['recipientName', 'leaveType', 'startDate', 'endDate', 'rejectedBy', 'reason'],
    htmlContent: `<div dir="ltr"><h1>Leave Request Not Approved</h1><p>Hello {{recipientName}},</p><p>Your leave request has not been approved:</p><ul><li>Type: {{leaveType}}</li><li>Dates: {{startDate}} – {{endDate}}</li><li>Reviewed by: {{rejectedBy}}</li></ul><p>Reason: {{reason}}</p><p>Please discuss with your manager if you wish to resubmit.</p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1>لم تتم الموافقة على طلب الإجازة</h1><p>مرحباً {{recipientName}}،</p><p>لم تتم الموافقة على طلب إجازتك:</p><ul><li>النوع: {{leaveType}}</li><li>التواريخ: {{startDate}} – {{endDate}}</li><li>راجعها: {{rejectedBy}}</li></ul><p>السبب: {{reason}}</p></div>`,
    textContent: 'Leave Request Not Approved\n\nHello {{recipientName}},\n\nYour {{leaveType}} leave request has not been approved.\n\nReason: {{reason}}\n\n---\n\nلم تتم الموافقة على طلب الإجازة\n\nمرحباً {{recipientName}}،\n\nالسبب: {{reason}}',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'leave-balance-updated',
    name: 'Leave Balance Updated / تحديث رصيد الإجازات',
    subject: 'Your Leave Balance Has Been Updated / تم تحديث رصيد إجازاتك',
    category: 'hr',
    isActive: true,
    variables: ['recipientName', 'annualBalance', 'usedDays', 'remainingDays', 'effectiveDate'],
    htmlContent: `<div dir="ltr"><h1>Leave Balance Updated</h1><p>Hello {{recipientName}},</p><p>Your leave balance has been updated as of {{effectiveDate}}:</p><ul><li>Annual Entitlement: {{annualBalance}} days</li><li>Used: {{usedDays}} days</li><li>Remaining: <strong>{{remainingDays}} days</strong></li></ul></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1>تحديث رصيد الإجازات</h1><p>مرحباً {{recipientName}}،</p><p>تم تحديث رصيد إجازاتك اعتباراً من {{effectiveDate}}:</p><ul><li>الاستحقاق السنوي: {{annualBalance}} يوم</li><li>المستخدم: {{usedDays}} يوم</li><li>المتبقي: <strong>{{remainingDays}} يوم</strong></li></ul></div>`,
    textContent: 'Leave Balance Updated\n\nHello {{recipientName}},\n\nAs of {{effectiveDate}}:\nAnnual: {{annualBalance}} days\nUsed: {{usedDays}} days\nRemaining: {{remainingDays}} days\n\n---\n\nتحديث رصيد الإجازات\n\nمرحباً {{recipientName}}،\n\nالمتبقي: {{remainingDays}} يوم',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },

  // === PAYROLL & FINANCIAL (EXTENDED) ===
  {
    id: 'payroll-approval-needed',
    name: 'Payroll Approval Needed / الموافقة على كشف الرواتب',
    subject: 'Action Required: Payroll Approval — {{month}} / مطلوب إجراء: الموافقة على الرواتب',
    category: 'hr',
    isActive: true,
    variables: ['recipientName', 'month', 'totalAmount', 'staffCount', 'actionUrl'],
    htmlContent: `<div dir="ltr"><h1>Payroll Approval Required</h1><p>Hello {{recipientName}},</p><p>The payroll for <strong>{{month}}</strong> is ready for your approval:</p><ul><li>Total Amount: {{totalAmount}} SDG</li><li>Staff Count: {{staffCount}}</li></ul><p><a href="{{actionUrl}}">Review & Approve Payroll</a></p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1>الموافقة على كشف الرواتب مطلوبة</h1><p>مرحباً {{recipientName}}،</p><p>كشف رواتب <strong>{{month}}</strong> جاهز لموافقتك:</p><ul><li>المبلغ الإجمالي: {{totalAmount}} جنيه سوداني</li><li>عدد الموظفين: {{staffCount}}</li></ul><p><a href="{{actionUrl}}">مراجعة والموافقة على الرواتب</a></p></div>`,
    textContent: 'Payroll Approval Required\n\nHello {{recipientName}},\n\nPayroll for {{month}} is ready for approval.\n\nTotal: {{totalAmount}} SDG\nStaff: {{staffCount}}\n\n---\n\nالموافقة على كشف الرواتب\n\nمرحباً {{recipientName}}،\n\nكشف رواتب {{month}} جاهز لموافقتك.',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'payroll-slip-ready',
    name: 'Payslip Ready / بيان الراتب جاهز',
    subject: 'Your Payslip for {{month}} is Ready / بيان راتبك لـ {{month}} جاهز',
    category: 'hr',
    isActive: true,
    variables: ['recipientName', 'month', 'netSalary', 'payDate', 'actionUrl'],
    htmlContent: `<div dir="ltr"><h1>Your Payslip is Ready</h1><p>Hello {{recipientName}},</p><p>Your payslip for <strong>{{month}}</strong> is now available:</p><ul><li>Net Salary: <strong>{{netSalary}} SDG</strong></li><li>Payment Date: {{payDate}}</li></ul><p><a href="{{actionUrl}}">View & Download Payslip</a></p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1>بيان راتبك جاهز</h1><p>مرحباً {{recipientName}}،</p><p>بيان راتبك لشهر <strong>{{month}}</strong> متاح الآن:</p><ul><li>صافي الراتب: <strong>{{netSalary}} جنيه سوداني</strong></li><li>تاريخ الدفع: {{payDate}}</li></ul><p><a href="{{actionUrl}}">عرض وتحميل بيان الراتب</a></p></div>`,
    textContent: 'Your Payslip is Ready\n\nHello {{recipientName}},\n\nPayslip for {{month}}\nNet Salary: {{netSalary}} SDG\nPayment Date: {{payDate}}\n\n---\n\nبيان راتبك جاهز\n\nمرحباً {{recipientName}}،\n\nصافي الراتب: {{netSalary}} جنيه سوداني\nتاريخ الدفع: {{payDate}}',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'advance-request-submitted',
    name: 'Salary Advance Request / طلب سلفة راتب',
    subject: 'Salary Advance Request from {{requesterName}} / طلب سلفة راتب',
    category: 'hr',
    isActive: true,
    variables: ['recipientName', 'requesterName', 'amount', 'reason', 'actionUrl'],
    htmlContent: `<div dir="ltr"><h1>Salary Advance Request</h1><p>Hello {{recipientName}},</p><p><strong>{{requesterName}}</strong> has requested a salary advance:</p><ul><li>Amount: {{amount}} SDG</li><li>Reason: {{reason}}</li></ul><p><a href="{{actionUrl}}">Review & Approve</a></p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1>طلب سلفة راتب</h1><p>مرحباً {{recipientName}}،</p><p>طلب <strong>{{requesterName}}</strong> سلفة راتب:</p><ul><li>المبلغ: {{amount}} جنيه سوداني</li><li>السبب: {{reason}}</li></ul><p><a href="{{actionUrl}}">مراجعة والموافقة</a></p></div>`,
    textContent: 'Salary Advance Request\n\nHello {{recipientName}},\n\n{{requesterName}} has requested a salary advance of {{amount}} SDG.\n\nReason: {{reason}}\n\n---\n\nطلب سلفة راتب\n\nمرحباً {{recipientName}}،\n\nطلب {{requesterName}} سلفة راتب بقيمة {{amount}} جنيه سوداني.',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'advance-request-approved',
    name: 'Salary Advance Approved / تمت الموافقة على السلفة',
    subject: 'Salary Advance Approved — {{amount}} SDG / تمت الموافقة على سلفة الراتب',
    category: 'hr',
    isActive: true,
    variables: ['recipientName', 'amount', 'approvedBy', 'recoverySchedule'],
    htmlContent: `<div dir="ltr"><h1 style="color:#059669;">Salary Advance Approved</h1><p>Hello {{recipientName}},</p><p>Your salary advance request has been approved:</p><ul><li>Amount: <strong>{{amount}} SDG</strong></li><li>Approved by: {{approvedBy}}</li><li>Recovery: {{recoverySchedule}}</li></ul></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1 style="color:#059669;">تمت الموافقة على السلفة</h1><p>مرحباً {{recipientName}}،</p><p>تمت الموافقة على طلب سلفة راتبك:</p><ul><li>المبلغ: <strong>{{amount}} جنيه سوداني</strong></li><li>وافق عليها: {{approvedBy}}</li><li>جدول الاسترداد: {{recoverySchedule}}</li></ul></div>`,
    textContent: 'Salary Advance Approved\n\nHello {{recipientName}},\n\nYour salary advance of {{amount}} SDG has been approved by {{approvedBy}}.\n\nRecovery: {{recoverySchedule}}\n\n---\n\nتمت الموافقة على السلفة\n\nمرحباً {{recipientName}}،\n\nتمت الموافقة على سلفتك بقيمة {{amount}} جنيه سوداني.',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  ...
  // === HR CONTRACTS ===
  {
    id: 'contract-expiring-30d',
    name: 'Contract Expiring — 30 Days / العقد ينتهي خلال 30 يوم',
    subject: 'Contract Expiry Notice — 30 Days: {{staffName}} / إشعار انتهاء العقد — 30 يوم',
    category: 'hr',
    isActive: true,
    variables: ['recipientName', 'staffName', 'expiryDate', 'contractType', 'actionUrl'],
    htmlContent: `<div dir="ltr"><h1>Contract Expiry Notice — 30 Days</h1><p>Hello {{recipientName}},</p><p>The following contract expires in <strong>30 days</strong>:</p><ul><li>Staff: {{staffName}}</li><li>Contract Type: {{contractType}}</li><li>Expiry Date: {{expiryDate}}</li></ul><p>Please take action to renew or close the contract.</p><p><a href="{{actionUrl}}">Manage Contract</a></p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1>إشعار انتهاء العقد — 30 يوم</h1><p>مرحباً {{recipientName}}،</p><p>ينتهي العقد التالي خلال <strong>30 يوماً</strong>:</p><ul><li>الموظف: {{staffName}}</li><li>نوع العقد: {{contractType}}</li><li>تاريخ الانتهاء: {{expiryDate}}</li></ul></div>`,
    textContent: 'Contract Expiry Notice — 30 Days\n\nHello {{recipientName}},\n\nStaff: {{staffName}}\nContract: {{contractType}}\nExpiry: {{expiryDate}}\n\n---\n\nإشعار انتهاء العقد\n\nمرحباً {{recipientName}}،\n\nينتهي عقد {{staffName}} خلال 30 يوماً في {{expiryDate}}.',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'contract-expiring-7d',
    name: 'Contract Expiring — 7 Days / العقد ينتهي خلال 7 أيام',
    subject: 'URGENT: Contract Expires in 7 Days — {{staffName}} / عاجل: العقد ينتهي خلال 7 أيام',
    category: 'hr',
    isActive: true,
    variables: ['recipientName', 'staffName', 'expiryDate', 'contractType', 'actionUrl'],
    htmlContent: `<div dir="ltr"><h1 style="color:#d97706;">URGENT: Contract Expires in 7 Days</h1><p>Hello {{recipientName}},</p><p>The following contract expires in <strong>7 days</strong> — immediate action required:</p><ul><li>Staff: {{staffName}}</li><li>Contract Type: {{contractType}}</li><li>Expiry Date: {{expiryDate}}</li></ul><p><a href="{{actionUrl}}">Renew Contract Now</a></p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1 style="color:#d97706;">عاجل: العقد ينتهي خلال 7 أيام</h1><p>مرحباً {{recipientName}}،</p><p>ينتهي العقد التالي خلال <strong>7 أيام</strong> — مطلوب إجراء فوري:</p><ul><li>الموظف: {{staffName}}</li><li>تاريخ الانتهاء: {{expiryDate}}</li></ul></div>`,
    textContent: 'URGENT: Contract Expires in 7 Days\n\nHello {{recipientName}},\n\nStaff: {{staffName}}\nExpiry: {{expiryDate}}\n\nImmediate action required!\n\n---\n\nعاجل: العقد ينتهي خلال 7 أيام\n\nمرحباً {{recipientName}}،\n\nالموظف: {{staffName}}\nتاريخ الانتهاء: {{expiryDate}}',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'contract-expired',
    name: 'Contract Expired / انتهى العقد',
    subject: 'Contract Expired: {{staffName}} / انتهى العقد',
    category: 'hr',
    isActive: true,
    variables: ['recipientName', 'staffName', 'expiryDate', 'contractType', 'actionUrl'],
    htmlContent: `<div dir="ltr"><h1 style="color:#dc2626;">Contract Expired</h1><p>Hello {{recipientName}},</p><p>The following contract has expired:</p><ul><li>Staff: {{staffName}}</li><li>Contract Type: {{contractType}}</li><li>Expired: {{expiryDate}}</li></ul><p>Please update the staff status and process end-of-service accordingly.</p><p><a href="{{actionUrl}}">Manage Staff Record</a></p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1 style="color:#dc2626;">انتهى العقد</h1><p>مرحباً {{recipientName}}،</p><p>انتهى العقد التالي:</p><ul><li>الموظف: {{staffName}}</li><li>تاريخ الانتهاء: {{expiryDate}}</li></ul><p>يرجى تحديث حالة الموظف ومعالجة نهاية الخدمة.</p></div>`,
    textContent: 'Contract Expired\n\nHello {{recipientName}},\n\nStaff: {{staffName}}\nExpired: {{expiryDate}}\n\nPlease update the staff status.\n\n---\n\nانتهى العقد\n\nمرحباً {{recipientName}}،\n\nالموظف: {{staffName}}\nانتهى في: {{expiryDate}}',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  ...
  // === TIMESHEET ===
  {
    ...
    name: 'Timesheet Submitted / تقديم كشف الدوام',
    subject: 'Timesheet Submitted for Approval — {{staffName}} / تقديم كشف الدوام للاعتماد',
    category: 'hr',
    isActive: true,
    variables: ['recipientName', 'staffName', 'periodStart', 'periodEnd', 'totalHours', 'actionUrl'],
    htmlContent: `<div dir="ltr"><h1>Timesheet Submitted for Approval</h1><p>Hello {{recipientName}},</p><p><strong>{{staffName}}</strong> has submitted their timesheet:</p><ul><li>Period: {{periodStart}} – {{periodEnd}}</li><li>Total Hours: {{totalHours}}</li></ul><p><a href="{{actionUrl}}">Review & Approve</a></p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1>تم تقديم كشف الدوام للاعتماد</h1><p>مرحباً {{recipientName}}،</p><p>قدم <strong>{{staffName}}</strong> كشف الدوام:</p><ul><li>الفترة: {{periodStart}} – {{periodEnd}}</li><li>إجمالي الساعات: {{totalHours}}</li></ul><p><a href="{{actionUrl}}">مراجعة واعتماد</a></p></div>`,
    textContent: 'Timesheet Submitted\n\nHello {{recipientName}},\n\n{{staffName}} submitted their timesheet for {{periodStart}} – {{periodEnd}} ({{totalHours}} hours).\n\n---\n\nتقديم كشف الدوام\n\nمرحباً {{recipientName}}،\n\nقدم {{staffName}} كشف الدوام.',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'timesheet-approved',
    name: 'Timesheet Approved / اعتماد كشف الدوام',
    subject: 'Your Timesheet Has Been Approved / تمت الموافقة على كشف الدوام',
    category: 'hr',
    isActive: true,
    variables: ['recipientName', 'periodStart', 'periodEnd', 'totalHours', 'approvedBy'],
    htmlContent: `<div dir="ltr"><h1 style="color:#059669;">Timesheet Approved</h1><p>Hello {{recipientName}},</p><p>Your timesheet has been approved:</p><ul><li>Period: {{periodStart}} – {{periodEnd}}</li><li>Total Hours: {{totalHours}}</li><li>Approved by: {{approvedBy}}</li></ul></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1 style="color:#059669;">تمت الموافقة على كشف الدوام</h1><p>مرحباً {{recipientName}}،</p><p>تمت الموافقة على كشف الدوام الخاص بك:</p><ul><li>الفترة: {{periodStart}} – {{periodEnd}}</li><li>إجمالي الساعات: {{totalHours}}</li><li>اعتمده: {{approvedBy}}</li></ul></div>`,
    textContent: 'Timesheet Approved\n\nHello {{recipientName}},\n\nYour timesheet for {{periodStart}} – {{periodEnd}} ({{totalHours}} hours) has been approved by {{approvedBy}}.\n\n---\n\nاعتماد كشف الدوام\n\nمرحباً {{recipientName}}،\n\nتمت الموافقة على كشف الدوام الخاص بك.',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'timesheet-rejected',
    name: 'Timesheet Not Approved / رفض كشف الدوام',
    subject: 'Timesheet Revision Required / مطلوب مراجعة كشف الدوام',
    category: 'hr',
    isActive: true,
    variables: ['recipientName', 'periodStart', 'periodEnd', 'rejectedBy', 'reason', 'actionUrl'],
    htmlContent: `<div dir="ltr"><h1>Timesheet Revision Required</h1><p>Hello {{recipientName}},</p><p>Your timesheet requires revision:</p><ul><li>Period: {{periodStart}} – {{periodEnd}}</li><li>Reviewed by: {{rejectedBy}}</li></ul><p>Reason: {{reason}}</p><p><a href="{{actionUrl}}">Revise & Resubmit</a></p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1>مطلوب مراجعة كشف الدوام</h1><p>مرحباً {{recipientName}}،</p><p>يتطلب كشف الدوام الخاص بك مراجعة:</p><ul><li>الفترة: {{periodStart}} – {{periodEnd}}</li><li>راجعه: {{rejectedBy}}</li></ul><p>السبب: {{reason}}</p></div>`,
    textContent: 'Timesheet Revision Required\n\nHello {{recipientName}},\n\nYour timesheet for {{periodStart}} – {{periodEnd}} requires revision.\n\nReason: {{reason}}\n\n---\n\nمطلوب مراجعة كشف الدوام\n\nمرحباً {{recipientName}}،\n\nالسبب: {{reason}}',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  ...
  // === FINANCIAL (EXTENDED) ===
  {
    id: 'cost-action-required',
    name: 'Cost Submission — Action Required / مطلوب إجراء على طلب التكلفة',
    subject: 'Action Required: Cost Review — {{refNumber}} / مطلوب إجراء: مراجعة التكلفة',
    category: 'financial',
    isActive: true,
    variables: ['recipientName', 'refNumber', 'amount', 'currency', 'submitterName', 'tier', 'actionUrl'],
    htmlContent: `<div dir="ltr"><h1>Action Required: Cost Submission Review</h1><p>Hello {{recipientName}},</p><p>A cost submission requires your review and approval:</p><ul><li>Reference: <strong>{{refNumber}}</strong></li><li>Amount: {{amount}} {{currency}}</li><li>Submitted by: {{submitterName}}</li><li>Approval Tier: Tier {{tier}}</li></ul><p><a href="{{actionUrl}}" style="background:#d97706;color:#fff;padding:10px 20px;border-radius:6px;text-decoration:none;display:inline-block;">Review &amp; Approve →</a></p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1>مطلوب إجراء: مراجعة طلب التكلفة</h1><p>مرحباً {{recipientName}}،</p><p>طلب التكلفة يتطلب مراجعتك وموافقتك:</p><ul><li>الرقم المرجعي: <strong>{{refNumber}}</strong></li><li>المبلغ: {{amount}} {{currency}}</li><li>مقدم من: {{submitterName}}</li><li>مستوى الموافقة: المستوى {{tier}}</li></ul></div>`,
    textContent: 'Action Required: Cost Review\n\nHello {{recipientName}},\n\nRef: {{refNumber}}\nAmount: {{amount}} {{currency}}\nSubmitted by: {{submitterName}}\nTier: {{tier}}\n\n---\n\nمطلوب إجراء: مراجعة التكلفة\n\nمرحباً {{recipientName}}،\n\nالمرجع: {{refNumber}}\nالمبلغ: {{amount}} {{currency}}',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'withdrawal-approved',
    name: 'Withdrawal Approved / تمت الموافقة على السحب',
    subject: 'Withdrawal Request Approved — {{amount}} SDG / تمت الموافقة على طلب السحب',
    category: 'financial',
    isActive: true,
    variables: ['recipientName', 'amount', 'currency', 'approvedBy', 'transactionId', 'processedDate'],
    htmlContent: `<div dir="ltr"><h1 style="color:#059669;">Withdrawal Approved</h1><p>Hello {{recipientName}},</p><p>Your withdrawal request has been approved and processed:</p><ul><li>Amount: <strong>{{amount}} {{currency}}</strong></li><li>Approved by: {{approvedBy}}</li><li>Transaction ID: {{transactionId}}</li><li>Date: {{processedDate}}</li></ul></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1 style="color:#059669;">تمت الموافقة على السحب</h1><p>مرحباً {{recipientName}}،</p><p>تمت الموافقة على طلب السحب ومعالجته:</p><ul><li>المبلغ: <strong>{{amount}} {{currency}}</strong></li><li>وافق عليه: {{approvedBy}}</li><li>رقم المعاملة: {{transactionId}}</li></ul></div>`,
    textContent: 'Withdrawal Approved\n\nHello {{recipientName}},\n\nAmount: {{amount}} {{currency}}\nApproved by: {{approvedBy}}\nTransaction: {{transactionId}}\n\n---\n\nتمت الموافقة على السحب\n\nمرحباً {{recipientName}}،\n\nالمبلغ: {{amount}} {{currency}}',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'budget-threshold-80',
    name: 'Budget Alert: 80% Used / تنبيه: 80% من الميزانية',
    subject: 'Budget Alert: 80% Utilized — {{projectName}} / تنبيه الميزانية: 80٪ مستخدمة',
    category: 'financial',
    isActive: true,
    variables: ['recipientName', 'projectName', 'totalBudget', 'usedAmount', 'remaining', 'actionUrl'],
    htmlContent: `<div dir="ltr"><h1 style="color:#f59e0b;">Budget Alert: 80% Utilized</h1><p>Hello {{recipientName}},</p><p>The budget for <strong>{{projectName}}</strong> has reached 80% utilization:</p><ul><li>Total Budget: {{totalBudget}} SDG</li><li>Used: {{usedAmount}} SDG</li><li>Remaining: <strong>{{remaining}} SDG</strong></li></ul><p>Please review spending and adjust plans as needed.</p><p><a href="{{actionUrl}}">View Budget</a></p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1 style="color:#f59e0b;">تنبيه الميزانية: 80٪ مستخدمة</h1><p>مرحباً {{recipientName}}،</p><p>وصلت ميزانية <strong>{{projectName}}</strong> إلى 80٪ من الاستخدام:</p><ul><li>الميزانية الإجمالية: {{totalBudget}} جنيه سوداني</li><li>المتبقي: <strong>{{remaining}} جنيه سوداني</strong></li></ul></div>`,
    textContent: 'Budget Alert: 80% Utilized\n\nHello {{recipientName}},\n\nProject: {{projectName}}\nTotal: {{totalBudget}} SDG\nUsed: {{usedAmount}} SDG\nRemaining: {{remaining}} SDG\n\n---\n\nتنبيه الميزانية\n\nمرحباً {{recipientName}}،\n\nالمشروع: {{projectName}}\nالمتبقي: {{remaining}} جنيه سوداني',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'budget-threshold-100',
    name: 'Budget Alert: Fully Utilized / تنبيه: الميزانية مستنفدة',
    subject: 'URGENT: Budget Fully Utilized — {{projectName}} / عاجل: الميزانية مستنفدة',
    category: 'financial',
    isActive: true,
    variables: ['recipientName', 'projectName', 'totalBudget', 'actionUrl'],
    htmlContent: `<div dir="ltr"><h1 style="color:#dc2626;">URGENT: Budget Fully Utilized</h1><p>Hello {{recipientName}},</p><p>The budget for <strong>{{projectName}}</strong> has been <strong>fully utilized</strong>.</p><ul><li>Total Budget: {{totalBudget}} SDG</li><li>Remaining: 0 SDG</li></ul><p>No further expenditures can be processed without budget amendment. Immediate action required.</p><p><a href="{{actionUrl}}">Review Budget Now</a></p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1 style="color:#dc2626;">عاجل: الميزانية مستنفدة</h1><p>مرحباً {{recipientName}}،</p><p>تم استنفاد ميزانية <strong>{{projectName}}</strong> بالكامل.</p><ul><li>الميزانية الإجمالية: {{totalBudget}} جنيه سوداني</li><li>المتبقي: 0</li></ul></div>`,
    textContent: 'URGENT: Budget Fully Utilized\n\nHello {{recipientName}},\n\nProject: {{projectName}}\nBudget: {{totalBudget}} SDG — FULLY USED\n\nNo further spending without budget amendment!\n\n---\n\nعاجل: الميزانية مستنفدة\n\nمرحباً {{recipientName}}،\n\nالمشروع: {{projectName}}\nالميزانية: {{totalBudget}} جنيه سوداني — مستنفدة بالكامل',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  ...
  {
    id: 'mmp-forwarded',
    name: 'MMP Forwarded for Approval / تحويل خطة المراقبة للموافقة',
    subject: 'MMP Forwarded for Approval — {{projectName}} / تحويل خطة المراقبة الشهرية للموافقة',
    category: 'projects',
    isActive: true,
    variables: ['recipientName', 'projectName', 'forwardedBy', 'cycleMonth', 'actionUrl'],
    htmlContent: `<div dir="ltr"><h1>MMP Forwarded for Your Approval</h1><p>Hello {{recipientName}},</p><p>A Monthly Monitoring Plan has been forwarded for your approval:</p><ul><li>Project: {{projectName}}</li><li>Cycle: {{cycleMonth}}</li><li>Forwarded by: {{forwardedBy}}</li></ul><p><a href="{{actionUrl}}">Review & Approve</a></p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1>تحويل خطة المراقبة الشهرية لموافقتك</h1><p>مرحباً {{recipientName}}،</p><p>تم تحويل خطة مراقبة شهرية لموافقتك:</p><ul><li>المشروع: {{projectName}}</li><li>الدورة: {{cycleMonth}}</li><li>أحيلت بواسطة: {{forwardedBy}}</li></ul></div>`,
    textContent: 'MMP Forwarded for Approval\n\nHello {{recipientName}},\n\nProject: {{projectName}}\nCycle: {{cycleMonth}}\nForwarded by: {{forwardedBy}}\n\n---\n\nتحويل خطة المراقبة للموافقة\n\nمرحباً {{recipientName}}،\n\nالمشروع: {{projectName}}\nالدورة: {{cycleMonth}}',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'mmp-cycle-closed',
    name: 'MMP Cycle Closed / إغلاق دورة المراقبة',
    subject: 'MMP Cycle Closed — {{cycleMonth}} / تم إغلاق دورة خطة المراقبة الشهرية',
    category: 'projects',
    isActive: true,
    variables: ['recipientName', 'projectName', 'cycleMonth', 'sitesVisited', 'totalSites', 'closedBy'],
    htmlContent: `<div dir="ltr"><h1 style="color:#059669;">MMP Cycle Closed</h1><p>Hello {{recipientName}},</p><p>The following MMP cycle has been officially closed:</p><ul><li>Project: {{projectName}}</li><li>Cycle: {{cycleMonth}}</li><li>Sites Visited: {{sitesVisited}} / {{totalSites}}</li><li>Closed by: {{closedBy}}</li></ul></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1 style="color:#059669;">تم إغلاق دورة خطة المراقبة</h1><p>مرحباً {{recipientName}}،</p><p>تم إغلاق دورة المراقبة الشهرية التالية رسمياً:</p><ul><li>المشروع: {{projectName}}</li><li>الدورة: {{cycleMonth}}</li><li>المواقع المزارة: {{sitesVisited}} / {{totalSites}}</li></ul></div>`,
    textContent: 'MMP Cycle Closed\n\nHello {{recipientName}},\n\nProject: {{projectName}}\nCycle: {{cycleMonth}}\nSites: {{sitesVisited}}/{{totalSites}}\nClosed by: {{closedBy}}\n\n---\n\nإغلاق دورة المراقبة\n\nمرحباً {{recipientName}}،\n\nتم إغلاق دورة {{cycleMonth}} للمشروع {{projectName}}.',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  ...
  // === PROJECTS (EXTENDED) ===
  {
    id: 'project-stage-advanced',
    name: 'Project Stage Advanced / تقدمت مرحلة المشروع',
    subject: 'Project Stage Advanced — {{projectName}} / تقدمت مرحلة المشروع',
    category: 'projects',
    isActive: true,
    variables: ['recipientName', 'projectName', 'previousStage', 'currentStage', 'advancedBy', 'actionUrl'],
    htmlContent: `<div dir="ltr"><h1>Project Stage Advanced</h1><p>Hello {{recipientName}},</p><p>A project has advanced to the next stage:</p><ul><li>Project: <strong>{{projectName}}</strong></li><li>Previous Stage: {{previousStage}}</li><li>Current Stage: <strong>{{currentStage}}</strong></li><li>Advanced by: {{advancedBy}}</li></ul><p><a href="{{actionUrl}}">View Project</a></p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1>تقدمت مرحلة المشروع</h1><p>مرحباً {{recipientName}}،</p><p>تقدم المشروع إلى المرحلة التالية:</p><ul><li>المشروع: <strong>{{projectName}}</strong></li><li>المرحلة الحالية: <strong>{{currentStage}}</strong></li></ul></div>`,
    textContent: 'Project Stage Advanced\n\nHello {{recipientName}},\n\nProject: {{projectName}}\nFrom: {{previousStage}} → To: {{currentStage}}\nAdvanced by: {{advancedBy}}\n\n---\n\nتقدمت مرحلة المشروع\n\nمرحباً {{recipientName}}،\n\nالمشروع: {{projectName}}\nالمرحلة الحالية: {{currentStage}}',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'project-stage-assigned',
    name: 'Assigned to Project Stage / تعيين في مرحلة مشروع',
    subject: 'You Have Been Assigned to a Project Stage — {{projectName}} / تم تعيينك في مرحلة مشروع',
    category: 'projects',
    isActive: true,
    variables: ['recipientName', 'projectName', 'stageName', 'assignedBy', 'dueDate', 'actionUrl'],
    htmlContent: `<div dir="ltr"><h1>Assigned to Project Stage</h1><p>Hello {{recipientName}},</p><p>You have been assigned to a stage in the following project:</p><ul><li>Project: <strong>{{projectName}}</strong></li><li>Stage: <strong>{{stageName}}</strong></li><li>Assigned by: {{assignedBy}}</li><li>Due: {{dueDate}}</li></ul><p><a href="{{actionUrl}}">View Project</a></p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1>تم تعيينك في مرحلة مشروع</h1><p>مرحباً {{recipientName}}،</p><p>تم تعيينك في مرحلة من المشروع:</p><ul><li>المشروع: <strong>{{projectName}}</strong></li><li>المرحلة: <strong>{{stageName}}</strong></li><li>الموعد: {{dueDate}}</li></ul></div>`,
    textContent: 'Assigned to Project Stage\n\nHello {{recipientName}},\n\nProject: {{projectName}}\nStage: {{stageName}}\nAssigned by: {{assignedBy}}\nDue: {{dueDate}}\n\n---\n\nتعيين في مرحلة مشروع\n\nمرحباً {{recipientName}}،\n\nالمشروع: {{projectName}}\nالمرحلة: {{stageName}}',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'project-milestone-overdue',
    name: 'Project Milestone Overdue / تأخر إنجاز المرحلة',
    subject: 'Milestone Overdue — {{projectName}} / تأخر إنجاز المرحلة',
    category: 'projects',
    isActive: true,
    variables: ['recipientName', 'projectName', 'milestoneName', 'dueDate', 'daysOverdue', 'actionUrl'],
    htmlContent: `<div dir="ltr"><h1 style="color:#dc2626;">Project Milestone Overdue</h1><p>Hello {{recipientName}},</p><p>A project milestone is overdue:</p><ul><li>Project: <strong>{{projectName}}</strong></li><li>Milestone: {{milestoneName}}</li><li>Was Due: {{dueDate}}</li><li>Days Overdue: {{daysOverdue}}</li></ul><p><a href="{{actionUrl}}">View Project</a></p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1 style="color:#dc2626;">تأخر إنجاز المرحلة</h1><p>مرحباً {{recipientName}}،</p><p>مرحلة المشروع متأخرة:</p><ul><li>المشروع: <strong>{{projectName}}</strong></li><li>المرحلة: {{milestoneName}}</li><li>أيام التأخير: {{daysOverdue}}</li></ul></div>`,
    textContent: 'Milestone Overdue\n\nHello {{recipientName}},\n\nProject: {{projectName}}\nMilestone: {{milestoneName}}\nDue: {{dueDate}}\nDays Overdue: {{daysOverdue}}\n\n---\n\nتأخر إنجاز المرحلة\n\nمرحباً {{recipientName}}،\n\nالمشروع: {{projectName}}\nالمرحلة: {{milestoneName}}\nأيام التأخير: {{daysOverdue}}',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'project-stalled',
    name: 'Project Stalled / المشروع متوقف',
    subject: 'Project Stalled — Action Needed: {{projectName}} / المشروع متوقف — مطلوب إجراء',
    category: 'projects',
    isActive: true,
    variables: ['recipientName', 'projectName', 'stalledSince', 'lastActivity', 'actionUrl'],
    htmlContent: `<div dir="ltr"><h1 style="color:#f59e0b;">Project Stalled — Action Needed</h1><p>Hello {{recipientName}},</p><p>The following project appears to be stalled:</p><ul><li>Project: <strong>{{projectName}}</strong></li><li>Stalled Since: {{stalledSince}}</li><li>Last Activity: {{lastActivity}}</li></ul><p>Please review and take action to move it forward.</p><p><a href="{{actionUrl}}">View Project</a></p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1 style="color:#f59e0b;">المشروع متوقف — مطلوب إجراء</h1><p>مرحباً {{recipientName}}،</p><p>يبدو أن المشروع التالي متوقف:</p><ul><li>المشروع: <strong>{{projectName}}</strong></li><li>متوقف منذ: {{stalledSince}}</li></ul></div>`,
    textContent: 'Project Stalled — Action Needed\n\nHello {{recipientName}},\n\nProject: {{projectName}}\nStalled Since: {{stalledSince}}\nLast Activity: {{lastActivity}}\n\n---\n\nالمشروع متوقف\n\nمرحباً {{recipientName}}،\n\nالمشروع: {{projectName}}\nمتوقف منذ: {{stalledSince}}',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'project-member-added',
    name: 'Added to Project / الانضمام إلى مشروع',
    subject: 'You Have Been Added to a Project — {{projectName}} / تمت إضافتك إلى مشروع',
    category: 'projects',
    isActive: true,
    variables: ['recipientName', 'projectName', 'role', 'addedBy', 'startDate', 'actionUrl'],
    htmlContent: `<div dir="ltr"><h1>Added to Project</h1><p>Hello {{recipientName}},</p><p>You have been added to the following project:</p><ul><li>Project: <strong>{{projectName}}</strong></li><li>Your Role: {{role}}</li><li>Added by: {{addedBy}}</li><li>Start Date: {{startDate}}</li></ul><p><a href="{{actionUrl}}">View Project</a></p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1>الانضمام إلى مشروع</h1><p>مرحباً {{recipientName}}،</p><p>تمت إضافتك إلى المشروع التالي:</p><ul><li>المشروع: <strong>{{projectName}}</strong></li><li>دورك: {{role}}</li><li>أضافك: {{addedBy}}</li></ul></div>`,
    textContent: 'Added to Project\n\nHello {{recipientName}},\n\nProject: {{projectName}}\nRole: {{role}}\nAdded by: {{addedBy}}\nStart: {{startDate}}\n\n---\n\nالانضمام إلى مشروع\n\nمرحباً {{recipientName}}،\n\nالمشروع: {{projectName}}\nدورك: {{role}}',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'project-stage-deadline-reminder',
    name: 'Stage Deadline Reminder / تذكير بموعد المرحلة',
    subject: 'Stage Deadline Reminder — {{projectName}} / تذكير بموعد المرحلة',
    category: 'projects',
    isActive: true,
    variables: ['recipientName', 'projectName', 'stageName', 'dueDate', 'daysRemaining', 'actionUrl'],
    htmlContent: `<div dir="ltr"><h1>Stage Deadline Reminder</h1><p>Hello {{recipientName}},</p><p>A project stage deadline is approaching:</p><ul><li>Project: <strong>{{projectName}}</strong></li><li>Stage: {{stageName}}</li><li>Due: {{dueDate}}</li><li>Days Remaining: <strong>{{daysRemaining}}</strong></li></ul><p><a href="{{actionUrl}}">View Stage</a></p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1>تذكير بموعد المرحلة</h1><p>مرحباً {{recipientName}}،</p><p>موعد مرحلة المشروع يقترب:</p><ul><li>المشروع: <strong>{{projectName}}</strong></li><li>المرحلة: {{stageName}}</li><li>الموعد: {{dueDate}}</li><li>الأيام المتبقية: <strong>{{daysRemaining}}</strong></li></ul></div>`,
    textContent: 'Stage Deadline Reminder\n\nHello {{recipientName}},\n\nProject: {{projectName}}\nStage: {{stageName}}\nDue: {{dueDate}}\nDays Remaining: {{daysRemaining}}\n\n---\n\nتذكير بموعد المرحلة\n\nمرحباً {{recipientName}}،\n\nالمشروع: {{projectName}}\nالأيام المتبقية: {{daysRemaining}}',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  ...
  // === CRM ===
  {
    id: 'crm-opportunity-stage-changed',
    name: 'Opportunity Stage Updated / تحديث مرحلة الفرصة',
    subject: 'Opportunity Stage Updated — {{opportunityName}} / تم تحديث مرحلة الفرصة',
    category: 'crm',
    isActive: true,
    variables: ['recipientName', 'opportunityName', 'partnerName', 'oldStage', 'newStage', 'updatedBy', 'actionUrl'],
    htmlContent: `<div dir="ltr"><h1>Opportunity Stage Updated</h1><p>Hello {{recipientName}},</p><p>A CRM opportunity has moved to a new stage:</p><ul><li>Opportunity: <strong>{{opportunityName}}</strong></li><li>Partner: {{partnerName}}</li><li>Stage: {{oldStage}} → <strong>{{newStage}}</strong></li><li>Updated by: {{updatedBy}}</li></ul><p><a href="{{actionUrl}}">View Opportunity</a></p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1>تحديث مرحلة الفرصة</h1><p>مرحباً {{recipientName}}،</p><p>انتقلت فرصة CRM إلى مرحلة جديدة:</p><ul><li>الفرصة: <strong>{{opportunityName}}</strong></li><li>المرحلة الجديدة: <strong>{{newStage}}</strong></li></ul></div>`,
    textContent: 'Opportunity Stage Updated\n\nHello {{recipientName}},\n\nOpportunity: {{opportunityName}}\nPartner: {{partnerName}}\nStage: {{oldStage}} → {{newStage}}\nUpdated by: {{updatedBy}}\n\n---\n\nتحديث مرحلة الفرصة\n\nمرحباً {{recipientName}}،\n\nالفرصة: {{opportunityName}}\nالمرحلة الجديدة: {{newStage}}',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'crm-opportunity-won',
    name: 'Opportunity Won / تم الفوز بالفرصة',
    subject: 'Opportunity Won! 🎉 — {{opportunityName}} / تم الفوز بالفرصة!',
    category: 'crm',
    isActive: true,
    variables: ['recipientName', 'opportunityName', 'partnerName', 'value', 'wonBy', 'actionUrl'],
    htmlContent: `<div dir="ltr"><h1 style="color:#059669;">Opportunity Won! 🎉</h1><p>Hello {{recipientName}},</p><p>Great news! An opportunity has been won:</p><ul><li>Opportunity: <strong>{{opportunityName}}</strong></li><li>Partner: {{partnerName}}</li><li>Value: {{value}}</li><li>Won by: {{wonBy}}</li></ul><p><a href="{{actionUrl}}">View Details</a></p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1 style="color:#059669;">تم الفوز بالفرصة! 🎉</h1><p>مرحباً {{recipientName}}،</p><p>أخبار رائعة! تم الفوز بالفرصة:</p><ul><li>الفرصة: <strong>{{opportunityName}}</strong></li><li>الشريك: {{partnerName}}</li><li>القيمة: {{value}}</li></ul></div>`,
    textContent: 'Opportunity Won!\n\nHello {{recipientName}},\n\nOpportunity: {{opportunityName}}\nPartner: {{partnerName}}\nValue: {{value}}\nWon by: {{wonBy}}\n\n---\n\nتم الفوز بالفرصة!\n\nمرحباً {{recipientName}}،\n\nالفرصة: {{opportunityName}}\nالشريك: {{partnerName}}',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'crm-partner-created',
    name: 'New Partner Added / إضافة شريك جديد',
    subject: 'New CRM Partner Added — {{partnerName}} / تمت إضافة شريك جديد',
    category: 'crm',
    isActive: true,
    variables: ['recipientName', 'partnerName', 'partnerType', 'country', 'addedBy', 'actionUrl'],
    htmlContent: `<div dir="ltr"><h1>New Partner Added</h1><p>Hello {{recipientName}},</p><p>A new partner has been added to the CRM:</p><ul><li>Name: <strong>{{partnerName}}</strong></li><li>Type: {{partnerType}}</li><li>Country: {{country}}</li><li>Added by: {{addedBy}}</li></ul><p><a href="{{actionUrl}}">View Partner</a></p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1>إضافة شريك جديد</h1><p>مرحباً {{recipientName}}،</p><p>تمت إضافة شريك جديد إلى CRM:</p><ul><li>الاسم: <strong>{{partnerName}}</strong></li><li>النوع: {{partnerType}}</li><li>البلد: {{country}}</li></ul></div>`,
    textContent: 'New Partner Added\n\nHello {{recipientName}},\n\nPartner: {{partnerName}}\nType: {{partnerType}}\nCountry: {{country}}\nAdded by: {{addedBy}}\n\n---\n\nإضافة شريك جديد\n\nمرحباً {{recipientName}}،\n\nالشريك: {{partnerName}}\nالنوع: {{partnerType}}',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  ...
  {
    id: 'workspace-share',
    name: 'File Shared With You / مشاركة ملف',
    subject: 'File Shared With You — {{fileName}} / تمت مشاركة ملف معك',
    category: 'workflow',
    isActive: true,
    variables: ['recipientName', 'fileName', 'sharedBy', 'fileType', 'accessLevel', 'actionUrl'],
    htmlContent: `<div dir="ltr"><h1>File Shared With You</h1><p>Hello {{recipientName}},</p><p><strong>{{sharedBy}}</strong> has shared a file with you:</p><ul><li>File: <strong>{{fileName}}</strong></li><li>Type: {{fileType}}</li><li>Access Level: {{accessLevel}}</li></ul><p><a href="{{actionUrl}}">Open File</a></p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1>تمت مشاركة ملف معك</h1><p>مرحباً {{recipientName}}،</p><p>شارك <strong>{{sharedBy}}</strong> ملفاً معك:</p><ul><li>الملف: <strong>{{fileName}}</strong></li><li>النوع: {{fileType}}</li><li>مستوى الوصول: {{accessLevel}}</li></ul></div>`,
    textContent: 'File Shared With You\n\nHello {{recipientName}},\n\n{{sharedBy}} shared {{fileName}} with you.\nAccess Level: {{accessLevel}}\n\n---\n\nتمت مشاركة ملف\n\nمرحباً {{recipientName}}،\n\nشارك {{sharedBy}} الملف {{fileName}} معك.',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'workspace-access-granted',
    name: 'Workspace Access Granted / منح الوصول للمساحة',
    subject: 'Workspace Access Granted — {{workspaceName}} / تم منحك الوصول إلى مساحة العمل',
    category: 'workflow',
    isActive: true,
    variables: ['recipientName', 'workspaceName', 'grantedBy', 'accessLevel', 'actionUrl'],
    htmlContent: `<div dir="ltr"><h1 style="color:#059669;">Workspace Access Granted</h1><p>Hello {{recipientName}},</p><p>You have been granted access to the following workspace:</p><ul><li>Workspace: <strong>{{workspaceName}}</strong></li><li>Access Level: {{accessLevel}}</li><li>Granted by: {{grantedBy}}</li></ul><p><a href="{{actionUrl}}">Open Workspace</a></p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1 style="color:#059669;">تم منحك الوصول إلى مساحة العمل</h1><p>مرحباً {{recipientName}}،</p><p>تم منحك الوصول إلى مساحة العمل:</p><ul><li>المساحة: <strong>{{workspaceName}}</strong></li><li>مستوى الوصول: {{accessLevel}}</li></ul></div>`,
    textContent: 'Workspace Access Granted\n\nHello {{recipientName}},\n\nWorkspace: {{workspaceName}}\nAccess Level: {{accessLevel}}\nGranted by: {{grantedBy}}\n\n---\n\nتم منح الوصول\n\nمرحباً {{recipientName}}،\n\nالمساحة: {{workspaceName}}\nمستوى الوصول: {{accessLevel}}',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  ...
  // === BROADCAST / SYSTEM ===
  {
    ...
    name: 'System Broadcast / إعلان النظام',
    subject: 'Announcement: {{title}} / إعلان: {{title}}',
    category: 'system',
    isActive: true,
    variables: ['recipientName', 'title', 'message', 'announcedBy', 'effectiveDate'],
    htmlContent: `<div dir="ltr"><h1>System Announcement</h1><p>Hello {{recipientName}},</p><h2>{{title}}</h2><p>{{message}}</p><p style="color:#6b7280;font-size:12px;">Announced by: {{announcedBy}} • Effective: {{effectiveDate}}</p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1>إعلان النظام</h1><p>مرحباً {{recipientName}}،</p><h2>{{title}}</h2><p>{{message}}</p><p style="color:#6b7280;font-size:12px;">أعلن بواسطة: {{announcedBy}} • تاريخ السريان: {{effectiveDate}}</p></div>`,
  ...
  {
    id: 'daily-digest',
    name: 'Daily Action Summary / الملخص اليومي',
    subject: 'Your Daily Summary — {{date}} / ملخص يومك',
    category: 'system',
    isActive: true,
    variables: ['recipientName', 'date', 'pendingTasks', 'pendingApprovals', 'overdueItems', 'upcomingDeadlines', 'actionUrl'],
    htmlContent: `<div dir="ltr"><h1>Daily Action Summary</h1><p>Hello {{recipientName}},</p><p>Here's your summary for <strong>{{date}}</strong>:</p><ul><li>Pending Tasks: {{pendingTasks}}</li><li>Pending Approvals: {{pendingApprovals}}</li><li>Overdue Items: <span style="color:#dc2626;">{{overdueItems}}</span></li><li>Upcoming Deadlines: {{upcomingDeadlines}}</li></ul><p><a href="{{actionUrl}}">Go to Dashboard</a></p></div><hr style="margin:25px 0;border:0;border-top:1px solid #eee;"><div dir="rtl" style="text-align:right;"><h1>الملخص اليومي</h1><p>مرحباً {{recipientName}}،</p><p>إليك ملخص يوم <strong>{{date}}</strong>:</p><ul><li>المهام المعلقة: {{pendingTasks}}</li><li>الموافقات المعلقة: {{pendingApprovals}}</li><li>البنود المتأخرة: <span style="color:#dc2626;">{{overdueItems}}</span></li></ul></div>`,
    textContent: 'Daily Action Summary\n\nHello {{recipientName}},\n\nSummary for {{date}}:\nPending Tasks: {{pendingTasks}}\nPending Approvals: {{pendingApprovals}}\nOverdue Items: {{overdueItems}}\nUpcoming Deadlines: {{upcomingDeadlines}}\n\n---\n\nالملخص اليومي\n\nمرحباً {{recipientName}}،\n\nالمهام المعلقة: {{pendingTasks}}\nالموافقات المعلقة: {{pendingApprovals}}',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
];
...
const CURRENT_TEMPLATES_VERSION = '3.0-all-event-types';
...
const TEMPLATE_CATEGORIES: TemplateCategory[] = [
  ...
  { id: 'tasks', name: 'Tasks', nameAr: 'المهام', icon: Bell, description: 'Task lifecycle, assignments, overdue, dependencies' },
  { id: 'hr', name: 'HR & Payroll', nameAr: 'الموارد البشرية والرواتب', icon: UserPlus, description: 'Leave, payroll, contracts, timesheets, advances' },
  { id: 'projects', name: 'Projects & MMP', nameAr: 'المشاريع وخطط المراقبة', icon: FolderOpen, description: 'Project stages, milestones, MMP lifecycle' },
  { id: 'crm', name: 'CRM', nameAr: 'إدارة العملاء', icon: MessageSquare, description: 'Opportunities, partners, engagements, contacts' },
  { id: 'workflow', name: 'Workflow', nameAr: 'سير العمل', icon: FolderOpen, description: 'Approvals, signatures, workspace sharing' },
  { id: 'system', name: 'System', nameAr: 'النظام', icon: Wrench, description: 'Maintenance, alerts, system updates, daily digest' },
  ...
  { id: 'reports', name: 'Reports', nameAr: 'التقارير', icon: FolderOpen, description: 'Weekly and monthly summary reports' },
...
export default function EmailManagement() {
  ...
  const sendBroadcastTestEmail = async () => {
    ...
    try {
      // Step 1: resolve admin/super_admin user IDs from the DB
      ...
      // Step 2: dispatch with explicit recipient_ids (not recipient_roles)
      ...
      toast({
        ...
        description: `Sent to ${recipientIds.length} admin/super_admin user(s). Check inboxes for the new email format with web + mobile buttons.`,
      });
    } catch (err: any) {
      toast({
        title: 'Send failed',
