...
/// Cost Submission Screen for PACT Mobile App
/// 
/// Features 4 tabs matching the web app:
/// 1. Submit - New cost submission form
  ...
  Widget _buildReconciliationTab() {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        return _buildSubmissionCard(disbursedSubmissions[index], showReconcileButton: true);
      },
    );
  }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            // Approval status
            if (submission.tier1Status != TierApprovalStatus.pending ||
                submission.tier2Status != TierApprovalStatus.pending) ...[
              const SizedBox(height: 12),
              const Divider(),
              const SizedBox(height: 8),
...
class _OperationalCostFormState extends State<OperationalCostForm> {
      ...
      child: Column(
        ...
        children: [
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    isArabic
                        ? 'هذا النموذج للمصروفات التشغيلية فقط. التقديمات تتطلب موافقة ثنائية المستوى.'
                        : 'This form is for operational expenses only. Submissions require two-tier approval.',
          ...
          DropdownButtonFormField<OperationalExpenseCategory>(
            ...
            hint: Text(isArabic ? 'اختر الفئة' : 'Select category'),
            items: OperationalExpenseCategory.values.map((category) {
              return DropdownMenuItem(
                value: category,
                child: Text(isArabic ? category.labelAr : category.labelEn),
              );
            }).toList(),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      onChanged: (value) => setState(() => _currency = value ?? 'SDG'),
                    ),
                  ],
                ),
              ),
          ...
          // Expense Date
          Text(
            isArabic ? 'تاريخ المصروف *' : 'Expense Date *',
            style: const TextStyle(fontWeight: FontWeight.w600),
          ),
          const SizedBox(height: 8),
          InkWell(
          ...
          // Vendor (Optional)
          Text(
            isArabic ? 'المورد (اختياري)' : 'Vendor (Optional)',
            style: const TextStyle(fontWeight: FontWeight.w600),
          ),
          const SizedBox(height: 8),
          TextFormField(
          ...
          Text(
            isArabic ? 'رقم المرجع (اختياري)' : 'Reference Number (Optional)',
            style: const TextStyle(fontWeight: FontWeight.w600),
          ),
          const SizedBox(height: 8),
          TextFormField(
            ...
            decoration: InputDecoration(
              ...
              hintText: isArabic ? 'رقم الفاتورة / الإيصال' : 'Invoice/Receipt number',
            ),
          ),
          const SizedBox(height: 24),
            ...
            Column(
              children: [
                ...
                TextButton.icon(
                  ...
                  label: Text(
                    widget.isArabic ? 'إضافة مستند آخر' : 'Add another document',
                  ),
                ),
              ],
            ),
  ...
  Future<void> _pickDocument() async {
    // TODO: Implement document picker using image_picker or file_picker
    // This is a placeholder - implement based on your preferred package
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          widget.isArabic
